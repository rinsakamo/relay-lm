...
5. Do not expose or hand-build `tools.physical_execution_queue -- <arbitrary child>` in normal operation.
6. The engine is fixed to llama.cpp and the current single-GPU resource key is fixed to `llama-cpp:local-gpu`.
7. While queued, another cooperative job causes `WAITING_RESOURCE`.
8. After lease acquisition, an external llama.cpp process/listener causes `WAITING_EXTERNAL_RUNTIME`. Never kill or reuse it.
9. The final gate rechecks the exact checkout, interpreter, target module, required distributions, protected branch remote head, and the pushed execution-branch head when present.
10. If authority drifted during the wait, stop before target invocation, release, reprepare, and requeue. The scientific owner remains unspent.
11. Once the registered target wrapper is invoked, never retry/replay/reseed/fallback through this controller.
12. Preserve the target result unchanged. The queue receipt is infrastructure-only.
