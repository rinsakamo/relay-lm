...
15. After lease acquisition, an external llama.cpp process or a target listener address that is not presently bindable causes `WAITING_EXTERNAL_RUNTIME`. Never kill or reuse it. Bindability, not only successful TCP connection, is the controller-side listener criterion because the target must be able to acquire that exact address before its owned server launch. If the process-table root itself cannot be enumerated, stop fail-closed instead of treating external process state as idle; individual process entries may disappear during enumeration and are best-effort.
