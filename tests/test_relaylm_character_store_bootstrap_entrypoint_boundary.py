"""Focused regression for the supported character-store bootstrap invocation boundary."""
...
def test_character_store_bootstrap_uses_only_canonical_installed_entrypoint() -> None:
    records = invocations.collect_all()
    console_records = [
        record
        for record in records
        if record.root_id == "console_script:relaylm-character-store-bootstrap"
    ]
