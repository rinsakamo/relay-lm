...
def test_r2_public_task_surface_quarantines_hidden_evaluator_information():
    ...
    for task in preregistration.tasks:
        public = json.dumps(task.public_mapping(), sort_keys=True)
        assert "hidden_regime" not in public
        assert "expected_answer" not in public
        assert task.hidden_regime not in public
        if task.retrieval_packet is not None:
            assert task.retrieval_packet not in public
        if task.observation_packet is not None:
            assert task.observation_packet not in public
...
def test_r2_allocator_message_cannot_see_regime_expected_answer_or_packets():
    preregistration = build_preregistration(_COMMIT_A)
    for task in preregistration.tasks:
        serialized = json.dumps(
            allocator_messages(task, base_answer="BASE"),
            ensure_ascii=False,
            sort_keys=True,
        )
        assert task.hidden_regime not in serialized
        assert task.expected_answer not in serialized or task.expected_answer == "BASE"
        if task.retrieval_packet is not None:
...
def test_r2_oracle_is_quarantined_counterfactual_and_tie_breaks_to_cheapest_correct():
    ...
    bank = TaskOperationBank(
        ...
        operation_results=(
            OperationResult("THINK", task.expected_answer, True, ResourceVector(calls=1, input_tokens=20)),
            OperationResult(
                "RETRIEVE",
                task.expected_answer,
                True,
...
def test_r2_interpretation_no_oracle_headroom():
    result = interpret_r2(
...
def test_r2_interpretation_heuristic_sufficient():
    result = interpret_r2(
        {
            "A0": _vector(25),
            "A1": _vector(34),
