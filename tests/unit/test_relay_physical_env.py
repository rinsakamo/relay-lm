...
def _symlinked_identity(tmp_path: Path) -> envtool.PhysicalEnvironmentIdentity:
...
def _bootstrap_for(
    identity: envtool.PhysicalEnvironmentIdentity,
    tmp_path: Path,
) -> Path:
...
def test_current_environment_rejects_bootstrap_even_when_resolved_binary_matches(
    ...
    identity = _symlinked_identity(tmp_path)
