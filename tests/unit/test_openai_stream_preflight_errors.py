...
class _UnusedStreamingProvider:
    async def generate(self, _: CognitiveInput) -> CognitiveOutput:
        raise AssertionError("invalid character must fail before provider generation")

    async def stream_generate(self, _: CognitiveInput, emit) -> CognitiveOutput:
        raise AssertionError("invalid character must fail before provider streaming")
...
def test_pre_emission_provider_failure_returns_http_502(tmp_path: Path) -> None:
    ...
    app = create_app(character=character, provider=_ImmediateProtocolFailureProvider())
...
def test_pre_emission_character_failure_returns_http_500(tmp_path: Path) -> None:
    character = _make_character(tmp_path)
    app = create_app(character=character, provider=_UnusedStreamingProvider())
    ...
    assert response.json() == {"detail": "character package is invalid"}
