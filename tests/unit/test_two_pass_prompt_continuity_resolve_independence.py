...
def test_pass2_prompt_keeps_continuity_resolution_kind_local() -> None:
    ...
    assert "Evaluate these meanings independently" in content
    assert (
    ...
    assert "`referent` is a specific cross-turn reference target" in content
    assert "`unresolved` is an open question or unknown value" in content
    assert "`active_task` is unfinished work" in content
