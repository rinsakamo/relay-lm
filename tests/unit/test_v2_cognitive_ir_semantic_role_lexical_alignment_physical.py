...
import relaylm.v2_cognitive_ir_semantic_role_lexical_alignment_physical as physical
from relaylm.v2_cognitive_ir_semantic_role_lexical_alignment_physical import (
    SemanticRoleLexicalAlignmentPhysicalError,
...
from tools.v2_cognitive_ir_semantic_role_lexical_alignment_llama_cpp import (
    SemanticRoleLexicalAlignmentLlamaCppClient,
    StrictOutputInputCounterPreflight,
)
import tools.v2_cognitive_ir_semantic_role_lexical_alignment_llama_cpp_transaction as tx
import tools.v2_cognitive_ir_semantic_role_lexical_alignment_llama_cpp_wsl as wsl
...
def test_binding_freezes_24_72_144_without_materializing_families(
    ...
    def forbidden_material(_seed: int) -> dict[str, object]:
        raise AssertionError("binding validation must not instantiate E4-LX1 material")
...
def test_synthetic_material_rejects_all_preregistered_e4_lx1_seeds() -> None:
    ...
    for seed in preregistered_seeds():
        with pytest.raises(SemanticRoleLexicalAlignmentPhysicalError):
...
def test_transaction_freezes_counter_before_material(monkeypatch) -> None:
    ...
    def forbidden_material(_seed: int) -> dict[str, object]:
        raise AssertionError("consumer freeze must not instantiate E4-LX1 material")
...
def test_synthetic_campaign_is_exact_72_calls_144_counts_and_e_d_o_paired() -> None:
    ...
    assert result.descriptive_exact_vs_opaque is not None
...
def test_surface_roles_are_exact_descriptive_and_opaque_without_legend() -> None:
    truth = generate_synthetic_canonical_payload(91_000_000_000_777)
    control = prepare_synthetic_mechanism_control(91_000_000_000_778, truth)
    exact = control.serialized_by_surface[EXACT_LEXEME_SURFACE]
    descriptive = control.serialized_by_surface[DESCRIPTIVE_SURFACE]
    opaque = control.serialized_by_surface[OPAQUE_SURFACE]
    assert '"operation"' in exact and '"permutation"' in exact
    assert '"transform_kind"' in descriptive
    assert '"index_reordering"' in descriptive
    assert '"additive_shifts"' in descriptive
    assert '"modular_divisor"' in descriptive
    assert '"source_handles"' in descriptive
    for forbidden in (
        '"operation"',
        '"permutation"',
        '"offsets"',
        ...
        assert forbidden not in descriptive
    messages = json.dumps(build_reconstruction_messages(descriptive), sort_keys=True)
    assert "source_handles -> provenance_handles" not in messages
    assert "transform_kind -> operation" not in messages
    assert '"refs"' in opaque and '"kind"' in opaque
...
def test_llama_cpp_transport_uses_wire_only_schema_and_frozen_envelope() -> None:
    truth = generate_synthetic_canonical_payload(91_000_000_000_779)
    ...
    try:
        completion = client.complete_named(
            ...
            build_reconstruction_messages(
                control.serialized_by_surface[SURFACES[0]]
            ),
...
def test_wsl_adapter_pins_common_generation_and_routes_transaction(monkeypatch) -> None:
    ...
    monkeypatch.setattr(wsl, "verify_common_physical_binding", lambda: identity)
    ...
    assert wsl.main(["--repo-root", "."]) == 19
    assert observed == [
        (
            (
                "tools.v2_cognitive_ir_semantic_role_lexical_alignment_"
                "llama_cpp_transaction"
            ),
...
def test_common_target_routes_only_through_wsl_transaction() -> None:
    ...
    target = targets["v2:semantic-role-lexical-alignment"]
    assert target.branch == "v2"
    assert target.module == (
        "tools.v2_cognitive_ir_semantic_role_lexical_alignment_llama_cpp_wsl"
    )
