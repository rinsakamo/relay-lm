...
from tools.v2_cognitive_ir_semantic_role_binding_llama_cpp import (
    SemanticRoleBindingLlamaCppClient,
    StrictOutputInputCounterPreflight,
)
import tools.v2_cognitive_ir_semantic_role_binding_llama_cpp_transaction as role_tx
import tools.v2_cognitive_ir_semantic_role_binding_llama_cpp_wsl as role_wsl
...
def test_binding_freezes_24_72_144_without_materializing_families(monkeypatch) -> None:
    def forbidden_material(_seed: int) -> dict[str, object]:
        raise AssertionError("binding validation must not instantiate E4-RB1 material")
...
def test_synthetic_material_rejects_all_preregistered_e4_rb1_seeds() -> None:
    ...
    for seed in preregistered_seeds():
        with pytest.raises(SemanticRoleBindingPhysicalError):
...
def test_transaction_freezes_counter_before_material(monkeypatch) -> None:
    ...
    def forbidden_material(_seed: int) -> dict[str, object]:
        raise AssertionError("consumer freeze must not instantiate E4-RB1 material")

    monkeypatch.setattr(role_tx, "preflight_strict_output_input_counter", fake_preflight)
    monkeypatch.setattr(physical, "_legacy_generate_canonical_payload", forbidden_material)
    identity, counter = role_tx._prepare_consumer_identity(
        repository_commit="a" * 40,
        repository_tree="b" * 40,
        model="gemma-test",
        runtime_attestation=_runtime_attestation(),
...
def test_synthetic_campaign_is_exact_72_calls_144_counts_and_three_surface_paired() -> None:
    client = _CampaignClient()
    result = run_semantic_role_binding_campaign(
        client,
        frozen_identity=_identity(),
    ...
    assert result.classification == "NO_DECLARED_SURFACE_GAP_DETECTED_AT_THIS_RESOLUTION"
    ...
    assert result.descriptive_typed_vs_opaque is not None
    ...
    for index in range(24):
        ...
        assert family[0].canonical_truth == family[1].canonical_truth == family[2].canonical_truth


def test_shape_valid_domain_failure_remains_scientific_outcome_not_admission_failure() -> None:
...
def test_llama_cpp_transport_uses_wire_only_native_schema_and_frozen_envelope() -> None:
    ...
    client = SemanticRoleBindingLlamaCppClient(
        base_url="http://127.0.0.1:1234/v1",
        model="gemma-test",
    ...
    try:
        completion = client.complete_named(
            ...
            build_reconstruction_messages(control.serialized_by_surface[SURFACES[0]]),
...
def test_wsl_adapter_pins_current_common_generation_and_routes_transaction(monkeypatch) -> None:
    root = Path(__file__).resolve().parents[2]
    identity = role_wsl.verify_common_physical_binding(root)
    ...
    monkeypatch.setattr(role_wsl, "verify_common_physical_binding", lambda: identity)
    ...
    assert role_wsl.main(["--repo-root", "."]) == 19
    assert observed == [
        (
            "tools.v2_cognitive_ir_semantic_role_binding_llama_cpp_transaction",
            "relaylm2-cognitive-ir-semantic-role-binding-wsl-wall-time-v1",
        )
    ]
...
def test_common_target_routes_only_through_wsl_transaction() -> None:
    root = Path(__file__).resolve().parents[2]
    registry = json.loads(
    ...
    target = registry["targets"]["v2:semantic-role-binding"]
    assert target["branch"] == "v2"
    assert target["module"] == (
        "tools.v2_cognitive_ir_semantic_role_binding_llama_cpp_wsl"
    )
    assert target["required_distributions"] == ["httpx"]
    loaded = _load_targets(root)["v2:semantic-role-binding"]
    assert loaded.module == target["module"]
    assert loaded.required_distributions == ("httpx",)


def test_repository_transform_authority_is_reused_without_official_seed() -> None:
    truth = generate_synthetic_canonical_payload(91_000_000_001_234)
    control = prepare_physical_mechanism_control(truth)
    repository_control = prepare_synthetic_mechanism_control(
        91_000_000_009_999,
        truth,
    )
    assert control.canonical_truth == repository_control.canonical_truth
    assert control.semantic_digest == repository_control.semantic_digest
    assert control.serialized_by_surface == repository_control.serialized_by_surface
    assert tuple(control.serialized_by_surface) == SURFACES
    assert TYPED_SURFACE in control.serialized_by_surface
    assert ROLE_EXPLICIT_SURFACE in control.serialized_by_surface
    assert OPAQUE_SURFACE in control.serialized_by_surface
