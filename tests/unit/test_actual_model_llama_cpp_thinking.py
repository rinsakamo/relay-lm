...
def test_thinking_counter_preserves_exact_template_control_in_both_requests() -> None:
    observed: list[dict[str, Any]] = []

    def post_json(_: str, payload: Mapping[str, Any], __: str | None) -> object:
        observed.append(dict(payload))
        return {"input_tokens": 12 if any(m["content"] for m in payload["messages"]) else 4}
    ...
    assert dict(counter.evidence_identity.parameters)["thinking_control"] == (
        "chat_template_kwargs.enable_thinking=false"
    )


def test_thinking_counter_rejects_unqualified_template_kwargs() -> None:
    counter = LlamaCppThinkingChatInputCounter(
        base_url="http://127.0.0.1:1234/v1",
        runtime_identity=_runtime(),
