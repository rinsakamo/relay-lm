...
theorem compileHistoryLaw_pathWeight {q : Nat} (law : HistoryLaw q)
  ...
  | cons a as ih =>
      change law h a *
          FinitelyBranchingKernel.pathWeight (compileHistoryLaw law) (h ++ [a]) as =
        law h a * historyLawPathWeight law (h ++ [a]) as
      rw [ih (h ++ [a])]
...
theorem compileHistoryLaw_run_eq_append {q : Nat} (law : HistoryLaw q)
  ...
  | cons a as ih =>
      change FinitelyBranchingKernel.run (compileHistoryLaw law) (h ++ [a]) as =
        h ++ (a :: as)
      calc
        FinitelyBranchingKernel.run (compileHistoryLaw law) (h ++ [a]) as
            = (h ++ [a]) ++ as := ih (h ++ [a])
        _ = h ++ (a :: as) := by simp [List.append_assoc]
