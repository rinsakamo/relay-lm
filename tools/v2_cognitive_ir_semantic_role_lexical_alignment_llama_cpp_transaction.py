...
class SemanticRoleLexicalAlignmentTransactionError(RuntimeError):
    """The #2857 E4-LX1 transaction cannot bind the frozen probe."""
    ...
    for index, item in enumerate(values):
        ...
        if item == name:
            if index + 1 >= len(values):
                raise SemanticRoleLexicalAlignmentTransactionError(
...
def _prepare_consumer_identity(
    *,
    ...
    if (
        ...
        raise SemanticRoleLexicalAlignmentTransactionError(
            "E4-LX1 structured input-counter preflight did not complete 2/2"
        )
...
def _finalize_artifacts(
    *,
    artifact_root: Path,
    run: SemanticRoleLexicalAlignmentLlamaCppRun,
        ...
        result.update(
            {
                ...
                "confirmatory": None,
                "descriptive_exact_vs_opaque": None,
                "completed": False,
    ...
    legacy._write_json(
        artifact_root / "semantic-role-lexical-alignment-result.json",
        result,
    )
    legacy._write_json(
        artifact_root / "semantic-role-lexical-alignment-receipt.json",
        {
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    def finalize_from_end_binding(
        *,
        ...
        if (
            not isinstance(run, SemanticRoleLexicalAlignmentLlamaCppRun)
    ...
    def lexical_alignment_runner(
        *,
        ...
        if not isinstance(binding, Mapping):
            raise SemanticRoleLexicalAlignmentTransactionError(
                "runtime identity was not frozen before E4-LX1 runner entry"
            )
        runtime_attestation = binding.get("runtime_attestation")
        if not isinstance(runtime_attestation, Mapping):
            raise SemanticRoleLexicalAlignmentTransactionError(
                "runtime attestation is missing before E4-LX1 material boundary"
            )

        repository = probe_s2_git_repository(repo_root)
        if not repository.clean:
            raise SemanticRoleLexicalAlignmentTransactionError(
                "E4-LX1 requires a clean repository checkout"
            )
        ...
        if not isinstance(artifact_root, Path):
            raise SemanticRoleLexicalAlignmentTransactionError(
                "fresh E4-LX1 artifact root was not captured"
            )
        legacy._write_json(
            artifact_root / "semantic-role-lexical-alignment-manifest.json",
            {
                "claim": CLAIM,
        ...
        run = run_llama_cpp_semantic_role_lexical_alignment(
            base_url=base_url,
            model=model,
    ...
    legacy.run_llama_cpp_shared_floor_calibration = lexical_alignment_runner
