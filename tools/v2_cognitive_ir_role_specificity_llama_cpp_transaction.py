...
from tools.v2_cognitive_ir_role_specificity_llama_cpp import (
    SemanticRoleSpecificityLlamaCppRun,
    StrictOutputInputCounterPreflight,
    preflight_strict_output_input_counter,
    run_llama_cpp_semantic_role_specificity,
)


class SemanticRoleSpecificityTransactionError(RuntimeError):
    """The #2889 E4-RS1 transaction cannot bind the frozen probe."""
    ...
    for index, item in enumerate(values):
        ...
        if item == name:
            if index + 1 >= len(values):
                raise SemanticRoleSpecificityTransactionError(
...
def _prepare_consumer_identity(
    *,
    ...
    if (
        ...
        raise SemanticRoleSpecificityTransactionError(
            "E4-RS1 structured input-counter preflight did not complete 2/2"
        )
...
def _finalize_artifacts(
    *,
    artifact_root: Path,
    run: SemanticRoleSpecificityLlamaCppRun,
        ...
        result.update(
            {
                ...
                "confirmatory": None,
                "descriptive_function_vs_opaque": None,
                "completed": False,
    ...
    legacy._write_json(
        artifact_root / "semantic-role-specificity-result.json",
        result,
    )
    legacy._write_json(
        artifact_root / "semantic-role-specificity-receipt.json",
        {
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    def finalize_from_end_binding(
        *,
        ...
        if (
            not isinstance(run, SemanticRoleSpecificityLlamaCppRun)
    ...
    def role_specificity_runner(
        *,
        ...
        if not isinstance(binding, Mapping):
            raise SemanticRoleSpecificityTransactionError(
                "runtime identity was not frozen before E4-RS1 runner entry"
            )
        runtime_attestation = binding.get("runtime_attestation")
        if not isinstance(runtime_attestation, Mapping):
            raise SemanticRoleSpecificityTransactionError(
                "runtime attestation is missing before E4-RS1 material boundary"
            )

        repository = probe_s2_git_repository(repo_root)
        if not repository.clean:
            raise SemanticRoleSpecificityTransactionError(
                "E4-RS1 requires a clean repository checkout"
            )
        ...
        if not isinstance(artifact_root, Path):
            raise SemanticRoleSpecificityTransactionError(
                "fresh E4-RS1 artifact root was not captured"
            )
        legacy._write_json(
            artifact_root / "semantic-role-specificity-manifest.json",
            {
                "claim": CLAIM,
        ...
        run = run_llama_cpp_semantic_role_specificity(
            base_url=base_url,
            model=model,
    ...
    legacy.run_llama_cpp_shared_floor_calibration = role_specificity_runner
