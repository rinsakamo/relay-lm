...
INNER_TRANSACTION_MODULE = "tools.v2_cognitive_ir_s3_r5_llama_cpp_transaction"
...
WALL_TIME_SCHEMA = "relaylm2-cognitive-ir-s3-r5-wsl-wall-time-v1"


class S3R5WslLauncherError(RuntimeError):
    """The S3-R5 WSL launcher cannot establish its writable exact-checkout envelope."""
...
def _write_wall_time_sidecar(
    *,
    ...
    if wall_seconds < 0:
        raise S3R5WslLauncherError("monotonic wall time must be non-negative")
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Create one writable WSL envelope and invoke the frozen #2211 S3-R5 "
    ...
    runtime_root = Path(
        tempfile.mkdtemp(prefix="relaylm-v2-2211-s3-r5-wsl-runtime-")
    ).resolve()
    runtime_home = runtime_root / "home"
    runtime_home.mkdir(parents=True, exist_ok=False)
    artifact_root = runtime_root / "artifacts"
    transaction_summary_path = artifact_root / "s3-r5-llama-cpp-transaction-summary.json"
    wall_time_path = artifact_root / "s3-r5-wsl-wall-time.json"
    try:
        DEFAULT_SHARED_LOCK_PATH.parent.mkdir(parents=True, exist_ok=True)
    except OSError as exc:
        raise S3R5WslLauncherError(
            "cannot prepare shared S3-R5 lifecycle-lock directory: "
    ...
    try:
        ...
        raise S3R5WslLauncherError(
            f"failed to invoke frozen S3-R5 transaction once: {exc}"
        ) from exc
    ...
    print(
        json.dumps(
            {
                "s3_r5_wsl_wall_time_path": str(wall_time_path),
                "s3_r5_wsl_wall_time_sha256": _sha256_file(wall_time_path),
                "wall_seconds": wall_seconds,
            },
            ensure_ascii=False,
            sort_keys=True,
