...
from tools.v2_cognitive_ir_semantic_role_binding_llama_cpp import (
    SemanticRoleBindingLlamaCppRun,
    StrictOutputInputCounterPreflight,
    preflight_strict_output_input_counter,
    run_llama_cpp_semantic_role_binding,
)


class SemanticRoleBindingTransactionError(RuntimeError):
    """The #2834 E4-RB1 transaction adapter cannot bind the frozen probe."""
    ...
    for index, item in enumerate(values):
        ...
        if item == name:
            if index + 1 >= len(values):
                raise SemanticRoleBindingTransactionError(
...
def _prepare_consumer_identity(
    *,
    ...
    if (
        ...
        raise SemanticRoleBindingTransactionError(
            "E4-RB1 structured input-counter preflight did not complete 2/2 requests"
        )
...
def _finalize_artifacts(
    *,
    artifact_root: Path,
    run: SemanticRoleBindingLlamaCppRun,
        ...
        result.update(
            {
                ...
                "confirmatory": None,
                "descriptive_typed_vs_opaque": None,
                "completed": False,
    ...
    legacy._write_json(
        artifact_root / "semantic-role-binding-result.json",
        result,
    )
    legacy._write_json(
        artifact_root / "semantic-role-binding-receipt.json",
        {
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    def finalize_from_end_binding(
        *,
        ...
        if (
            not isinstance(run, SemanticRoleBindingLlamaCppRun)
    ...
    def semantic_role_binding_runner(
        *,
        base_url: str,
        model: str,
        before_call=None,
        ...
        if not isinstance(binding, Mapping):
            raise SemanticRoleBindingTransactionError(
                "runtime identity was not frozen before E4-RB1 runner entry"
            )
        runtime_attestation = binding.get("runtime_attestation")
        if not isinstance(runtime_attestation, Mapping):
            raise SemanticRoleBindingTransactionError(
                "runtime attestation is missing before E4-RB1 material boundary"
            )

        repository = probe_s2_git_repository(repo_root)
        if not repository.clean:
            raise SemanticRoleBindingTransactionError(
                "E4-RB1 requires a clean repository checkout"
            )
        ...
        if not isinstance(artifact_root, Path):
            raise SemanticRoleBindingTransactionError(
                "fresh E4-RB1 artifact root was not captured"
            )
        legacy._write_json(
            artifact_root / "semantic-role-binding-manifest.json",
            {
                "claim": CLAIM,
                ...
                "typed_vs_opaque": "descriptive_only",
            },
        )

        run = run_llama_cpp_semantic_role_binding(
            base_url=base_url,
            model=model,
    ...
    legacy.run_llama_cpp_shared_floor_calibration = semantic_role_binding_runner
