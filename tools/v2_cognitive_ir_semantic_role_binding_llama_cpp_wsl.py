...
INNER_TRANSACTION_MODULE_SEMANTIC_ROLE_BINDING = (
    "tools.v2_cognitive_ir_semantic_role_binding_llama_cpp_transaction"
)
WALL_TIME_SCHEMA_SEMANTIC_ROLE_BINDING = (
...
TARGET_NAME = "v2:semantic-role-binding"
TARGET_BRANCH = "v2"
TARGET_MODULE = INNER_TRANSACTION_MODULE_SEMANTIC_ROLE_BINDING.removesuffix(
    "_transaction"
) + "_wsl"
TARGET_REQUIRED_DISTRIBUTIONS = ("httpx",)


class SemanticRoleBindingWslLauncherError(RuntimeError):
    """The #2834 WSL adapter cannot bind the E4-RB1 transaction."""
...
def verify_common_physical_binding(
    ...
    if target is None:
        raise SemanticRoleBindingWslLauncherError(
    ...
    if (
        ...
        raise SemanticRoleBindingWslLauncherError(
            "E4-RB1 common target registry binding drifted"
        )
    return identity
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    legacy.INNER_TRANSACTION_MODULE = INNER_TRANSACTION_MODULE_SEMANTIC_ROLE_BINDING
    legacy.WALL_TIME_SCHEMA = WALL_TIME_SCHEMA_SEMANTIC_ROLE_BINDING
    try:
