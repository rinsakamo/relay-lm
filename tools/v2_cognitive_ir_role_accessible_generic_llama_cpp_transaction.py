...
class RoleAccessibleGenericTransactionError(RuntimeError):
    """The #2904 E5-RA1 transaction cannot bind the frozen probe."""
    ...
    for index, item in enumerate(values):
        ...
        if item == name:
            if index + 1 >= len(values):
                raise RoleAccessibleGenericTransactionError(
...
def _prepare_consumer_identity(
    *,
    ...
    counter_preflight = preflight_role_accessible_generic_input_counter(
        base_url=base_url,
        model=model,
    )
    if (
        ...
        raise RoleAccessibleGenericTransactionError(
            "E5-RA1 strict input-counter preflight did not complete 2/2"
        )
...
def _finalize_artifacts(
    *,
    artifact_root: Path,
    run: RoleAccessibleGenericLlamaCppRun,
    counter_preflight: RoleAccessibleGenericInputCounterPreflight,
        ...
        result.update(
            {
                ...
                "citable_for_downstream_claim": False,
                "confirmatory": None,
                "descriptive_typed_vs_legacy": None,
                "completed": False,
    ...
    legacy._write_json(
        artifact_root / "role-accessible-generic-result.json",
        result,
    )
    legacy._write_json(
        artifact_root / "role-accessible-generic-receipt.json",
        {
            ...
            "hidden_repair": 0,
            "judge": 0,
            "alternate_model_runtime_interface": 0,
        },
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    def finalize_from_end_binding(
        *, binding: Mapping[str, object] | None, error: str | None
    ) -> None:
        ...
        if (
            not isinstance(run, RoleAccessibleGenericLlamaCppRun)
    ...
    def role_accessible_generic_runner(
        *, base_url: str, model: str, before_call=None
    ):
        binding = captured.get("start_binding")
        if not isinstance(binding, Mapping):
            raise RoleAccessibleGenericTransactionError(
                "runtime identity was not frozen before E5-RA1 runner entry"
            )
        runtime_attestation = binding.get("runtime_attestation")
        if not isinstance(runtime_attestation, Mapping):
            raise RoleAccessibleGenericTransactionError(
                "runtime attestation is missing before E5-RA1 material boundary"
            )

        repository = probe_s2_git_repository(repo_root)
        if not repository.clean:
            raise RoleAccessibleGenericTransactionError(
                "E5-RA1 requires a clean repository checkout"
            )
        ...
        if not isinstance(artifact_root, Path):
            raise RoleAccessibleGenericTransactionError(
                "fresh E5-RA1 artifact root was not captured"
            )
        legacy._write_json(
            artifact_root / "role-accessible-generic-manifest.json",
            {
                "claim": CLAIM,
                "citable_only_when_measurement_admitted": True,
                ...
                "scientific_input_token_requests": SCIENTIFIC_INPUT_TOKEN_REQUESTS,
                "mechanical_preflight_input_token_requests": asdict(
                    counter_preflight
                ),
                "preregistered_seed_identities": list(preregistered_seeds()),
                "material_generation_boundary": (
                    "after_live_runtime_and_e5_counter_preflight_freeze"
                ),
                "confirmatory": list(CONFIRMATORY_CONTRASTS),
                "descriptive_only": DESCRIPTIVE_ONLY_CONTRAST,
            },
        )

        run = run_llama_cpp_role_accessible_generic(
            base_url=base_url,
            model=model,
    ...
    legacy.run_llama_cpp_shared_floor_calibration = role_accessible_generic_runner
    legacy.SHARED_FLOOR_CLAIM = CLAIM
    legacy.SHARED_FLOOR_CITABLE = False
    legacy.SHARED_FLOOR_ARCHITECTURE_CONSEQUENCE = ARCHITECTURE_CONSEQUENCE
    legacy.CALIBRATION_INCOMPLETE = "INVALID_BEFORE_PHYSICAL_EXECUTION"
    legacy.SHARED_FLOOR_MAX_SEMANTIC_CALLS = SEMANTIC_PROVIDER_CALLS
    legacy.SHARED_FLOOR_MAX_INPUT_TOKEN_REQUESTS = SCIENTIFIC_INPUT_TOKEN_REQUESTS
    legacy.SHARED_FLOOR_INPUT_TOKEN_REQUESTS_PER_DIFFICULTY = (
        SCIENTIFIC_INPUT_TOKEN_REQUESTS
    )
