...
from relaylm.v2_cognitive_ir_p2_termination_qual import (
    P2_TERMINATION_QUAL_CLAIM,
    P2_TERMINATION_QUAL_INCOMPLETE,
    P2_TERMINATION_QUAL_LABEL,
...
class P2TerminationQualificationTransactionError(RuntimeError):
    """The frozen non-citable P2 termination qualification cannot proceed truthfully."""
...
def _fresh_artifact_root(value: str | None) -> Path:
    if value:
        ...
        return path
    return Path(
        tempfile.mkdtemp(prefix="relaylm-v2-2211-p2-termination-qual-artifacts-")
    ).resolve()


def _fingerprint(repo: Mapping[str, object], runtime: Mapping[str, object]) -> tuple[str, str]:
    payload = ["relaylm2-p2-termination-qualification-identity-v1", P2_TERMINATION_QUAL_LABEL, dict(repo), dict(runtime)]
    digest = hashlib.sha256(_canonical(payload).encode("utf-8")).hexdigest()
    return f"sha256:{digest}", f"p2q-{digest}"


def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run the frozen non-citable 12-call P2 termination qualification exactly once."
    )
    ...
    summary_path = (
        ...
        else artifact_root / "p2-termination-qual-summary.json"
    )
    lock_path = Path(args.lock_path).expanduser().resolve()
    log_path = Path.home() / "logs" / "relaylm-v2-2211-p2-termination-qual.log"

    summary: dict[str, Any] = {
        "format_version": TRANSACTION_FORMAT_VERSION,
        "label": P2_TERMINATION_QUAL_LABEL,
        "claim": P2_TERMINATION_QUAL_INCOMPLETE,
        "classification": "P2_TERMINATION_QUALIFICATION_INCOMPLETE",
        "disposition": None,
        "citable": False,
        "architecture_consequence": "NONE",
        "artifact_root": str(artifact_root),
        "planned_semantic_calls": P2_TERMINATION_QUAL_TOTAL_SEMANTIC_CALLS,
        "planned_input_token_requests": P2_TERMINATION_QUAL_TOTAL_INPUT_TOKEN_REQUESTS,
    ...
    client: P2TerminationQualificationClient | None = None
    exit_code = 3
    ...
    try:
        ...
        if args.origin.rstrip("/") != DEFAULT_ORIGIN or args.port != DEFAULT_PORT:
            raise P2TerminationQualificationTransactionError(
                f"qualification endpoint must be exactly {DEFAULT_ORIGIN}/v1"
            )
        if S2_SELECTED_LLAMA_CPP_ENDPOINT != f"{DEFAULT_ORIGIN}/v1":
            raise P2TerminationQualificationTransactionError("qualification transport endpoint drifted")
        if S2_SELECTED_LLAMA_CPP_REASONING_EFFORT != "none":
            raise P2TerminationQualificationTransactionError("qualification reasoning control drifted")
        if not artifact_path.is_file():
            raise P2TerminationQualificationTransactionError(f"canonical GGUF is not a file: {artifact_path}")
        ...
        if not _port_has_no_listener("127.0.0.1", args.port):
            raise P2TerminationQualificationTransactionError("127.0.0.1:1234 is occupied before qualification")
        ...
        if log_path.exists():
            raise P2TerminationQualificationTransactionError(f"fresh qualification log already exists: {log_path}")
        ...
        if not isinstance(attested, Mapping):
            raise P2TerminationQualificationTransactionError("qualification runtime attestation is not an object")
        ...
        runtime = {
            ...
            "temperature": 0.0,
            "seed": None,
            "max_output_tokens": P2_TERMINATION_QUAL_MAX_OUTPUT_TOKENS,
        ...
        client = P2TerminationQualificationClient(
            base_url=S2_SELECTED_LLAMA_CPP_ENDPOINT,
            model=request_model,
            call_plan=plan,
            timeout_seconds=S2_SELECTED_LLAMA_CPP_TIMEOUT_SECONDS,
        ...
        for regime in ("shared", "null", "mismatch", "shift"):
            for family_index in range(3):
                ...
                family = generate_qualification_family(regime, family_index)
                _lightweight_binding_check(
                    process=process,
                ...
                client.complete_named(
                    question_id,
                    build_bounded_p2_formation_messages(family),
                    output_kind="text",
                )
                call_index += 1
        ...
        if _canonical(end_binding) != _canonical(start_binding):
            raise P2TerminationQualificationTransactionError("runtime/material binding drifted during qualification")
        if _material_stat(server_binary) != binary_stat or _material_stat(artifact_path) != artifact_stat:
            raise P2TerminationQualificationTransactionError("runtime material stat drifted during qualification")

        if client.provider_attempts != P2_TERMINATION_QUAL_TOTAL_SEMANTIC_CALLS:
            raise P2TerminationQualificationTransactionError("qualification semantic call total drifted")
        if client.input_count_attempts != P2_TERMINATION_QUAL_TOTAL_INPUT_TOKEN_REQUESTS:
            raise P2TerminationQualificationTransactionError("qualification input-token request total drifted")
        if summary["lightweight_live_binding_check_count"] != P2_TERMINATION_QUAL_TOTAL_SEMANTIC_CALLS:
            raise P2TerminationQualificationTransactionError("qualification live-binding check total drifted")

        summary["claim"] = P2_TERMINATION_QUAL_CLAIM
        summary["classification"] = "P2_TERMINATION_QUALIFIED"
        summary["disposition"] = "ALL_12_TERMINATED_WITHIN_ENVELOPE"
        exit_code = 0
        return exit_code
    except (P2TerminationQualificationTransactionError, SelectedS2TransactionError, StructureProposalError) as exc:
        summary["classification"] = "P2_TERMINATION_QUALIFICATION_INCOMPLETE"
        ...
        if process is not None:
            ...
            try:
                ...
                if exit_code == 0:
                    summary["claim"] = P2_TERMINATION_QUAL_INCOMPLETE
                    summary["classification"] = "P2_TERMINATION_QUALIFICATION_INCOMPLETE"
