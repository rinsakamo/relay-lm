"""Thin v1 WSL adapter for the #2697 no-Pass1 discriminator."""
...
INNER_TRANSACTION = (
    "relaylm.actual_model_stage_r_llama_cpp_production_unresolved_no_pass1_transaction"
)
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Invoke exactly one production-context unresolved-only no-Pass1 "
            "transaction using an explicit retained epistemic-formation artifact."
        )
    )
