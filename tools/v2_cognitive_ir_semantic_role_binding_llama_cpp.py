...
class SemanticRoleBindingLlamaCppClient(S3LlamaCppClient):
    """Native wire-only strict-json-schema llama.cpp transport for E4-RB1."""
    ...
    def _response_format(
        ...
        if output_kind != "semantic_role_payload":
            raise ValueError(
                f"unsupported E4-RB1 output kind for {question_id}: {output_kind}"
            )
...
class SemanticRoleBindingLlamaCppRun:
    result: SemanticRoleBindingCampaignResult
...
def run_llama_cpp_semantic_role_binding(
    *,
    base_url: str,
    model: str,
    ...
    if base_url.rstrip("/") != S3_LLAMA_CPP_ENDPOINT:
        raise ValueError(
            f"E4-RB1 endpoint must be exactly {S3_LLAMA_CPP_ENDPOINT}"
        )
    frozen_identity.validate()
    if frozen_identity.model != model:
        raise ValueError("frozen consumer model differs from E4-RB1 request model")
    ...
    try:
        ...
        if client.provider_attempts != result.semantic_calls:
            raise ValueError("E4-RB1 provider attempt ledger drifted")
        if client.provider_completions != result.provider_completions:
            raise ValueError("E4-RB1 provider completion ledger drifted")
        if client.input_count_attempts != result.input_token_requests:
            raise ValueError("E4-RB1 input-count attempt ledger drifted")
        if client.input_count_completions != result.input_token_completions:
            raise ValueError("E4-RB1 input-count completion ledger drifted")
        if client.live_binding_checks != client.provider_attempts:
            raise ValueError("E4-RB1 live binding did not cover every semantic attempt")

        return SemanticRoleBindingLlamaCppRun(
            result=result,
