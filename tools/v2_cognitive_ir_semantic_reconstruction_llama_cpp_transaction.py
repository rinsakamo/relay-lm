...
from tools.v2_cognitive_ir_semantic_reconstruction_llama_cpp import (
    ...
    run_llama_cpp_semantic_reconstruction,
)
...
class SemanticReconstructionTransactionError(RuntimeError):
    """The #2723 transaction adapter cannot bind the frozen probe."""
    ...
    for index, item in enumerate(values):
        ...
        if item == name:
            if index + 1 >= len(values):
                raise SemanticReconstructionTransactionError(
...
def _prepare_consumer_identity(
    *,
    ...
    base_url: str,
) -> tuple[object, SemanticReconstructionInputCounterPreflight]:
    counter_preflight = preflight_semantic_reconstruction_input_counter(
        base_url=base_url,
        model=model,
    )
    if (
        ...
        raise SemanticReconstructionTransactionError(
            "exact input-token accounting preflight did not complete 2/2 requests"
        )
...
def _finalize_artifacts(
    *,
    artifact_root: Path,
    run: SemanticReconstructionLlamaCppRun,
    counter_preflight: SemanticReconstructionInputCounterPreflight,
    ...
    legacy._write_json(
        artifact_root / "semantic-reconstruction-result.json",
        result,
    )
    legacy._write_json(
        artifact_root / "semantic-reconstruction-receipt.json",
        {
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    def finalize_from_end_binding(
        *,
        ...
        if (
            not isinstance(run, SemanticReconstructionLlamaCppRun)
    ...
    def reconstruction_runner(
        *,
        ...
        if not isinstance(binding, Mapping):
            raise SemanticReconstructionTransactionError(
                "runtime identity was not frozen before scientific runner entry"
            )
        runtime_attestation = binding.get("runtime_attestation")
        if not isinstance(runtime_attestation, Mapping):
            raise SemanticReconstructionTransactionError(
                "runtime attestation is missing before scientific material boundary"
            )

        repository = probe_s2_git_repository(repo_root)
        if not repository.clean:
            raise SemanticReconstructionTransactionError(
                "semantic reconstruction requires a clean repository checkout"
            )
        ...
        if not isinstance(artifact_root, Path):
            raise SemanticReconstructionTransactionError(
                "fresh scientific artifact root was not captured"
            )
        legacy._write_json(
            artifact_root / "semantic-reconstruction-manifest.json",
            {
                "claim": CLAIM,
                "citable_when_clean": CITABLE_WHEN_CLEAN,
                ...
                "material_generation_boundary": (
                    "after_live_runtime_and_counter_preflight_freeze"
                ),
            },
        )

        run = run_llama_cpp_semantic_reconstruction(
            base_url=base_url,
            model=model,
    ...
    legacy.run_llama_cpp_shared_floor_calibration = reconstruction_runner
    legacy.SHARED_FLOOR_CLAIM = CLAIM
    legacy.SHARED_FLOOR_CITABLE = CITABLE_WHEN_CLEAN
