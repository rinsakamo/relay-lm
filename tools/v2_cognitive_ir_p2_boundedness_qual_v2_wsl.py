...
INNER_TRANSACTION_MODULE = "tools.v2_cognitive_ir_p2_boundedness_qual_v2_entry"
...
WALL_TIME_SCHEMA = "relaylm2-cognitive-ir-p2-boundedness-qual-v2-wsl-wall-time-v1"


class P2BoundednessQualV2WslLauncherError(RuntimeError):
    """The boundedness qualification launcher cannot establish its exact-checkout WSL envelope."""
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Create one writable WSL envelope and invoke P2 boundedness qualification v2 exactly once."
    )
    ...
    if not source_root.is_dir():
        raise P2BoundednessQualV2WslLauncherError(
    ...
    runtime_root = Path(
        tempfile.mkdtemp(prefix="relaylm-v2-2211-p2-boundedness-qual-v2-wsl-runtime-")
    ).resolve()
    runtime_home = runtime_root / "home"
    runtime_home.mkdir(parents=True, exist_ok=False)
    artifact_root = runtime_root / "artifacts"
    summary_path = artifact_root / "p2-boundedness-qual-v2-summary.json"
    wall_time_path = artifact_root / "p2-boundedness-qual-v2-wsl-wall-time.json"
    try:
    ...
    try:
        ...
        raise P2BoundednessQualV2WslLauncherError(
            f"failed to invoke boundedness qualification transaction once: {exc}"
        ) from exc
    ...
    print(
        json.dumps(
            {
                "p2_boundedness_qual_v2_summary_path": str(summary_path),
                "p2_boundedness_qual_v2_wsl_wall_time_path": str(wall_time_path),
                "p2_boundedness_qual_v2_wsl_wall_time_sha256": _sha256_file(wall_time_path),
                "wall_seconds": wall_seconds,
            },
            ensure_ascii=False,
            sort_keys=True,
