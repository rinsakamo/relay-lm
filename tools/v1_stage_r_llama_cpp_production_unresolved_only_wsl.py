"""One-command WSL boundary for the #2676 unresolved-only diagnostic."""
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Invoke exactly one production-context unresolved-only transaction "
            "using an explicit retained epistemic-formation artifact."
        )
    )
