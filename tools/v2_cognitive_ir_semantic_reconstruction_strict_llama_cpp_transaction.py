...
class StrictSemanticReconstructionTransactionError(RuntimeError):
    """The #2774 E4-SR2 transaction adapter cannot bind the frozen probe."""
    ...
    for index, item in enumerate(values):
        ...
        if item == name:
            if index + 1 >= len(values):
                raise StrictSemanticReconstructionTransactionError(
...
def _prepare_consumer_identity(
    *,
    ...
    if (
        ...
        raise StrictSemanticReconstructionTransactionError(
            "E4-SR2 structured input-counter preflight did not complete 2/2 requests"
        )
...
def _finalize_artifacts(
    *,
    artifact_root: Path,
    run: StrictSemanticReconstructionLlamaCppRun,
        ...
        result.update(
            {
                ...
                "paired": None,
                "paired_accuracy_difference": None,
                "completed": False,
    ...
    legacy._write_json(
        artifact_root / "semantic-reconstruction-strict-result.json",
        result,
    )
    legacy._write_json(
        artifact_root / "semantic-reconstruction-strict-receipt.json",
        {
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    def finalize_from_end_binding(
        *,
        ...
        if (
            not isinstance(run, StrictSemanticReconstructionLlamaCppRun)
    ...
    def strict_reconstruction_runner(
        *,
        base_url: str,
        model: str,
        before_call=None,
        ...
        if not isinstance(binding, Mapping):
            raise StrictSemanticReconstructionTransactionError(
                "runtime identity was not frozen before E4-SR2 runner entry"
            )
        runtime_attestation = binding.get("runtime_attestation")
        if not isinstance(runtime_attestation, Mapping):
            raise StrictSemanticReconstructionTransactionError(
                "runtime attestation is missing before E4-SR2 material boundary"
            )

        repository = probe_s2_git_repository(repo_root)
        if not repository.clean:
            raise StrictSemanticReconstructionTransactionError(
                "E4-SR2 requires a clean repository checkout"
            )
        ...
        if not isinstance(artifact_root, Path):
            raise StrictSemanticReconstructionTransactionError(
                "fresh E4-SR2 artifact root was not captured"
            )
        legacy._write_json(
            artifact_root / "semantic-reconstruction-strict-manifest.json",
            {
                "claim": CLAIM,
        ...
        run = run_llama_cpp_strict_semantic_reconstruction(
            base_url=base_url,
            model=model,
    ...
    legacy.run_llama_cpp_shared_floor_calibration = strict_reconstruction_runner
