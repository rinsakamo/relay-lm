...
INNER_TRANSACTION_MODULE_SEMANTIC_ROLE_SPECIFICITY = (
    "tools.v2_cognitive_ir_role_specificity_llama_cpp_transaction"
)
WALL_TIME_SCHEMA_SEMANTIC_ROLE_SPECIFICITY = (
    "relaylm2-cognitive-ir-semantic-role-specificity-wsl-wall-time-v1"
)
...
TARGET_NAME = "v2:semantic-role-specificity"
TARGET_BRANCH = "v2"
TARGET_MODULE = INNER_TRANSACTION_MODULE_SEMANTIC_ROLE_SPECIFICITY.removesuffix(
    "_transaction"
) + "_wsl"
TARGET_REQUIRED_DISTRIBUTIONS = ("httpx",)


class SemanticRoleSpecificityWslLauncherError(RuntimeError):
    """The #2889 WSL adapter cannot bind the E4-RS1 transaction."""
...
def verify_common_physical_binding(
    ...
    if target is None:
        raise SemanticRoleSpecificityWslLauncherError(
    ...
    if (
        ...
        raise SemanticRoleSpecificityWslLauncherError(
            "E4-RS1 common target registry binding drifted"
        )
    return identity
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    legacy.INNER_TRANSACTION_MODULE = INNER_TRANSACTION_MODULE_SEMANTIC_ROLE_SPECIFICITY
    legacy.WALL_TIME_SCHEMA = WALL_TIME_SCHEMA_SEMANTIC_ROLE_SPECIFICITY
    try:
