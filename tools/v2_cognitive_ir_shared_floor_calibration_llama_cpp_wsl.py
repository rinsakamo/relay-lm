...
class SharedFloorCalibrationWslLauncherError(RuntimeError):
    """The #2600 WSL launcher cannot establish its exact-checkout envelope."""
...
def _write_wall_time_sidecar(
    *,
    ...
    if wall_seconds < 0:
        raise SharedFloorCalibrationWslLauncherError(
            "monotonic wall time must be non-negative"
        )
    payload = {
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Create one writable WSL envelope and invoke the frozen #2600 "
            "non-citable shared-floor calibration exactly once."
        )
    )
    ...
    runtime_root = Path(
        tempfile.mkdtemp(
            prefix="relaylm-v2-2211-shared-floor-calibration-wsl-runtime-"
        )
    ).resolve()
    ...
    transaction_summary_path = (
        artifact_root / "shared-floor-calibration-transaction-summary.json"
    )
    wall_time_path = artifact_root / "shared-floor-calibration-wsl-wall-time.json"
    try:
        DEFAULT_SHARED_LOCK_PATH.parent.mkdir(parents=True, exist_ok=True)
    except OSError as exc:
        raise SharedFloorCalibrationWslLauncherError(
            "cannot prepare shared calibration lifecycle-lock directory: "
    ...
    child_env["PYTHONPATH"] = (
        exact_src
        if not inherited
        else os.pathsep.join((exact_src, inherited))
    )
    ...
    try:
        ...
        raise SharedFloorCalibrationWslLauncherError(
            f"failed to invoke frozen shared-floor calibration once: {exc}"
        ) from exc
    ...
    print(
        json.dumps(
            {
                "shared_floor_calibration_wsl_wall_time_path": str(
                    wall_time_path
                ),
                "shared_floor_calibration_wsl_wall_time_sha256": (
                    _sha256_file(wall_time_path)
                ),
                "wall_seconds": wall_seconds,
            },
            ensure_ascii=False,
