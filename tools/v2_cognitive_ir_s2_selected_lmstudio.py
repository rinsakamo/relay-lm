...
# Exact case/shape here follows the completed calibration-v2 Native binding.
_SELECTED_RUNTIME = {
    "architecture": "gemma4",
    "format": "gguf",
    "quantization": "Q4_K_M",
...
class SelectedS2OpenAIClient:
    """OpenAI-compatible selected-S2 client with completion-verified reasoning off.

    P2/P3 remain ordinary text formation calls. P4 and P0-P6 are machine-readable
    protocol surfaces and therefore use strict JSON Schema response_format. Every
    successful completion must independently expose zero reasoning tokens and no
    non-empty reasoning payload.
    """
    ...
    def complete(self, messages: tuple[dict[str, str], ...]) -> ExperimentCompletion:
        if self._call_index >= len(S2_SELECTED_CALL_PLAN):
            raise StructureProposalError("selected S2 attempted an undeclared extra provider call")
        question_id = S2_SELECTED_CALL_PLAN[self._call_index]
...
def _validate_selected_physical_condition(
    binding: Mapping[str, object],
    *,
    model: str,
) -> None:
    if model != _SELECTED_MODEL:
        raise S2HostError(f"selected-regime S2 is frozen to model {_SELECTED_MODEL!r}")
...
def run_lmstudio_selected_s2_transaction(
    *,
    ...
    """Run one fresh selected-regime #2211 S2 transaction from observed authority."""
    ...
    if (
        preregistration.planned_provider_calls != len(S2_SELECTED_CALL_PLAN)
        or len(S2_SELECTED_CALL_PLAN) != S2_SELECTED_PHYSICAL_CALLS
    ):
    ...
    try:
        ...
        identity: dict[str, object] = {
            ...
            **binding,
            ...
            "execution_order": list(S2_SELECTED_CALL_PLAN),
