...
RETRIEVAL_AUTHORIZATION_RECORD_KINDS = frozenset({'subjective_mem_decision', 'subjective_mem_lifecycle_transition'})
RETRIEVAL_EXCLUSION_REASONS = frozenset({'projection_row_invalid', 'current_selector_ambiguous', 'not_latest_persisted_revision', 'canonical_binding_unverified', 'receipt_unverified', 'authorization_unverified', 'scope_not_admitted', 'unresolved_intent', 'lifecycle_held', 'lifecycle_hidden', 'lifecycle_superseded', 'lifecycle_purged', 'lifecycle_unsupported', 'mutation_prepared', 'mutation_recovery_required', 'mutation_corrupt', 'mutation_unsupported', 'retrieval_not_eligible', 'retrieval_not_visible'})
_TOKEN_RE = re.compile('[A-Za-z0-9][A-Za-z0-9_.:-]*\\Z')
...
class SubjectiveMemRetrievalProjectionManifest:
    ...
    def to_digest_input(self) -> dict[str, object]:
        return _body(self, SUBJECTIVE_MEM_RETRIEVAL_PROJECTION_MANIFEST_SCHEMA, row_count=len(self.row_digests), canonical_authority=False, rebuildable=True)
...
class SubjectiveMemRetrievalSelection:
    ...
    def to_digest_input(self) -> dict[str, object]:
        return _body(self, SUBJECTIVE_MEM_RETRIEVAL_SELECTION_SCHEMA, runtime_handoff_performed=False, usage_event_written=False, content_free=True)
...
class SubjectiveMemRetrievalUsageEvent:
    ...
    def usage_slot_id(self) -> str:
        return _opaque('smretrievalusekey', '\x00'.join((self.request_correlation_digest, self.row_digest, self.idempotency_key_digest)))
    ...
    def to_dict(self) -> dict[str, object]:
        return {**self.to_digest_input(), 'usage_slot_id': self.usage_slot_id, 'usage_event_id': self.usage_event_id, 'result_id': self.result_id, 'event_digest': self.input_digest}
...
def validate_subjective_mem_retrieval_request(request: object) -> tuple[str, ...]:
    ...
    if not _digests((request.workspace_authority_digest, request.admitted_scope_binding_digest, request.query_plan_digest, request.request_correlation_digest, request.projection_manifest_digest)):
        reasons.append('subjective_mem_retrieval_request_digest_invalid')
    if type(request.memory_kinds) is not tuple or not request.memory_kinds or request.memory_kinds != tuple(sorted(set(request.memory_kinds))) or any((value not in RETRIEVAL_MEMORY_KINDS for value in request.memory_kinds)):
    ...
    if type(request.boundary) is not SubjectiveMemRetrievalBoundary or request.boundary.to_dict() != SubjectiveMemRetrievalBoundary().to_dict():
...
def validate_subjective_mem_retrieval_projection_row(row: object) -> tuple[str, ...]:
    ...
    reasons: list[str] = []
    tokens = (row.projection_generation_id, row.character_id, row.memory_id, row.page_id, row.block_id, row.current_selector_id, row.current_receipt_id, row.authorization_id, row.source_revision_schema, row.source_current_state_schema, row.source_page_schema, row.source_block_schema, row.source_renderer_revision, row.source_partition_revision, row.source_platform_revision)
    ...
    digests = (row.block_digest, row.revision_digest, row.current_selector_digest, row.current_receipt_digest, row.authorization_digest, row.workspace_authority_digest, row.scope_binding_digest)
    ...
    bools = (row.retrieval_eligible, row.retrieval_visible, row.current_selector_unambiguous, row.latest_persisted_revision, row.finalized_receipt_verified, row.authorization_verified, row.canonical_binding_verified, row.scope_admitted, row.unresolved_intent_present)
    if any((type(value) is not bool for value in bools)):
        reasons.append('subjective_mem_retrieval_projection_boolean_invalid')
    elif row.lifecycle_state in RETRIEVAL_LIFECYCLE_STATES and row.mutation_state in RETRIEVAL_MUTATION_STATES:
        expected = row.mutation_state == 'none' and row.lifecycle_state in {'active', 'pinned'}
        if row.retrieval_eligible is not expected:
            reasons.append('subjective_mem_retrieval_projection_eligibility_pair_invalid')
...
def subjective_mem_retrieval_exclusion_reasons(row: object) -> tuple[str, ...]:
    ...
    reasons: list[str] = []
    checks = ((row.current_selector_unambiguous, 'current_selector_ambiguous'), (row.latest_persisted_revision, 'not_latest_persisted_revision'), (row.canonical_binding_verified, 'canonical_binding_unverified'), (row.finalized_receipt_verified, 'receipt_unverified'), (row.authorization_verified, 'authorization_unverified'), (row.scope_admitted, 'scope_not_admitted'))
    reasons.extend((reason for value, reason in checks if not value))
    if row.unresolved_intent_present:
        reasons.append('unresolved_intent')
    if row.lifecycle_state not in {'active', 'pinned'}:
        reasons.append(f'lifecycle_{row.lifecycle_state}' if row.lifecycle_state in {'held', 'hidden', 'superseded', 'purged'} else 'lifecycle_unsupported')
    if row.mutation_state != 'none':
        reasons.append(f'mutation_{row.mutation_state}' if row.mutation_state in {'prepared', 'recovery_required', 'corrupt'} else 'mutation_unsupported')
...
def validate_subjective_mem_retrieval_projection_manifest(manifest: object) -> tuple[str, ...]:
    ...
    if type(manifest.row_digests) is not tuple or manifest.row_digests != tuple(sorted(set(manifest.row_digests))) or (not _digests(manifest.row_digests)):
...
def validate_subjective_mem_retrieval_exclusion(exclusion: object) -> tuple[str, ...]:
    ...
    if not _digest(exclusion.request_input_digest) or (exclusion.row_digest_or_null is not None and (not _digest(exclusion.row_digest_or_null))):
...
def validate_subjective_mem_retrieval_selection(*, request: object, manifest: object, rows: tuple[SubjectiveMemRetrievalProjectionRow, ...], selection: object) -> tuple[str, ...]:
    reasons = [*validate_subjective_mem_retrieval_request(request), *validate_subjective_mem_retrieval_projection_manifest(manifest)]
    ...
    if type(request) is not SubjectiveMemRetrievalRequest or type(manifest) is not SubjectiveMemRetrievalProjectionManifest:
        return _dedupe(reasons)
    if type(rows) is not tuple or any((type(row) is not SubjectiveMemRetrievalProjectionRow for row in rows)):
    ...
    if selection.projection_generation_id not in {request.projection_generation_id, manifest.projection_generation_id} or request.projection_generation_id != manifest.projection_generation_id:
        reasons.append('subjective_mem_retrieval_selection_generation_mismatch')
    if selection.projection_manifest_digest not in {request.projection_manifest_digest, manifest.manifest_digest} or request.projection_manifest_digest != manifest.manifest_digest:
        reasons.append('subjective_mem_retrieval_selection_manifest_mismatch')
    if type(selection.selected_row_digests) is not tuple or selection.selected_row_digests != tuple(sorted(set(selection.selected_row_digests))) or (not _digests(selection.selected_row_digests)):
        reasons.append('subjective_mem_retrieval_selection_row_digests_invalid')
    counts = (selection.candidate_count, selection.eligible_count, selection.selected_count, selection.total_token_estimate)
    if any((type(value) is not int or value < 0 for value in counts)):
        reasons.append('subjective_mem_retrieval_selection_counts_invalid')
    elif not (selection.candidate_count >= selection.eligible_count >= selection.selected_count == len(selection.selected_row_digests) and selection.selected_count <= request.candidate_limit and (selection.total_token_estimate <= request.token_budget)):
        reasons.append('subjective_mem_retrieval_selection_budget_invalid')
    if selection.policy_revision != SUBJECTIVE_MEM_RETRIEVAL_POLICY_REVISION:
        reasons.append('subjective_mem_retrieval_policy_revision_invalid')
    by_digest = {row.row_digest: row for row in rows}
    for digest in selection.selected_row_digests:
        row = by_digest.get(digest)
        if row is None or digest not in manifest.row_digests:
            reasons.append('subjective_mem_retrieval_selection_row_missing')
        elif row.projection_generation_id != selection.projection_generation_id or subjective_mem_retrieval_exclusion_reasons(row):
            reasons.append('subjective_mem_retrieval_selection_row_ineligible')
    return _dedupe(reasons)

def derive_subjective_mem_retrieval_usage_event(*, request: SubjectiveMemRetrievalRequest, manifest: SubjectiveMemRetrievalProjectionManifest, rows: tuple[SubjectiveMemRetrievalProjectionRow, ...], selection: SubjectiveMemRetrievalSelection, row: SubjectiveMemRetrievalProjectionRow, event_kind: str, occurred_at: str, idempotency_key: str) -> tuple[SubjectiveMemRetrievalUsageEvent | None, tuple[str, ...]]:
    reasons = list(validate_subjective_mem_retrieval_selection(request=request, manifest=manifest, rows=rows, selection=selection))
    if validate_subjective_mem_retrieval_projection_row(row):
        reasons.append('subjective_mem_retrieval_usage_row_invalid')
    if event_kind != 'grounded_context_admitted':
    ...
    if row.row_digest not in selection.selected_row_digests or subjective_mem_retrieval_exclusion_reasons(row):
        return (None, ('subjective_mem_retrieval_usage_binding_invalid',))
    assert canonical_time is not None
    return (SubjectiveMemRetrievalUsageEvent(projection_generation_id=request.projection_generation_id, request_input_digest=request.input_digest, request_correlation_digest=request.request_correlation_digest, selection_digest=selection.selection_digest, row_digest=row.row_digest, memory_id=row.memory_id, memory_revision=row.memory_revision, event_kind=event_kind, occurred_at=canonical_time, idempotency_key_digest=sha256_hex(idempotency_key.encode('utf-8')), policy_revision=SUBJECTIVE_MEM_RETRIEVAL_POLICY_REVISION), ())
...
def validate_subjective_mem_retrieval_usage_event(event: object) -> tuple[str, ...]:
    ...
    if not _digests((event.request_input_digest, event.request_correlation_digest, event.selection_digest, event.row_digest, event.idempotency_key_digest)):
        reasons.append('subjective_mem_retrieval_usage_digest_invalid')
    if event.event_kind != 'grounded_context_admitted':
        reasons.append('subjective_mem_retrieval_usage_event_kind_invalid')
...
def _token(value: object, max_length: int=256) -> bool:
    return isinstance(value, str) and 1 <= len(value) <= max_length and (_TOKEN_RE.fullmatch(value) is not None)
...
__all__ = [name for name in globals() if name.startswith('SUBJECTIVE_MEM_RETRIEVAL_') or name.startswith('RETRIEVAL_')]
__all__ += ['SubjectiveMemRetrievalBoundary', 'SubjectiveMemRetrievalExclusion', 'SubjectiveMemRetrievalProjectionManifest', 'SubjectiveMemRetrievalProjectionRow', 'SubjectiveMemRetrievalRequest', 'SubjectiveMemRetrievalSelection', 'SubjectiveMemRetrievalUsageEvent', 'derive_subjective_mem_retrieval_usage_event', 'subjective_mem_retrieval_exclusion_reasons', 'validate_subjective_mem_retrieval_exclusion', 'validate_subjective_mem_retrieval_projection_manifest', 'validate_subjective_mem_retrieval_projection_row', 'validate_subjective_mem_retrieval_request', 'validate_subjective_mem_retrieval_selection', 'validate_subjective_mem_retrieval_usage_event']
