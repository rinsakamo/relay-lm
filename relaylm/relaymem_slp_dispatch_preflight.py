...
def _validate_handoff_result(
    ...
    if (
        source.response_finalized is not True
...
def _validate_candidate(
    ...
    if not isinstance(runtime, Mapping):
        return None, ("a2_candidate_runtime_shape_invalid",)
    if (
        len(runtime) != len(_A2_CANDIDATE_FIELDS)
        or set(runtime) != _A2_CANDIDATE_FIELDS
    ):
    ...
    if (
        type(candidate.source_count) is not int
        or not 1 <= candidate.source_count <= _MAX_SOURCES
    ):
