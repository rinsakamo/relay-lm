...
Canonical implementation of RelayEMO's affect estimation and scene-hint
cleanup helpers, folded in from the ACG-5 slice (formerly
relaylm.relayemo_acg5). RelayEMO remains responsible for affect and
expression only; relaylm.relayemo_acg5 is kept as a compatibility import
path.
"""
...
def infer_scene_hint_type(text: str) -> str:
    """Return a non-authoritative scene hint for RelayEMO expression gating only."""
...
def build_scene_hint_candidate(
    *,
    scene_hint_type: str,
    ...
    """Build a content-free, non-authoritative RelayEMO scene hint candidate."""

    hint_type = scene_hint_type if scene_hint_type in SCENE_HINT_TYPES else "unknown"
    errors = [] if hint_type == scene_hint_type else ["unknown_enum_value"]
    governance = build_analyzer_candidate_artifact(
        ...
        reason_ids=reason_ids or ["fail_closed_candidate_source"],
        enum_values=[hint_type],
    )
    public_governance = content_free_projection(governance)
    return {
        "schema_version": "relayemo.scene_hint_candidate.v0",
        "candidate_present": hint_type != "unknown",
        "scene_type": hint_type,
        "source_class": governance["source"],
        "source_authoritative": False,
        "policy_authority": "none",
        "restrictive_only": True,
        "candidate_applied": False,
        "can_open_runtime_policy": can_open_runtime_policy(governance),
        "reason_ids": tuple(public_governance["reason_ids"]),
        "validation_error_ids": tuple(public_governance["validation_error_ids"]),
        "content_free": True,
...
def run_relayemo(
    *,
    ...
    scene_hint_candidate = build_scene_hint_candidate(
        ...
        source="heuristic",
        confidence=affect.get("confidence", 0.0),
    )
    ...
    artifact: dict[str, Any] = {
        ...
        "scene_hint_candidate": scene_hint_candidate,
        "scene_state": {
            "scene_type": scene_hint_type,
            "deprecated": True,
        ...
        "scene_hint_candidate_public": {
            ...
            **scene_hint_candidate["public_governance"],
        },
        "text_marker_preview": {
            "gate_open": False,
            "marker": "",
    ...
    synthetic = json.dumps(
        {
            ...
            "scene_hint_candidate": {"scene_type": scene_hint, "confidence": 0.6},
        }
    )
