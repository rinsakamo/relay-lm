...
def build_relayint_quick_clarification_preflight(
    *,
    ...
    preflight_applicable = source_candidate_action == "ask_clarification" and quick_clarification_allowed
    ...
    return {
        ...
        "content_free": True,
        "source_artifact_schema_version": source_schema_version if isinstance(source_schema_version, str) else None,
...
def build_relayint_quick_clarification_apply_plan(
    *,
    ...
    source_schema_version = relayint_quick_clarification_preflight.get("schema_version") if preflight_present else None
    ...
    scene_allows = isinstance(scene_gate, Mapping) and scene_gate.get("quick_clarification_allowed") is True
...
def build_relayint_request_compatibility_gate(payload: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    for condition, reason in (
        ...
        (tool_choice_present, "tool_choice_requested"),
        (functions_count > 0, "functions_requested"),
        (function_call_present, "function_call_requested"),
        ...
        (stop_present, "stop_sequence_requested"),
    ):
...
def _detected_reference_kind(*, prior_memory_count: int, continuation_count: int, pronoun_count: int) -> ReferenceKind:
...
def _ambiguity_detected(*, detected_reference_kind: ReferenceKind, ctx_signal_present: bool, explicit_prior_memory: bool) -> bool:
    if detected_reference_kind == "none" or explicit_prior_memory:
        return False
    return not ctx_signal_present


def _candidate_action(*, detected_reference_kind: ReferenceKind, explicit_prior_memory: bool, ambiguity_detected: bool, ctx_signal_present: bool) -> CandidateAction:
...
def _confidence_score(*, detected_reference_kind: ReferenceKind, explicit_prior_memory: bool, ambiguity_detected: bool, ctx_signal_present: bool) -> float:
    if explicit_prior_memory:
        return 0.82
    if ambiguity_detected:
        return 0.48
...
def _llm_path_reasons(*, ambiguity_detected: bool, confidence_bucket: str, detected_reference_kind: ReferenceKind) -> list[str]:
    reasons: list[str] = []
    if ambiguity_detected:
...
def _decision_reasons(*, detected_reference_kind: ReferenceKind, explicit_prior_memory: bool, ambiguity_detected: bool, mem_query_needed: bool, ctx_signal_present: bool) -> list[str]:
...
def _ctx_metadata_summary(ctx_hints: Mapping[str, Any]) -> dict[str, Any]:
    safe_keys = [key for key in ctx_hints.keys() if isinstance(key, str) and key in SAFE_CTX_KEYS]
...
def _modalities_constraint(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {"modalities_present": modalities_present, "modalities_count": modalities_count, "audio_modality_requested": audio_modality_requested, "audio_options_present": audio_options_present, "block_reasons": block_reasons}
