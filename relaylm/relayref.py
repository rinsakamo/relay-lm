...
def build_relayref_dry_run_artifact(
    *,
    relayscn_artifact: Mapping[str, Any] | None,
    messages: Sequence[Mapping[str, Any]] | None = None,
    ctx_hints: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    """Build a diagnostics-only RelayREF Wake-side recovery artifact.

    RelayREF stays dry-run in this MVP. It can recommend reflection or context
    repair, but it never applies a rewrite, performs forced sleep, resumes a
    handoff, or mutates backend payloads.
    """
    ...
    unresolved_reference = _detect_unresolved_reference(messages)
    ...
    return {
        ...
        "ctx_handoff_guess": ctx_handoff_guess,
        "context_rewrite": {
            "candidate": mode in {"context_repair", "suggest_reflect"},
            "applied": False,
            "auto_resume_allowed": False,
        },
        "forced_sleep_candidate": forced_sleep_candidate,
    }
...
def _parse_relayscn_artifact(relayscn_artifact: Mapping[str, Any] | None) -> dict[str, Any]:
    if not isinstance(relayscn_artifact, Mapping):
        return {
            "malformed": True,
            "scene_state": {"scene_type": "unknown", "confidence": 0.0, "stability": 0.0},
            "scene_policy": {},
    ...
    return {
        ...
        "persistence_block_reasons": (
            [str(reason) for reason in persistence_reasons]
            if isinstance(persistence_reasons, Sequence)
            and not isinstance(persistence_reasons, str)
            else []
        ),
    }
...
def _build_persistence_guard(parsed: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "persistence_block": safe_block or malformed,
        "persistence_block_reasons": safe_reasons,
        "safe_to_persist": not (safe_block or malformed),
    }
...
def _select_mode(
    *,
    ...
    if unresolved_reference:
        return "suggest_reflect"
    if confidence < LOW_CONFIDENCE_THRESHOLD or stability < LOW_STABILITY_THRESHOLD:
        return "suggest_reflect"
    return "none"
...
def _extract_ctx_handoff_guess(ctx_hints: Mapping[str, Any]) -> dict[str, Any] | None:
    ...
    if isinstance(raw, Mapping):
        value = raw.get("value") or raw.get("text") or raw.get("summary")
    else:
        value = raw
...
def _build_forced_sleep_candidate(scene_policy: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "candidate": candidate,
        "slp_mode": slp_mode if isinstance(slp_mode, str) else None,
        "apply_allowed": False,
        "reason": "relayscn_slp_mode" if candidate else None,
    }
