...
def parse_m3d_handoff(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if candidate_id and namespace and event_kind in EVENT_KINDS and memory_kind in KIND_TARGET and is_sha256(lineage) and is_sha256(key):
    ...
    expected_path = f"{TARGET_DIR[target_category]}/{key}.md" if target_category in TARGET_DIR and is_sha256(key) else ""
    ...
    if isinstance(page_bytes, bool) or not isinstance(page_bytes, int) or page_bytes != len(encoded):
    ...
    if parsed_page.get("valid") is not True:
        ...
        if not isinstance(summary, str) or not summary or summary != summary.strip() or len(summary) > MAX_SUMMARY:
            reasons.append("primary_writer_handoff_page_summary_invalid")
        if not isinstance(title, str) or title != title.strip() or len(title) > MAX_TITLE or bad_text(title) or any(char in title for char in "\n\r\t"):
            reasons.append("primary_writer_handoff_page_title_invalid")
        if parsed_page.get("body") != f"# Primary memory\n\n## Summary\n\n{summary.strip()}\n":
