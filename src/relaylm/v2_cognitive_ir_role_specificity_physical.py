...
from relaylm.v2_cognitive_ir_semantic_reconstruction_physical import FrozenConsumerIdentity
...
PHYSICAL_ADAPTER_ISSUE = 2889
INPUT_TOKEN_REQUESTS_PER_SEMANTIC_CALL = 2
SCIENTIFIC_INPUT_TOKEN_REQUESTS = SEMANTIC_COMPLETIONS * INPUT_TOKEN_REQUESTS_PER_SEMANTIC_CALL
CLAIM = "SEMANTIC_ROLE_SPECIFICITY_ACCESSIBILITY_PROBE"
_CONTROL_SENTINEL_SEED = 91_2889_000_000_001


class SemanticRoleSpecificityPhysicalError(ValueError):
    """The #2889 E4-RS1 physical adapter cannot proceed truthfully."""


def generate_synthetic_canonical_payload(seed: int) -> dict[str, object]:
    if isinstance(seed, bool) or not isinstance(seed, int) or seed < 0:
        raise SemanticRoleSpecificityPhysicalError("synthetic seed must be a non-negative integer")
    if seed in set(preregistered_seeds()):
        raise SemanticRoleSpecificityPhysicalError(
            "synthetic helper must not materialize a preregistered E4-RS1 seed"
        )
...
def generate_scientific_canonical_payload(
    *, index: int, seed: int, frozen_identity: FrozenConsumerIdentity
    ...
    if not isinstance(frozen_identity, FrozenConsumerIdentity):
        raise SemanticRoleSpecificityPhysicalError(
            "scientific E4-RS1 material requires FrozenConsumerIdentity"
        )
    ...
    if isinstance(index, bool) or not isinstance(index, int) or not 0 <= index < FAMILY_COUNT:
        raise SemanticRoleSpecificityPhysicalError("scientific E4-RS1 family index must be 0..23")
    if isinstance(seed, bool) or not isinstance(seed, int) or seed != seeds[index]:
        raise SemanticRoleSpecificityPhysicalError(
            "scientific E4-RS1 seed does not match preregistered family index"
        )
    return _legacy_generate_canonical_payload(seed)
...
def prepare_physical_mechanism_control(
    ...
    if _CONTROL_SENTINEL_SEED in set(preregistered_seeds()):
        raise SemanticRoleSpecificityPhysicalError(
            "physical transform sentinel collides with E4-RS1 preregistration"
        )
    return prepare_synthetic_mechanism_control(_CONTROL_SENTINEL_SEED, canonical_payload)


def physical_call_plan() -> tuple[str, ...]:
    plan = tuple(
        f"semantic-role-specificity:{index}:{seed}:{surface}"
        for index, seed, surface in semantic_call_plan()
    )
    if len(plan) != SEMANTIC_COMPLETIONS or len(set(plan)) != SEMANTIC_COMPLETIONS:
        raise SemanticRoleSpecificityPhysicalError("E4-RS1 physical semantic call plan drifted")
    return plan


class SemanticRoleSpecificityClient(Protocol):
...
FamilyFactory = Callable[[int, int, FrozenConsumerIdentity], Mapping[str, object]]
...
def _campaign_result(
    *,
    verdict: RoleSpecificityVerdict,
    cells: list[RoleSpecificityCell],
    confirmatory: Mapping[str, ConfirmatoryContrast] | None,
    descriptive: PairedTable | None,
    semantic_equal_families: int,
    client: SemanticRoleSpecificityClient,
    completed: bool,
    admitted: bool,
    failure: str | None,
) -> SemanticRoleSpecificityCampaignResult:
    return SemanticRoleSpecificityCampaignResult(
        ...
        descriptive_function_vs_opaque=descriptive,
        semantic_equal_families=semantic_equal_families,
        wire_shape_valid=sum(cell.score.wire_shape_valid for cell in cells),
        semantic_domain_valid=sum(cell.score.semantic_domain_valid for cell in cells),
...
def run_semantic_role_specificity_campaign(
    client: SemanticRoleSpecificityClient,
    *,
    ...
    function_vs_opaque: list[tuple[bool, bool]] = []
    ...
    for index, seed in enumerate(preregistered_seeds()):
        ...
        if set(control.serialized_by_surface) != set(SURFACES):
            raise SemanticRoleSpecificityPhysicalError("F/C/O mechanism-control surface set drifted")
        if len(set(control.semantic_digest_by_surface.values())) != 1:
            raise SemanticRoleSpecificityPhysicalError("F/C/O semantic digest equality drifted")
        semantic_equal_families += 1
        outcomes: dict[str, bool] = {}

        for surface in SURFACES:
            question_id = f"semantic-role-specificity:{index}:{seed}:{surface}"
            ...
            try:
                ...
                completed_input_attempts = INPUT_TOKEN_REQUESTS_PER_SEMANTIC_CALL * len(cells)
                if provider_attempts <= len(cells) and input_count_attempts <= completed_input_attempts:
                    raise
                verdict = classify_result(None, protocol_failure_after_spend=True)
            ...
            score = score_reconstruction(completion.content, control.canonical_truth)
            cost = ReconstructionCost(
        ...
        if set(outcomes) != set(SURFACES):
            raise AssertionError("E4-RS1 surface outcome ledger drifted")
        h1_outcomes.append((outcomes[FUNCTION_SURFACE], outcomes[CATEGORY_SURFACE]))
        h2_outcomes.append((outcomes[CATEGORY_SURFACE], outcomes[OPAQUE_SURFACE]))
        function_vs_opaque.append((outcomes[FUNCTION_SURFACE], outcomes[OPAQUE_SURFACE]))

    client.require_complete_plan()
    if len(cells) != SEMANTIC_COMPLETIONS:
        raise SemanticRoleSpecificityPhysicalError("campaign did not complete exactly 72 semantic cells")
    ...
    contrasts = confirmatory_analysis(
        h1_function_vs_category=h1_outcomes,
        h2_category_vs_opaque=h2_outcomes,
    )
    descriptive = descriptive_function_vs_opaque(function_vs_opaque)
...
def validate_physical_adapter_binding() -> None:
    validate_repository_binding()
    if PHYSICAL_ADAPTER_ISSUE != 2889:
        raise SemanticRoleSpecificityPhysicalError("E4-RS1 physical adapter owner drifted")
    if FAMILY_COUNT != 24 or SEMANTIC_COMPLETIONS != 72:
        raise SemanticRoleSpecificityPhysicalError("E4-RS1 24-family/72-call contract drifted")
    if INPUT_TOKEN_REQUESTS != 144 or SCIENTIFIC_INPUT_TOKEN_REQUESTS != 144:
        raise SemanticRoleSpecificityPhysicalError("E4-RS1 144-request input-token contract drifted")
    ...
    if TEMPERATURE != 0.0 or REASONING != "none" or REQUEST_SEED is not None:
        raise SemanticRoleSpecificityPhysicalError("E4-RS1 decoding envelope drifted")
    if STREAM is not False or PARALLEL_SEMANTIC_SLOTS != 1:
        raise SemanticRoleSpecificityPhysicalError("E4-RS1 stream/parallelism envelope drifted")
    if PHYSICAL_EXECUTION_AUTHORIZED is not False:
        raise SemanticRoleSpecificityPhysicalError("physical adapter must not authorize THIS RUN")
    if ARCHITECTURE_CONSEQUENCE != "NONE":
        raise SemanticRoleSpecificityPhysicalError("E4-RS1 architecture consequence drifted")
    candidate = response_format()
    if candidate.get("type") != "json_schema":
        raise SemanticRoleSpecificityPhysicalError("E4-RS1 native structured-output transport drifted")
    json_schema = candidate.get("json_schema")
    if not isinstance(json_schema, Mapping) or json_schema.get("strict") is not True:
        raise SemanticRoleSpecificityPhysicalError("E4-RS1 strict JSON-schema transport drifted")
    expected = tuple(
        f"semantic-role-specificity:{index}:{seed}:{surface}"
        for index, seed, surface in semantic_call_plan()
    )
    if physical_call_plan() != expected:
        raise SemanticRoleSpecificityPhysicalError("E4-RS1 physical call plan drifted")
