...
def _canonical_form(expr: Expr, env: dict[str, str]) -> object:
    ...
    if expr.symbol in _BINDERS:
        ...
        return ['apply', expr.symbol, [['var', canonical_name], _canonical_form(expr.args[1], inner)]]
    return ['apply', expr.symbol, [_canonical_form(arg, env) for arg in expr.args]]
...
class SemanticTransactionStore:
    ...
    def _create_genesis(self) -> str:
        ...
        generation = self._make_generation(parent=None, roots=(), anchors=(), provenance_head=None, target_cognition=None, horizon=None, tx_id=tx_id)
        self.generations[generation.generation_id] = generation
        return generation.generation_id

    def _make_generation(self, *, parent: str | None, roots: Iterable[str], anchors: Iterable[str], provenance_head: str | None, target_cognition: str | None, horizon: str | None, tx_id: str) -> Generation:
        root_tuple = tuple(sorted(set(roots)))
        anchor_tuple = tuple(sorted(set(anchors)))
        anchor_root = _digest(['anchors', anchor_tuple])
        payload = ['generation', parent, root_tuple, anchor_root, provenance_head, target_cognition, horizon, tx_id]
        generation_id = _digest(payload)
        return Generation(generation_id=generation_id, parent_generation_id=parent, active_roots=root_tuple, anchor_root=anchor_root, provenance_head=provenance_head, target_cognition=target_cognition, horizon=horizon, created_by_transaction=tx_id)
    ...
    def _provenance_record_id(self, *, origin: Origin, time: str, source: str, payload_ref: str | None, links: tuple[ProvenanceLink, ...]) -> str:
        return _digest(['provenance', origin, time, source, payload_ref, [[link.relation, link.target] for link in links]])

    def _append_provenance(self, *, origin: Origin, time: str, source: str, payload: str | None=None, payload_ref: str | None=None, links: tuple[ProvenanceLink, ...]=()) -> str:
        ...
        record_id = self._provenance_record_id(origin=origin, time=time, source=source, payload_ref=payload_ref, links=links)
        self.provenance.setdefault(record_id, ProvenanceRecord(id=record_id, origin=origin, time=time, source=source, payload_ref=payload_ref, links=links))
        return record_id

    def _latest_provenance_head(self) -> str | None:
        if not self.provenance:
            return None
        return max(self.provenance.values(), key=lambda record: (record.time, record.id)).id

    def _transaction_id(self, request: TransactionRequest, policy: GovernancePolicy) -> str:
        return _digest(['tx', request.base_generation, [[obs.slot, obs.time, obs.source, obs.payload] for obs in request.observations], [[canonical_form(proposal.expr), proposal.observed_support_slots, proposal.existing_provenance_support, proposal.revision_of, proposal.deactivate_roots, proposal.requested_anchors] for proposal in request.proposals], [[action.name, action.payload] for action in request.actions], sorted(policy.protected_root_symbols), sorted(policy.outcome_symbols), policy.allow_protected_roots])

    def transact(self, request: TransactionRequest, *, policy: GovernancePolicy=GovernancePolicy()) -> TransactionResult:
        ...
        for obs in request.observations:
            observation_records[obs.slot] = self._append_provenance(origin='observed', time=obs.time, source=obs.source, payload=obs.payload)
        ...
        for proposal_index, proposal in enumerate(request.proposals):
            decision = self._govern_proposal(proposal, observation_records=observation_records, policy=policy)
            if decision.status == 'accepted':
                decision = ProposalDecision('accepted', 'accepted', semantic_id=semantic_id(proposal.expr))
        ...
        for proposal_index, proposal, decision in staged:
            ...
            self._append_provenance(origin='endogenous', time=self._proposal_time(request, proposal_index), source=f'semantic-transaction:{tx_id}', payload=None, links=tuple(sorted(links, key=lambda link: (link.relation, link.target))))
        self.anchors.update(accepted_anchors)
        generation = self._make_generation(parent=request.base_generation, roots=active_roots, anchors=self.anchors, provenance_head=self._latest_provenance_head(), target_cognition=self.active_generation().target_cognition, horizon=self.active_generation().horizon, tx_id=tx_id)
        self.generations[generation.generation_id] = generation
        ...
        for action_index, action in enumerate(request.actions):
            action_records.append(self._append_provenance(origin='endogenous', time=self._action_time(request, action_index), source=f'action:{tx_id}', payload=action.payload, links=(ProvenanceLink('attempts', f'action:{action.name}'),)))
        return TransactionResult(generation_id=generation.generation_id, decisions=tuple(decisions), observation_records=tuple(observation_records.values()), action_records=tuple(action_records))
    ...
    def _govern_proposal(self, proposal: Proposal, *, observation_records: dict[str, str], policy: GovernancePolicy) -> ProposalDecision:
        try:
        ...
        support_ids.extend((observation_records[slot] for slot in proposal.observed_support_slots))
        ...
        if symbol in policy.protected_root_symbols and (not policy.allow_protected_roots):
    ...
    def derive_pressures(self, now: str) -> tuple[str, ...]:
        ...
        for expr in self.active_exprs():
            if isinstance(expr, Apply) and expr.symbol == 'deadline' and (len(expr.args) == 2) and isinstance(expr.args[1], Literal) and isinstance(expr.args[1].value, str):
                deadline = datetime.fromisoformat(expr.args[1].value.replace('Z', '+00:00'))
                if current >= deadline:
                    pressures.append(f'deadline_due:{semantic_id(expr)}')
        return tuple(sorted(pressures))

    def migrate_generation(self, generation_id: str, *, native_symbols: frozenset[str]) -> MigrationCheck:
        generation = self.generations[generation_id]
        definitions = self._active_symbol_definitions(generation)
        missing: set[str] = set()
        native = set(native_symbols) | {'defines', 'believes', 'hypothetical', 'counterfactual', 'not', 'same_as', 'different_from', 'forall', 'exists', 'lambda'}
    ...
    def _active_symbol_definitions(self, generation: Generation) -> set[str]:
        ...
        for root in generation.active_roots:
            expr = self.expr_for_id(root)
            if isinstance(expr, Apply) and expr.symbol == 'defines' and (len(expr.args) == 2) and isinstance(expr.args[0], Ref):
                defined.add(expr.args[0].anchor)
        return defined

    def _find_missing_symbols(self, expr: Expr, native: set[str], defined: set[str], missing: set[str]) -> None:
        if not isinstance(expr, Apply):
            return
    ...
    def garbage_collect_semantics(self) -> int:
        retained_generations = [generation for generation_id, generation in self.generations.items() if generation_id not in self.retired_generations]
        reachable = {root for generation in retained_generations for root in generation.active_roots}
        before = len(self.semantic_nodes)
        self.semantic_nodes = {node_id: payload for node_id, payload in self.semantic_nodes.items() if node_id in reachable}
        return before - len(self.semantic_nodes)

    def delete_payload(self, record_id: str, *, time: str, full_erasure: bool=False) -> str:
        record = self.provenance.get(record_id)
        if record is None:
        ...
        tombstone = self._append_provenance(origin='endogenous', time=time, source='governance', payload=None, links=(ProvenanceLink(relation, f'prov:{record_id}'),))
        if full_erasure and self.current_generation in self.retired_generations:
            ...
            generation = self._make_generation(parent=parent, roots=old.active_roots, anchors=self.anchors, provenance_head=self._latest_provenance_head(), target_cognition=old.target_cognition, horizon=old.horizon, tx_id=tx_id)
            self.generations[generation.generation_id] = generation
            self.current_generation = generation.generation_id
        return tombstone

    def canonical_snapshot(self) -> bytes:
        value = {'anchors': sorted(self.anchors), 'semantic_nodes': {node_id: payload.decode('utf-8') for node_id, payload in sorted(self.semantic_nodes.items())}, 'provenance': {record_id: {'origin': record.origin, 'time': record.time, 'source': record.source, 'payload_ref': record.payload_ref, 'links': [[link.relation, link.target] for link in record.links]} for record_id, record in sorted(self.provenance.items())}, 'payloads': dict(sorted(self.payloads.items())), 'generations': {generation_id: {'parent': generation.parent_generation_id, 'roots': generation.active_roots, 'anchor_root': generation.anchor_root, 'provenance_head': generation.provenance_head, 'target_cognition': generation.target_cognition, 'horizon': generation.horizon, 'created_by': generation.created_by_transaction} for generation_id, generation in sorted(self.generations.items())}, 'retired_generations': sorted(self.retired_generations), 'current_generation': self.current_generation}
    ...
    def from_snapshot(cls, snapshot: bytes) -> 'SemanticTransactionStore':
        ...
        expected_keys = {'anchors', 'semantic_nodes', 'provenance', 'payloads', 'generations', 'retired_generations', 'current_generation'}
        ...
        for node_id, serialized in raw['semantic_nodes'].items():
            ...
            try:
                ...
                raise InvalidTransactionError('invalid semantic snapshot payload') from exc
            canonical_bytes = _json_bytes(form)
            if _digest(form) != node_id or canonical_bytes.decode('utf-8') != serialized:
        ...
        for record_id, record_raw in raw['provenance'].items():
            ...
            try:
                links = tuple((ProvenanceLink(str(relation), str(target)) for relation, target in record_raw['links']))
                record = ProvenanceRecord(id=record_id, origin=record_raw['origin'], time=record_raw['time'], source=record_raw['source'], payload_ref=record_raw['payload_ref'], links=links)
            except (KeyError, TypeError, ValueError) as exc:
                raise InvalidTransactionError('invalid provenance snapshot entry') from exc
            if record.origin not in ('observed', 'endogenous'):
                raise InvalidTransactionError('invalid provenance origin')
            expected_id = store._provenance_record_id(origin=record.origin, time=record.time, source=record.source, payload_ref=record.payload_ref, links=record.links)
        ...
        for generation_id, generation_raw in raw['generations'].items():
            ...
            try:
                generation = Generation(generation_id=generation_id, parent_generation_id=generation_raw['parent'], active_roots=tuple(generation_raw['roots']), anchor_root=generation_raw['anchor_root'], provenance_head=generation_raw['provenance_head'], target_cognition=generation_raw['target_cognition'], horizon=generation_raw['horizon'], created_by_transaction=generation_raw['created_by'])
            except (KeyError, TypeError) as exc:
                raise InvalidTransactionError('invalid generation snapshot entry') from exc
            expected_id = _digest(['generation', generation.parent_generation_id, generation.active_roots, generation.anchor_root, generation.provenance_head, generation.target_cognition, generation.horizon, generation.created_by_transaction])
        ...
        for generation_id, generation in store.generations.items():
            ...
            if any((root not in store.semantic_nodes for root in generation.active_roots)):
                raise InvalidTransactionError('retained generation has missing semantics')
            if generation.provenance_head is not None and generation.provenance_head not in store.provenance:
                raise InvalidTransactionError('retained generation has missing provenance')
        return store
    ...
    def derived_symbol_index(self) -> dict[str, tuple[str, ...]]:
        ...
        return {symbol: tuple(sorted(node_ids)) for symbol, node_ids in sorted(index.items())}
