...
class VLLMTokenCapacityLaunchClass:
    ...
    def __post_init__(self) -> None:
        ...
        _require_hex_revision(
            self.target_artifact_revision,
            "target_artifact_revision",
            length=40,
        )
        _require_sha256_identity(self.target_digest, "target_digest")
    ...
    def from_mapping(cls, value: object) -> VLLMTokenCapacityLaunchClass:
        ...
        try:
            return cls(
                ...
                target_artifact_revision=_require_hex_revision(
                    mapping["target_artifact_revision"],
                    "target_artifact_revision",
                    length=40,
                ),
                target_digest=_require_sha256_identity(
                    mapping["target_digest"],
                    "target_digest",
                ),
...
class VLLMTokenCapacityReferenceEvidence:
    ...
    def from_mapping(cls, value: object) -> VLLMTokenCapacityReferenceEvidence:
        ...
        if format_version != _TOKEN_CAPACITY_REFERENCE_EVIDENCE_FORMAT_VERSION:
            raise VLLMTokenCapacityReferenceEvidenceError(
                "unsupported token-capacity-reference evidence format_version"
            )
