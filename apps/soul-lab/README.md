...
`apps/soul-lab` is the browser-based local interface for RelayLM character continuity. As of CW-A5, the visible shell is organized as a file-first Character Workspace with an explicit character creation/import entry point rather than an internal SOUL Lab administration surface.

The current bounded implementation covers UI-A0 through UI-A7, Phase I-2, Phase I-3, UI-B0/UI-B1A, I-4E Forget UI, I-5B Pin / Unpin UI, I-7C Held Governance UI, CW-A3, and CW-A5:
...
- persistent active-character display preference only when a real projected character exists,
- exact server-projected character records from `GET /lab/api/characters`,
- zero-character state routed to Create / Import without auto-creating or auto-restoring a default character,
- top-level Character Workspace routes: Home, Create, Character, Scenes, Relationships, Memory Wiki, Runtime, and Advanced,
- CW-A5 Create surface for Quick Create, Advanced Create, Showcase, and Import entry points,
...
CW-A3 does not create a new route, character, SOUL, memory, prompt, credential, backend, worker, source write, or runtime authority. CW-A5 adds a creation/import surface and loopback-backed domain APIs, but it still requires explicit user approval for workspace commit and does not set active character automatically. The browser sends only the server-projected route model and standard user/assistant history to the same-origin RelayLM endpoint. Existing RelayLM character resolution, M2 retrieval, RelayCTX injection, backend forwarding, and RelaySLP boundaries remain unchanged.
...
http://127.0.0.1:5173/lab/#/create
...
- **Create** shows Quick Create, Advanced Create, Showcase, and Import entry points and is the zero-character landing surface.
...
The Create surface is presentation/interaction only unless the user invokes explicit loopback-backed create APIs. Templates remain source candidates until committed. A committed workspace is not activated by the browser automatically; active character selection remains an explicit user action.
...
Server projections and bounded observation responses exclude API keys, URL credentials, absolute source paths/content, raw traces, prompt text, compiled context, and protected source. Conversation transcripts remain browser-process-local and are not written to `localStorage`; CW-A5 does not persist raw source or memory bodies to browser storage.
