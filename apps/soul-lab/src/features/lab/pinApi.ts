...
export function parsePinPreflight(value: unknown, kind: "pin" | "unpin", memoryId: string, revision: number): MemoryPinPreflight | null {
  ...
  if (!hasExactKeys(value, keys) || value.schema !== `relaylm.lab.memory_${kind}_preflight.v0` || value.operation_kind !== kind || !["ready", "already_pinned", "already_unpinned"].includes(String(value.status)) || value.read_only !== true || value.memory_id !== memoryId || value.current_revision !== revision || value.current_lifecycle_state !== "active" || value.current_mutation_state !== "none" || value.target_pin_state !== target || value.pin_state_contract_only !== false || !parseEffects(value.effects, kind)) return null;
  if (value.status === "ready") return isOpaqueToken(value.apply_token, 8192) && isSafeText(value.expires_at, 128) ? value as unknown as MemoryPinPreflight : null;
  return value.apply_token === null && value.expires_at === null ? value as unknown as MemoryPinPreflight : null;
}
...
export function parsePinApply(value: unknown, kind: "pin" | "unpin", memoryId: string, revision: number): MemoryPinApplyReceipt | null {
  ...
  const target = kind === "pin" ? "pinned" : "unpinned";
  if (!hasExactKeys(value, keys) || value.schema !== `relaylm.lab.memory_${kind}_apply.v0` || value.operation_kind !== kind || !["applied", "already_pinned", "already_unpinned"].includes(String(value.status)) || value.memory_id !== memoryId || value.current_revision !== revision || value.current_lifecycle_state !== "active" || value.current_mutation_state !== "none" || value.target_pin_state !== target || value.retrieval_eligible !== true || value.ordinary_retrieval_excluded !== false || value.priority_hint_enabled !== (target === "pinned") || value.semantic_content_changed !== false || value.physical_deletion !== false || value.audit_evidence_retained !== true || typeof value.idempotent_replay !== "boolean" || typeof value.effect_applied !== "boolean" || !isSafeId(value.receipt_id) || value.content_included !== false || value.path_included !== false || value.physical_id_included !== false || value.reason_included !== false || value.token_included !== false) return null;
