...
usage() {
  cat >&2 <<'EOF'
usage:
  startup-preflight.sh OUT_DIR SERVER_BIN MODEL_PATH PORT [SERVER_EXTRA_ARGS...]

Non-generative mechanical preflight for the RelayLM Gemma 4 cache/SWA matrix.
It never sends a completion/token-generation request.
...
Pass --swa-full in SERVER_EXTRA_ARGS only for the F arm.
EOF
}
...
mkdir -p "$out_dir"
: >"$out_dir/preflight-events.txt"

event() {
  printf '%s\t%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$*" | tee -a "$out_dir/preflight-events.txt" >&2
}
...

{
  ...
  printf 'model_path=%q\n' "$model_path"
  printf 'port=%q\n' "$port"
  printf 'extra_arg_count=%d\n' "$#"
  i=0
  for arg in "$@"; do
    printf 'extra_arg_%d=%q\n' "$i" "$arg"
    i=$((i + 1))
  done
} >"$out_dir/invocation-input.txt"

if [[ ! -x "$server_bin" ]]; then
  event "server binary is not executable"
  printf 'SERVER_BINARY_NOT_EXECUTABLE\n' >"$out_dir/classification.txt"
  exit 21
fi

if [[ ! -f "$model_path" ]]; then
  event "model path is not a regular file"
...
command -v nvidia-smi >/dev/null 2>&1 && nvidia-smi -q >"$out_dir/nvidia-smi-q.txt" 2>&1 || true
command -v nvidia-smi >/dev/null 2>&1 && nvidia-smi >"$out_dir/nvidia-smi.txt" 2>&1 || true
...
if (( version_rc != 0 )); then
  event "server --version failed rc=$version_rc"
...
if command -v ss >/dev/null 2>&1; then
  ...
  if grep -Eq "[.:]${port}[[:space:]]" "$out_dir/ports.before.txt"; then
    event "port $port already occupied"
...
while (( SECONDS < deadline )); do
  if ! kill -0 "$pid" 2>/dev/null; then
    ...
    dmesg 2>/dev/null | tail -n 200 >"$out_dir/dmesg.tail.txt" || true
    event "server exited before readiness rc=$rc"
    printf 'SERVER_EXITED_BEFORE_READINESS\n' >"$out_dir/classification.txt"
    exit 25
  fi

  set +e
  last_http=$(curl --silent --show-error --max-time 2 \
    --output "$out_dir/health-response.tmp" \
    --write-out '%{http_code}' \
    "http://127.0.0.1:${port}/health" \
    2>"$out_dir/health-curl.stderr.tmp")
  last_curl_rc=$?
  set -e

  printf '%s\n' "$last_http" >"$out_dir/health.last-http-code.txt"
  printf '%d\n' "$last_curl_rc" >"$out_dir/health.last-curl-exit.txt"

  if (( last_curl_rc == 0 )) && [[ "$last_http" == "200" ]]; then
...
command -v ss >/dev/null 2>&1 && ss -ltnp >"$out_dir/ports.ready.txt" 2>&1 || true
command -v nvidia-smi >/dev/null 2>&1 && nvidia-smi >"$out_dir/nvidia-smi.ready.txt" 2>&1 || true
if [[ -r "/proc/$pid/status" ]]; then
  cat "/proc/$pid/status" >"$out_dir/proc-status.ready.txt" 2>/dev/null || true
fi

if (( health_ok == 0 )); then
  event "startup readiness timeout after ${timeout_s}s"
...
# This preflight must remain non-generative. Stop after proving readiness.
...
grep -Ei \
  'flash|attention|cuda|n_batch|n_ubatch|ctx|context|parallel|slot|model|swa' \
  "$out_dir/server.stdout.txt" "$out_dir/server.stderr.txt" \
  >"$out_dir/startup-evidence.filtered.txt" 2>/dev/null || true

if ! grep -Eqi 'flash[^[:alnum:]]*att|flash_attn|flash attention' "$out_dir/startup-evidence.filtered.txt"; then
  event "health ready but explicit Flash Attention startup evidence not found"
