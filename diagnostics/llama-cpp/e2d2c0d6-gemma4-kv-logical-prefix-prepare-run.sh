...
usage() {
  cat >&2 <<'EOF'
usage:
  logical-prefix-prepare-run.sh OUT_ROOT LLAMA_SOURCE_REPO MODEL_PATH PORT_PLAIN PORT_PROBE [JOBS]
...
build_runner="$script_dir/e2d2c0d6-gemma4-kv-logical-prefix-build-run.sh"
qual_runner="$script_dir/e2d2c0d6-gemma4-kv-logical-prefix-qualification-run.sh"
...
if build is not None and build.get("primary_classification") != "LOGICAL_PREFIX_REPLACEMENT_BUILD_READY":
    errors.append("unexpected build terminal classification")
if qual is not None and qual.get("primary_classification") != "LOGICAL_PREFIX_REPLACEMENT_PREMEASURED_READY":
...
out = {
    "primary_classification": (
        ...
        if not errors
        else "LOGICAL_PREFIX_REPLACEMENT_PREPARATION_FAILED"
    ),
    "errors": errors,
