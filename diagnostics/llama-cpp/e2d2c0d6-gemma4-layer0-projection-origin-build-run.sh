...
usage() {
  cat >&2 <<'EOF'
usage:
  layer0-projection-origin-build-run.sh OUT_ROOT LLAMA_SOURCE_REPO [JOBS]
...
for required in "$aligned_patch" "$logical_patch" "$origin_patch" "$patch_selftest" "$origin_patch_selftest"; do
  if [[ ! -f "$required" ]]; then
...
out = {
    "primary_classification": "LAYER0_PROJECTION_ORIGIN_BUILD_READY",
