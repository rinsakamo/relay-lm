...
RUNNER = HERE / "e2d2c0d6-gemma4-layer0-projection-origin-measured-run.py"
RUNNER_SELFTEST = HERE / "e2d2c0d6-gemma4-layer0-projection-origin-measured-run-selftest.py"
POSTHOC_SELFTEST = HERE / "e2d2c0d6-gemma4-layer0-projection-origin-posthoc-selftest.py"
...
def reconcile(out_root: Path, child_rc: int):
    ...
    if w_record.is_file():
        return {
            "primary_classification": "LAYER0_PROJECTION_ORIGIN_PROBE_EXERCISED_INCOMPLETE",
    ...
    return {
        "primary_classification": "LAYER0_PROJECTION_ORIGIN_PROBE_NOT_EXERCISED",
        "measured_terminal_present": False,
...
def main():
    ...
    try:
        ...
        posthoc = subprocess.run(
            [sys.executable, str(POSTHOC_SELFTEST)],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        )
        (args.preflight_root / "posthoc-selftest.stdout").write_bytes(posthoc.stdout)
        (args.preflight_root / "posthoc-selftest.stderr").write_bytes(posthoc.stderr)
        if posthoc.returncode != 0 or b"LAYER0_PROJECTION_ORIGIN_POSTHOC_SELFTEST_PASS" not in posthoc.stdout:
            raise RuntimeError("posthoc selftest failed")

        preflight_cmd = [
            sys.executable, str(RUNNER),
            "--premeasured-root", str(args.premeasured_root),
        ...
        pf = subprocess.run(preflight_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ...
        if pf_obj.get("classification") != "LAYER0_PROJECTION_ORIGIN_MEASURED_PREFLIGHT_PASS":
