...
def main():
    ...
    if args.server_bin.is_file():
        data = args.server_bin.read_bytes()
        for marker in REQUIRED_BINARY_MARKERS:
            key = marker.decode("utf-8")
            present = marker in data
            marker_results[key] = present
            if not present:
                errors.append(f"required binary marker missing: {key}")
        old_marker_present = FORBIDDEN_OLD_BINARY_MARKER in data
        if old_marker_present:
            errors.append("old generated-prefix instrumentation marker still present")

    result = {
        "status": (
            "LOGICAL_PREFIX_BINARY_PREFLIGHT_PASS"
            if not errors
            else "LOGICAL_PREFIX_BINARY_PREFLIGHT_FAIL"
        ),
        ...
        "required_binary_markers": marker_results,
