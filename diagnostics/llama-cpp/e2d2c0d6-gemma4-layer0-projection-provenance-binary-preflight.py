...
def main():
    ...
    if not PROVENANCE_PATCH.is_file():
        errors.append("projection-provenance patch file missing")
    ...
    provenance_patch_sha = sha256(PROVENANCE_PATCH) if PROVENANCE_PATCH.is_file() else None
    ...
    result = {
        "status": (
            ...
            if not errors
            else "LAYER0_PROJECTION_PROVENANCE_BINARY_PREFLIGHT_FAIL"
        ),
        "server_binary": str(args.server_bin),
        "server_sha256": server_sha,
        "server_sha_matches_historical": server_sha in HISTORICAL_SERVER_SHA256 if server_sha else False,
        ...
        "projection_provenance_patch": str(PROVENANCE_PATCH),
        "projection_provenance_patch_sha256": provenance_patch_sha,
