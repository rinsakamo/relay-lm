...
def main():
    ...
    write_json(args.out_root / "physical-identity.json", {
        ...
        "fixture_files": fixture_evidence,
        "fixture_geometry": fixture_manifest.get("geometry"),
    ...
    warm_tokens = fixture_dir / "warm.tokens.json"
    target_tokens = fixture_dir / "target.tokens.json"
    l0_request = fixture_dir / "L0.request.json"
    l1_request = fixture_dir / "L1.request.json"
    lc_request = fixture_dir / "LC.request.json"

    admission_cmd = [
        sys.executable, str(admission),
        "--fixture-dir", str(fixture_dir),
        "--out", str(args.out_root / "fixture-admission.json"),
    ]
    ...
    try:
        ...
        l0 = send_request(wr, "L0", l0_request, 0, 883)
        dump_geometry["WR-P512"] = require_dump(kv_root, "WR-P512")

        submitted_requests.append("L1")
        l1 = send_request(wr, "L1", l1_request, 512, 2415)
        ...
        l0r = send_request(wr2, "L0R", l0_request, 0, 883)
        ...
        lc = send_request(cold, "LC", lc_request, 0, 2927)
