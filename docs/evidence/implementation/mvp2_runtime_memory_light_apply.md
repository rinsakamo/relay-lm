...
Current request-compilation application behavior, the full `CompiledRequest` field set, and current compile-decision diagnostics belong to [Runtime Compile Current/Target](../../contracts/runtime_compile_current_target.md) and [Context Compiler Contract](../../contracts/context_compiler_contract.md). Mode semantics and managed-route failure posture belong to [Request / Response Pipeline](../../architecture/runtime/request-response-pipeline.md) and [Runtime Reliability and Compatibility](../../architecture/runtime/reliability-and-compatibility.md). Repository-wide current implementation status remains owned by [Project Status](../../PROJECT_STATUS.md).
