...
The four-stage RelayCTX short-term diagnostics/injection chain's current schemas, config owners, full gate/blocked-reason taxonomy, insertion mechanics, apply-result schema, stage ordering, and exact CTX-Repack mutation-order rule belong to [RelayCTX Short-Term Runtime Contract](../../contracts/relayctx_short_term_runtime_contract.md) and are implemented in `relaylm/relayctx_repack.py`, `relaylm/diagnostics.py`, and `relaylm/managed_chat_runtime.py`. [Project Status](../../PROJECT_STATUS.md) records that this ordering was once wrong and has since been fixed, a historical fact this frozen source has no way to reflect.
