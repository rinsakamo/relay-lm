...
Current `CompileApplyDecision` fields, `decide_compile_apply()` semantics, and the full current/target compile-decision taxonomy belong to [Runtime Compile Current/Target](../../contracts/runtime_compile_current_target.md), [Runtime Compile Artifact Contract](../../contracts/runtime_compile_artifact_contract.md), and [Runtime Compile Gate Design](../../architecture/runtime_compile_gate_design.md), and are implemented in `relaylm/compile_gate.py`. Repository-wide current implementation status remains owned by [Project Status](../../PROJECT_STATUS.md).
