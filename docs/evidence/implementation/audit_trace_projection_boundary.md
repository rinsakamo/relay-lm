...
5. "Any remaining validation is defense in depth for already-projected fields: scalar types, finite non-negative numbers, bounded opaque identifiers, exact media type grammar, and URL/path rejection." — Confirmed true at the categorical level this source describes. The current validator set (`_bool`, `_non_negative_int`, `_non_negative_number`, `_http_status`, `_bounded_token`, `_lower_token`, `_class_token`, `_opaque_id`, `_sha256`, `_content_type`, `_scoped_uuid_id`, `_enum`, `_list_of`, `_mapping`, `_looks_like_url_or_path`, all in `relaylm/audit_projection.py`) is already covered by the same architecture contract's "Fail-closed behavior" section. An independent review of the initial cutover found that the wired smokes did not yet directly regression-pin every claim in this category — specifically finite/non-negative numeric rejection of bools/negatives/NaN/infinities, the complete opaque-identifier bound and URL/path-rejection categories, exact SHA-256 grammar, and exact content-type grammar including its optional-charset and unsupported-parameter cases. `scripts/relaylm_audit_projection_exact_contract_smoke.py` was extended in this same PR to assert all of these directly against `project_audit_metadata()`'s public projection boundary (not private validator functions), with exact dropped-field counters per probe; see "Current authority" below for what it now pins.

No block required absorption into a current authority. One accuracy correction, independent of this file's retirement, was made to [Audit Trace Content-Free Contract](../../architecture/audit_trace_content_free_contract.md): its "P0-A1 compatibility boundary" section used present/future-tense "During P0-A1..." and "P0-A2 removes ... entirely" phrasing for a phase pair that has no separate tracked existence anywhere else in the repository (`docs/PROJECT_STATUS.md` records no P0-A1/P0-A2 entries) and for behavior that current code has already fully achieved. The section was reworded to present tense to state the current, already-complete compatibility-shim behavior — current code proves only that the shim exists today, not that any current document commits to keeping it forever, so the wording does not assert permanence — and the contract's validation command list was extended to name the two contract smokes (`scripts/relaylm_audit_projection_contract_smoke.py`, `scripts/relaylm_audit_projection_exact_contract_smoke.py`) that this cutover wires into CI.
...
`scripts/relaylm_audit_projection_exact_contract_smoke.py` now directly regression-pins, through `project_audit_metadata()` probes with exact dropped-field-counter assertions: finite non-negative numeric acceptance/rejection (valid int/float, negative, bool, NaN, +infinity, -infinity); the complete bounded opaque-identifier boundary (empty, over-length, and every URL/path-shaped rejection category); exact SHA-256 grammar (short, long, non-hex, prefixed, URL/path-shaped); exact content-type grammar (malformed, unsupported-parameter, whitespace-invalid-charset, URL/path-shaped, overlong-component, and non-string rejection, plus the supported optional-charset acceptance form); the complete URL/path-rejection category set (URI schemes, protocol-relative, `www.`-prefixed, absolute, relative, home-relative, Windows-drive, and slash/backslash-containing values) through both `_bounded_token` and `_lower_token` fields; and exact nested-field projection (unknown nested fields dropped with an exact counter while known valid siblings are retained).
