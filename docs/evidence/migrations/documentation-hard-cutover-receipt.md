...
fail_closed_guards_added:
  - scripts/relaylm_docs_semantic_audit.py check_no_live_e1_local_runtime_architecture_path (existence check for the retired path plus a repository-wide active-reference scan for the full repository-root-qualified old path literal docs/architecture/e1_local_runtime_evaluation_2026_06_25.md only, deliberately not a bare-filename or stable-stem pattern the way check_no_live_lat1_scaffold uses, since the canonical target retains the identical basename and a stem-only pattern would false-positive on every legitimate live reference to the new docs/evidence/evaluations/ location; mirrors check_no_live_lat1_scaffold's exact-allowlist structure otherwise: a whole-file allowlist for this receipt and the guard's own implementation, an exact reviewed frozen-snapshot allowlist for the two -source.txt files that legitimately retain the old relative-form literal, and exact reviewed line allowances for the one legitimate mention each in docs/planning/documentation-architecture-inventory.md and docs/planning/documentation-cutover-rules.yaml; no generic frozen/historical_after_merge/historical whole-document status bypass)
self_test_assertions_added: 6
self_test_assertions_added_note: six_new_deterministic_self_test_assertions_bringing_relaylm_docs_semantic_audit_py_dash_dash_self_test_to_62_total_assertions_up_from_56_after_the_c1c39_lat1_hardening_covering_the_real_repositorys_current_silence_a_reintroduced_retired_file_rejection_a_current_documents_full_old_path_link_rejection_a_frozen_status_documents_unallowlisted_mention_rejection_the_two_exact_reviewed_frozen_snapshot_allowances_and_the_canonical_migrated_documents_own_new_path_not_self_triggering_the_guard
local_validation:
  compileall: passed
  docs_link_check: passed
  docs_semantic_audit: passed
  docs_semantic_audit_self_test: passed_62_assertions
  documentation_current_boundary_smoke: passed
  cutover_prepare_self_test: passed
  mvp_completion_report_smoke_check_model_check_all: passed
  mvp_completion_report_smoke_self_test: passed
...
final_pr_net_diff: {insertions: 383, deletions: 30}
reviews: 0
pr_comments: 0
unresolved_review_threads: 0
receipt_finalization: performed_after_validated_content_head
...
A new fail-closed guard, `check_no_live_e1_local_runtime_architecture_path()` in `scripts/relaylm_docs_semantic_audit.py`, fails closed if the retired path is reintroduced under `docs/architecture/`, or if any non-allowlisted file anywhere in the same repository-wide scan scope used by `check_no_live_mvp_tree()`/`check_no_live_lat1_scaffold()` contains an active reference to the exact old repository-root-qualified path. This guard deliberately does not reuse the bare-filename/stable-stem detection pattern from `check_no_live_lat1_scaffold()`: unlike the LAT-1 scaffold, whose canonical replacement used a different (hyphenated) filename, this record's canonical target keeps the identical basename — only the directory changed — so a bare-filename or underscore-stem pattern would false-positive on every legitimate live reference to the new `docs/evidence/evaluations/` location (confirmed empirically: the first implementation of this guard used stem-only matching and immediately and correctly failed against `docs/README.md`'s own newly-corrected, entirely legitimate link). The corrected guard matches only the full `docs/architecture/e1_local_runtime_evaluation_2026_06_25.md` literal, which no longer appears in any live file after this batch's edits. A whole-file allowlist covers this receipt and the guard's own implementation; an exact reviewed frozen-snapshot allowlist covers the two `-source.txt` files (which use the relative form `../../architecture/e1_local_runtime_evaluation_2026_06_25.md` and so do not even match the directory-qualified pattern, but are allowlisted defensively in case a future frozen snapshot legitimately uses the full form); and exact reviewed line allowances cover the one legitimate mention each in `docs/planning/documentation-architecture-inventory.md` and `docs/planning/documentation-cutover-rules.yaml`. Six new deterministic `--self-test` assertions were added, bringing `relaylm_docs_semantic_audit.py --self-test` to 62 total assertions (up from 56 after the Cutover 1C-39 LAT-1 hardening): the real repository's current silence, a reintroduced-file rejection, a full-old-path-link rejection in a current document, an unallowlisted mention rejection in a `historical_after_merge`-status document (confirming no generic status bypass), the two exact reviewed frozen-snapshot allowances, and confirmation that the canonical migrated document's own new-path filename does not self-trigger the guard.
