...
verification:
  ...
  consolidated_selector_contract_new_assertions:
    - no_group_in_any_workflow_retains_a_docs_mvp_wave4_slash_star_pattern
...
`cutover_pr` is now `600` (`rinsakamo/relay-lm#600`), corrected in this same entry from its initial `pending` placeholder once the pull request was created; no reconciliation was needed beyond filling in the real number. `23472fcaba751d7146f5c0f119aa17dae57bdd32` is the `validated_content_head`: all 44 triggered GitHub Actions check runs completed successfully, including every RelayMEM/runtime/UI consolidated-smoke group. Those groups ran in full rather than reporting `skipped` because `scripts/relaylm_ci_consolidated_smoke.py` is itself a `GLOBAL_PATTERNS` entry for all three groups, so editing it to remove the obsolete `docs/mvp/wave4/*` selector correctly forced a full run this one time — the underlying selection behavior for ordinary content changes (asserted by the new consolidated-smoke-contract checks) is unaffected. Zero reviews, zero PR comments, and zero unresolved review threads were present at that head. Per the `validated_content_head` / `receipt_finalization` pattern established in prior batches, this finalization is recorded in a further, separate commit after `23472fcaba751d7146f5c0f119aa17dae57bdd32`, which remains the exact validated content head; the finalization commit itself is not claimed as a re-validated head. C1C35 remains finalized to merge commit `5d60433713574c042afe5ceab15b865a48824ae5`. This PR is not merged.
