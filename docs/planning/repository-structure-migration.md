...
The R6 classification baseline completed in PR #1159 with exact resulting main `a58bd6178c8aaa8239c99680a71f5dac982fa716`. PR #1165 corrected its E1 recall evaluation-surface classification with exact resulting main `4e20c161e4181d9b12099e0e17d310d2e11f9702`. PR #1167 then atomically removed the sole pre-retirement relevance smoke, `scripts/relaylm_e1r5_primary_mem_recall_bridge_relevance_bounds_smoke.py`, together with its current registry, consolidation, and current-document references, producing exact resulting main `8da6bb98c8019736704a21a12262e6c8d59f5fa9`; historical completion and wave evidence remains retained. PR #1169 converged the four duplicate E1-named structural proofs into `scripts/relaylm_primary_recall_post_retirement_structure_smoke.py` at exact resulting main `6f8d5d0d18c4460622e0a5e2e2e70e1488e611db`, updating every current caller and changing its classification to `active` / `retained_current_component` with `removal_gate: null`. Current authority covers 61 surviving Primary modules, 2 current Primary recall proof smokes, and 10 reviewed R6 rows, with no `retired_after_cutover` row; historical implementation and wave evidence remains retained. The ordinary Primary serving reader, selection, and fallback remain retired; post-retirement structural proof, content-free audit projection, RT-1D rollback evidence, characterization, Correction / Forget / Pin, index/log recovery, page/mutation coordination, and retained-current projections remain protected.

The post-retirement structural-proof removal gate is closed. The only remaining transitional rows are `r6.primary.characterization_suite` and `r6.primary.reader_seam_rollback`; neither removal gate is closed by PR #1169. Fresh R6 P1 must audit replacement coverage and accepted rollback authority before selecting another atomic responsibility. No other Primary runtime asset deletion or move is authorized by this convergence. Retained-current and operator/recovery assets are not deletion candidates merely because their namespace still contains `primary`; any other asset may be deleted or moved only after its classification and removal or migration gate legitimately converge through a separate atomic review.
