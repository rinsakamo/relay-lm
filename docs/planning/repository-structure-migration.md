...
The R6 classification baseline completed in PR #1159 with exact resulting main `a58bd6178c8aaa8239c99680a71f5dac982fa716`. PR #1165 corrected its E1 recall evaluation-surface classification with exact resulting main `4e20c161e4181d9b12099e0e17d310d2e11f9702`. The authority now covers 61 surviving Primary modules and every existing E1-R5 Primary recall smoke in 11 reviewed R6 rows. The ordinary Primary serving reader remains retired; characterization, rollback, Correction, Forget, Pin, index/log recovery, page/mutation coordination, retained-current projection, and post-retirement structural evidence remain protected.

The failing pre-retirement relevance smoke is the sole `retired_after_cutover` row. Its exact atomic gate—remove the smoke together with every stale registry, consolidation, and current-document claim while retaining structural and audit-projection proofs—is the next uniquely selected R6 responsibility. Retained-current and operator/recovery assets are not deletion candidates merely because their namespace still contains `primary`; any other asset may be deleted or moved only after its classification and removal or migration gate legitimately converge through a separate atomic review.
