...
Lane R  R6 Primary MEM classification baseline             COMPLETE; LR-D35-1 classification detachment COMPLETE (PR #1238); D6-R35-R complete (PR #1240); D6-R35-S complete (PR #1244); D6-R35-T complete (PR #1249); D6-R35-U complete (PR #1251); D6-R35-V complete (PR #1253); D6-R35-W next eligible
...
The remaining D6 criteria are not met. The D6-R34 completion audit against exact current `main` found every criterion unmet: remaining non-active and transitional sources are still live, the retirement manifest cannot yet be treated as complete while those live sources remain, a hand-maintained long-list repository documentation router remains, bespoke one-document documentation guards and receipt-style validation surfaces remain, and documentation validation has therefore not converged to generic checks only. The ordered remainder is:
...
D6-R19 root reference relocation
  ...
  -> D6-R35 residual non-active source-family retirement
  -> D6-R36 root documentation router rebuild
  -> D6-R37 bespoke documentation guard / receipt / ledger retirement
...
- **D6-R34** is complete as a failed completion audit (PR #1143). Re-running the five D6 completion criteria against exact current `main` found every criterion unmet: remaining non-active and transitional sources are still live; the retirement manifest cannot be treated as complete while those live sources remain; a hand-maintained long-list repository documentation router remains; bespoke one-document documentation guards, receipts, and active-ledger bookkeeping remain; and documentation validation has therefore not converged to generic checks only. D6 remains incomplete and PD-1 remains closed. The finite D6-R35 through D6-R39 remediation sequence below is authorized; D6-R35 is next.
- **D6-R35** disposes the remaining non-active and transitional source families from the current tree. Each exact source family is handled in a bounded exact-main PR or explicitly reviewed sub-slice, durable semantics are absorbed into existing canonical owners, qualifying historical evidence may remain only as frozen evidence, all other history is Git-only, and every retired path updates the central retirement manifest atomically. No new permanent transitional doc type or archive tree is created. The first bounded source family is complete (PR #1151): the two SOUL Lab UI-A6 shared shell / Settings and UI-A7 management projection implementation handoffs are retired to Git history, stable browser/server SOUL Lab semantics remain owned by `docs/architecture/ui/soul-lab.md`, and exact current management-read semantics remain owned by `docs/contracts/ui/soul-lab-management.md`. Both old architecture paths and their exact recovery identities are recorded in `records/documentation/retirement-manifest.json`, so this bullet names only the current authorities. No redirect, stub, or archive copy remains. The second bounded source family is complete (PR #1157): the SOUL Lab UI-A0/A1 foundation / mock-Home and UI-A2 first-launch / adoption implementation handoffs are retired to Git history, stable browser/server SOUL Lab semantics remain owned by `docs/architecture/ui/soul-lab.md`, current SOUL Lab implementation completion remains recorded by `docs/architecture/soul_lab_ui_mvp.md`, and durable creation/import semantics remain owned by `docs/architecture/character-workspace/creation-and-import.md`; their old paths and exact recovery identities are likewise recorded in `records/documentation/retirement-manifest.json`, and no redirect, stub, archive copy, or duplicate frozen-evidence copy remains. The third bounded source family is complete (PR #1162): the SOUL Lab UI-A3 Communication, UI-A4 Pod / SOUL Intervention, and UI-A5 Memory Inspector preview implementation handoffs are retired to Git history, stable browser/server SOUL Lab semantics remain owned by `docs/architecture/ui/soul-lab.md`, current SOUL Lab implementation completion and pending integration status remain recorded by `docs/architecture/soul_lab_ui_mvp.md`, and the durable memory-governance browser boundary inherited from UI-A5 remains owned by `docs/architecture/memory/mutation-governance.md`; their old paths and exact recovery identities are likewise recorded in `records/documentation/retirement-manifest.json`, no redirect, alias, stub, archive copy, or duplicate frozen-evidence copy remains, no new implementation handoff was created in the closed `docs/evidence/implementation/` collection, and the exact rollout and mock preview details remain Git-history-only rather than being promoted into current schema or contract authority. The fourth bounded source family is complete (PR #1171): the SOUL Lab UI-B0 real Home conversation and UI-B1A lifecycle visibility implementation handoffs are retired to Git history, exact current Home conversation semantics remain owned by `docs/contracts/ui/home-conversation.md`, exact lifecycle visibility semantics remain owned by `docs/contracts/ui/lifecycle-visibility.md`, stable browser/server SOUL Lab semantics remain owned by `docs/architecture/ui/soul-lab.md`, and current SOUL Lab implementation completion remains recorded by `docs/architecture/soul_lab_ui_mvp.md`; qualifying dated/frozen evidence remains historical evidence, their old paths and exact recovery identities are likewise recorded in `records/documentation/retirement-manifest.json`, and no redirect, alias, stub, archive copy, runtime or UI production change, workflow change, or UI-facing I-series retirement was introduced. The fifth bounded source family is complete (PR #1175): the I-4E Forget API/UI, I-4F Forget validation, and I-7C Held Apply/Discard runtime implementation handoffs are retired to Git history, exact current Forget lifecycle/API/product semantics remain owned by `docs/architecture/phase_i4_primary_mem_forget_hide_contract.md`, I-4F remains validation-only historical evidence and current focused validation without gaining a new semantic owner, exact current Held runtime/API/UI/public-projection governance remains owned by `docs/contracts/memory/held-governance.md`, stable browser/server SOUL Lab semantics remain owned by `docs/architecture/ui/soul-lab.md`, and stable cross-operation mutation semantics remain owned by `docs/architecture/memory/mutation-governance.md`; qualifying completion reports and exact source snapshots remain historical evidence, their old paths and exact recovery identities are likewise recorded in `records/documentation/retirement-manifest.json`, and no redirect, alias, stub, archive copy, duplicate frozen-evidence copy, runtime or UI production change, workflow change, or Lane R classification change was introduced. The sixth bounded source family is complete (PR #1181): the I-5B Pin/Unpin apply/API/UI implementation handoff is retired to Git history after Lane R PR #1179 removed the former direct `r6.primary.pin_operations` classification evidence dependency and mandatory Lane R P8 #1180 synchronized that convergence, exact current SOUL Lab Primary Pin/Unpin management semantics remain owned by `docs/contracts/ui/memory-pin-unpin-management.md`, stable pinned-memory meaning and the eligibility-before-ranking rule remain owned by `docs/architecture/memory/pinned-memory.md`, and stable cross-operation mutation and ordinary retrieval/ranking responsibilities remain with `docs/architecture/memory/mutation-governance.md` and `docs/architecture/memory/retrieval-and-grounding.md` where applicable; the I-5A Pin/Unpin contract and read-only preflight handoff `docs/architecture/phase_i5_pin_unpin_contract.md` remains live and transitional and is not retired here, qualifying completion reports and exact source snapshots remain historical evidence, the old path and its exact recovery identity are likewise recorded in `records/documentation/retirement-manifest.json`, and no redirect, alias, stub, archive copy, duplicate frozen-evidence copy, runtime, API/schema/route, UI production, workflow, test, or Lane R classification change was introduced. The seventh bounded source family is complete (PR #1184): the phase-named I-7A/B Held Apply/Discard contract and read-only preflight implementation handoff is retired to Git history, exact current Held candidate, source-evidence, governability-preflight, related-Primary fail-closed reread, preflight-token, durable-decision, public-projection, and queue non-authority semantics remain owned by `docs/contracts/memory/held-governance.md`, stable browser/server SOUL Lab semantics remain owned by `docs/architecture/ui/soul-lab.md`, and stable cross-operation mutation/governance semantics remain owned by `docs/architecture/memory/mutation-governance.md`; section-by-section comparison found the permanent Held governance contract already a strict superset of the retiring handoff, so no durable semantic absorption was required and no new owner, redirect, alias, stub, archive copy, or duplicate frozen-evidence copy was created, qualifying completion reports and exact source snapshots remain historical evidence, the old path and its exact recovery identity are likewise recorded in `records/documentation/retirement-manifest.json`, and no runtime, API/schema/route, UI production, workflow, test, or Lane R classification change was introduced. The eighth bounded source family is complete (PR #1188): the ACG-2 Grounded Recall detail-safety implementation handoff is retired to Git history, exact current detail-safety schema, detail vocabulary, restrictive-only fail-closed merge, and public projection semantics remain owned by `docs/contracts/query-detail-analyzer.md`, shared analyzer candidate governance remains owned by `docs/architecture/analyzers/candidate-governance.md`, and stable retrieval/grounding architecture remains owned by `docs/architecture/memory/retrieval-and-grounding.md`; the old path and its exact recovery identity are likewise recorded in `records/documentation/retirement-manifest.json`, and no new permanent owner, redirect, alias, stub, archive copy, duplicate frozen-evidence copy, runtime, API/schema/route, workflow, test, or Lane R classification change was introduced. The ninth bounded source family is complete (PR #1195): the remaining phase-named Analyzer Candidate Governance roadmap and the ACG-1, ACG-3, ACG-4, ACG-5, and ACG-6 sources are retired to Git history after the roadmap itself recorded ACG-0 through ACG-6 complete and therefore carried no unfinished sequencing authority; exact current shared analyzer candidate schema, source-class, validation, and content-free projection semantics remain owned by `docs/contracts/analyzer-candidate.md`, exact retrieval-query normalization semantics by `docs/contracts/retrieval-query-analyzer.md`, exact reference/intent consolidation semantics by `docs/contracts/reference-intent-analyzer.md`, exact RelayEMO scene-hint non-authority semantics by `docs/contracts/relayemo-scene-hint.md`, and exact scene classifier / scene-wiki matching semantics by `docs/contracts/scene-classifier.md`; stable cross-analyzer governance remains owned by `docs/architecture/analyzers/candidate-governance.md`, with stable retrieval, reference/intent, affect, and scene architecture remaining owned by `docs/architecture/memory/retrieval-and-grounding.md`, `docs/architecture/analyzers/reference-and-intent.md`, `docs/architecture/emotion/affect-modulation.md`, and `docs/architecture/scene/scene-model.md` with `docs/architecture/memory/scene-memory-scope.md` where applicable, and repository sequencing remaining with `docs/architecture/project_execution_plan.md`; all six old paths and their exact recovery identities are likewise recorded in `records/documentation/retirement-manifest.json`, and no new permanent owner, redirect, alias, stub, archive copy, duplicate frozen-evidence copy, runtime, API/schema/route, workflow, test, or Lane R classification change was introduced. The tenth bounded source family is complete (PR #1198): the phase-named CW-A1 file-first source tree/parser and CW-A2 workspace compiler projection implementation handoffs are retired to Git history as the inventory's single source tree/parser/compiler family; exact current workspace layout, path classification, and durable source-set semantics remain owned by `docs/contracts/character-workspace/source-tree.md`, exact read-only Markdown parsing, validation, block identity, and read-only manifest semantics by `docs/contracts/character-workspace/parser-and-validation.md`, and exact deterministic compile, accepted build artifact set, dry-run-first explicit write, `.relaylm/build/**` write boundary, context tier projection, and content-free compiler projection semantics by `docs/contracts/character-workspace/compiled-projections.md`; stable source/parser/compiler architecture remains owned by `docs/architecture/character-workspace/source-compiler.md` and stable parent workspace architecture by `docs/architecture/character-workspace/system.md`; section-by-section comparison found the permanent architecture and exact contracts already a strict superset of both retiring handoffs, so no durable semantic absorption was required and no new owner, redirect, alias, stub, archive copy, or duplicate frozen-evidence copy was created; CW-A3, CW-A4, and CW-A5 remain live separate families whose only change is repointing their retired A1/A2 related-authority backlinks, and the stale CW-A3 inventory target discrepancy is deliberately left unresolved for a separate transaction; both old paths and their exact recovery identities are likewise recorded in `records/documentation/retirement-manifest.json`, and no runtime, API/schema/route, workflow, test, or Lane R classification change was introduced. The eleventh bounded source family is complete (PR #1201): the target Character Template and Creation Flow source and the transitional CW-A5 character creation, templates, and showcase-import implementation handoff are retired to Git history as the inventory's single creation/templates/import family; the stable creation, template, local-import, staging, approval, commit, build-generation, and activation-separation responsibility model remains owned by `docs/architecture/character-workspace/creation-and-import.md`, exact current bundled template registry, no-character startup projection, Quick/Advanced/Showcase staging, approval and target-conflict gates, build-before-publish generation, loopback and CLI write boundaries, validation-only external import, and content-free public projection semantics remain owned by `docs/contracts/character-workspace/creation-commit.md`, the accepted target ownership split between developer showcase characters, the public adoptable starter, user-authored or imported characters, and official RelayLM product knowledge remains owned by `docs/architecture/character-workspace/showcase-starter-product-knowledge.md`, and stable parent workspace architecture remains owned by `docs/architecture/character-workspace/system.md`; block-by-block classification of the retiring target source found every durable accepted target already permanently owned, so no new owner, redirect, alias, stub, archive copy, or duplicate frozen-evidence copy was created, the superseded framing of official RelayLM product-help knowledge as ordinary personal or character memory remains superseded by the concept policy and was not restored, and remote registry, custom GitHub template URL, network distribution, template update channels, and LLM-drafted source generation remain separately governed or unadopted future work rather than current authority; CW-A3 and CW-A4 remain live separate families untouched by this transaction and the stale CW-A3 inventory target discrepancy is still deliberately left unresolved for a separate transaction; both old paths and their exact recovery identities are likewise recorded in `records/documentation/retirement-manifest.json`, and no runtime, API/schema/route, workflow, test, or Lane R classification change was introduced. The twelfth bounded source family is complete (PR #1206): the CW-A3 Character Workspace UI rebuild implementation handoff is retired to Git history as the inventory's single UI presentation family; the stable Character Workspace/SOUL Lab UI topology, browser-versus-server authority boundary, Real Runtime versus Local Preview separation, character-scope and stale-request fencing, content-free default projection, browser storage minimization, and loopback security boundary remain owned by `docs/architecture/ui/soul-lab.md`, with exact continuing UI semantics owned by `docs/contracts/ui/home-conversation.md` for Home real-conversation transport, `docs/contracts/ui/soul-lab-management.md` for loopback read-only management projections, and `docs/contracts/ui/lifecycle-visibility.md` for read-only lifecycle and operation visibility, while operation-specific Correct, Forget/Hide, Pin/Unpin, and Held Apply/Discard mutations remain governed by their own exact contracts; section-by-section comparison found the permanent UI architecture and exact UI contracts already a superset of the retiring handoff, so no durable semantic absorption was required and no new owner, redirect, alias, stub, archive copy, or duplicate frozen-evidence copy was created; the documentation inventory's stale `architecture/ui/character-workspace.md` target never existed in the tree and is corrected to the existing `architecture/ui/soul-lab.md` owner rather than created as a duplicate UI architecture page, and the handoff's exact hash-route table was already stale against the current app and remains implementation detail in `apps/soul-lab/**` rather than permanent architecture; CW-A4 remains a live separate family whose only change is repointing its retired A3 related-authority backlink, and its exact candidate/proposal contract coverage gap remains for a separate bounded transaction; the old path and its exact recovery identity are likewise recorded in `records/documentation/retirement-manifest.json`, and no runtime, API/schema/route, workflow, test, or Lane R classification change was introduced. The thirteenth bounded source family is complete (PR #1209): the four Phase 5.5 stream/TTS bounded-slice and C1/C2/C3 handoff sources are retired to Git history; stable cross-stage ownership remains with `docs/architecture/voice/streaming-and-tts.md`, exact A/B/B2 behavior with `docs/contracts/runtime/stream-suppression.md`, exact C0 and current C2 admission with `docs/contracts/runtime/tts-segmentation.md`, and exact C1/C3 plus current C4 wiring with `docs/contracts/runtime/tts-transport.md`; qualifying Phase 5.5 implementation records remain historical evidence, runtime semantics are unchanged, and no redirect, alias, stub, archive copy, runtime/config/API/route/stream/TTS/workflow/test, Lane R, or Lane C change was introduced. D6-R35 remains in progress and D6-R36 does not start in this transaction.
- **D6-R35-N** is complete in implementation PR #1215 with its mandatory P8 authority synchronization. The exact current Character Workspace maintenance run, candidate, proposal, identifier, status, field, validation, write-candidate, approval, and public-projection semantics are now owned by docs/contracts/character-workspace/maintenance-candidates.md; stable responsibility remains in docs/architecture/character-workspace/maintenance-candidates.md. **D6-R35-O** is complete in implementation PR #1218: the CW-A4 implementation handoff is retired to Git history with its exact recovery identity in records/documentation/retirement-manifest.json. **D6-R35-P** is complete in implementation PR #1221: the Phase 6 A/B admission, response-handoff, dispatch-preflight, durable-enqueue, and queue-state transitional sources are retired with exact recovery identities in the retirement manifest, while the permanent SLP contracts remain current. **D6-R35-Q** is complete in implementation PR #1223: the Phase 6-C worker/source-capture/finalization sources are retired with exact recovery identities in the retirement manifest, while the permanent primary-worker, governed-source-capture, durable-finalization, and memory-formation owners remain current; Primary writer, M3D/M3E, index/log recovery, and I1G transaction ownership remain outside Q. **D6-R35-R** is complete in implementation PR #1240: the former Primary writer/index-log recovery and I1-G/I1-GD architecture handoffs are retired with exact recovery identities in the retirement manifest, while permanent storage/recovery and SLP durable-finalization owners remain current. **D6-R35-S** is complete in implementation PR #1244: the former ASM-1, SM-1, and ST-1 runtime handoffs are retired with exact recovery identities in the retirement manifest, while permanent formation, memory-subsystem, storage/recovery, lifecycle-publication, and Subjective MEM contract owners remain current. No runtime, API, schema implementation, test, workflow, Lane R/C, or evidence change is introduced. D6-R35-T Grounded Recall exact-contract promotion is complete in implementation PR #1249; `docs/contracts/grounded-recall.md` is the exact owner while the E1-R4 handoff remains transitional evidence. **D6-R35-U** E1 retrieval/grounding/evaluation-source retirement is complete in implementation PR #1251: the E1-R4 and E1-R5 handoffs are retired with exact recovery identities in the retirement manifest while retrieval/grounding authorities and current evidence remain current. **D6-R35-V** legacy stable Character / Relationship / Scene source-family retirement is complete in implementation PR #1253: the six pre-canonical stable design sources are retired with exact recovery identities in the retirement manifest while permanent Character Workspace, relationship, belief/observation, scene, affect, context/pipeline, and privacy owners remain current. D6-R35 remains in progress, D6-R35-W is next eligible, D6 remains incomplete, and D6-R36 has not started.
- **Lane R LR-D35-1** is complete in implementation PR #1238. The remaining E1, RT-1, and O3 classification dependencies are detached in the Lane R authority pair, the current evidence and protected responsibilities remain, and the project-execution related-authority edge now points to `docs/planning/project-execution.md`. Lane D D6-R35-R then retired the Primary writer/index-log recovery source family in PR #1240 while preserving live runtime/code/test evidence and the Lane R authority. D6-R35-S then retired the Shared Assessment/Subjective MEM runtime source family in PR #1244 while preserving live runtime/code/test evidence and permanent memory authorities. D6-R35-T then promoted the exact Grounded Recall contract in PR #1249 while retaining the E1-R4 implementation handoff as transitional evidence. D6-R35-U then retired the E1-R4 and E1-R5 retrieval/grounding handoffs in PR #1251 while preserving current retrieval/grounding authorities and evidence. D6-R35 remains in progress, D6-R35-V is next eligible, D6 remains incomplete, and D6-R36 has not started.
- **D6-R36** replaces the remaining hand-maintained long-list repository documentation router with bounded navigation that does not duplicate milestone or status authority. It reuses existing canonical graph and generator ownership where possible and creates no second architecture or repository-status authority.
- **D6-R37** retires documentation-specific one-document guards, validation receipts, and active-ledger bookkeeping that only protect retired or historical document boundaries. Functional, runtime, and domain tests that validate product behavior are preserved, and remaining documentation validation converges to generic metadata, link, authority, normative-extraction, Git-recoverability, retained-record, retirement-manifest, and generated-index checks.
- **D6-R38** closes the central retirement-manifest and current-tree classification boundary: every retired live path is represented with recovery identity, no retired path remains live or actively referenced, the transitional-assets registry stays empty unless a separately reviewed authority change says otherwise, and only allowlisted retained records and frozen evidence survive outside the permanent active graph.
- **D6-R39** re-runs all five D6 completion criteria against exact current `main` and may declare D6 complete only when every criterion passes. PD-1 remains gated by both that declaration and Lane R R5 completion.
...
The R6 classification baseline completed in PR #1159, PR #1165 corrected the E1 recall evaluation surface, PR #1167 removed the sole pre-retirement relevance smoke, and PR #1169 converged four duplicate structural proofs into `scripts/relaylm_primary_recall_post_retirement_structure_smoke.py`; historical implementation and wave evidence remains retained. The reviewed PR #1172 reader-seam classification correction is integrated in exact main `eac68b895333d5e107e9ec3ce6bc02a29c64afdf`. PR #1176 removed the two legacy Primary recall compatibility no-op installers through their reviewed atomic gate and updated the package-import purity smoke without losing its metadata-only import, canonical identity, registry non-mutation, or explicit audit-projection installer checks. PR #1179 removed the former Lane R `r6.primary.pin_operations` evidence dependency on the I-5B implementation handoff without retiring or changing active protected Primary Pin runtime authority; Lane D subsequently retired that handoff in PR #1181 and synchronized the retirement in mandatory P8 PR #1183. PR #1182 split shared characterization support from the Primary-only transitional characterization row without changing runtime or test semantics. PR #1203 classified `r6.primary.recall_audit_projection` as the one reviewed transitional `retired_after_cutover` row and gave it an explicit reviewed atomic removal gate; that gate remains open and unexecuted. Its former content-free `relaymem.primary_recall_projection.v0` diagnostic boundary is transitional retirement and compatibility evidence after ordinary Primary producer retirement, not a current ordinary Primary recall producer, and no Primary reader, discovery, selection, recall, fallback, serving, or evidence-release authority was revived. Current authority covers 59 surviving Primary modules, 2 current Primary recall proof smokes, and 12 reviewed R6 rows; `r6.primary.recall_legacy_noop_installers` is removed and current reviewed `retired_after_cutover` rows are exactly 1. The ordinary Primary serving reader, selection, and fallback remain retired. Active protected responsibilities are the post-retirement structural proof, read-only Primary admin/store access, reader-retirement regression, current read projections, Correction / Forget / Pin, index/log recovery, and page/mutation coordination.

`r6.primary.characterization_suite` still contains exactly the four Primary formation, lifecycle, store-I/O, and review-regression characterization tests and remains transitional and protected because its replacement-coverage gate is open. `tests/_relaymem_characterization_support.py` remains separately active and protected as retained-current `r6.primary.characterization_support`, including for its maintained `tests/test_relaymem_slp_queue_characterization.py` caller; neither the support nor any characterization test is authorized for retirement. Retained-current and operator/recovery assets remain protected. No further R6 deletion or move is authorized automatically; actual audit-projection removal requires its explicit reviewed atomic gate plus fresh exact-head P1 evidence, and the next bounded responsibility requires a fresh P1 dependency and removal-gate audit rather than selection by naming alone. Any other asset may be deleted or moved only after its classification and removal or migration gate legitimately converge through a separate atomic review.

The bounded Lane R classification-evidence detachment for `r6.primary.index_log_recovery` completed in PR #1233. D6-R35-R subsequently retired the former M3D–M3H and I1-G/I1-GD architecture handoffs in PR #1240, and D6-R35-S retired the former ASM-1, SM-1, and ST-1 runtime handoffs in PR #1244; D6-R35-T promoted the exact Grounded Recall contract in PR #1249; D6-R35-U retired the E1-R4 and E1-R5 retrieval/grounding handoffs in PR #1251; D6-R35-V retired the six pre-canonical stable Character / Relationship / Scene sources in PR #1253; the active asset, classification authority, permanent memory owners, and runtime/code/test evidence remain protected, and D6-R35-W is next.
