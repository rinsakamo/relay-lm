...
- **D6-R35-N** is complete in implementation PR #1215 with its mandatory P8 authority synchronization. The exact current Character Workspace maintenance run, candidate, proposal, identifier, status, field, validation, write-candidate, approval, and public-projection semantics are now owned by docs/contracts/character-workspace/maintenance-candidates.md; stable responsibility remains in docs/architecture/character-workspace/maintenance-candidates.md. **D6-R35-O** is complete in implementation PR #1218: the CW-A4 implementation handoff is retired to Git history with its exact recovery identity in records/documentation/retirement-manifest.json, with no runtime, API, schema implementation, test, workflow, Lane R/C, or evidence change. D6-R35 remains in progress, D6-R35-P is next, D6 remains incomplete, and D6-R36 has not started.
