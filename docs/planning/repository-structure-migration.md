...
The R6 classification baseline completed in PR #1159 with exact resulting main `a58bd6178c8aaa8239c99680a71f5dac982fa716`. PR #1165 corrected its E1 recall evaluation-surface classification with exact resulting main `4e20c161e4181d9b12099e0e17d310d2e11f9702`. PR #1167 then atomically removed the sole pre-retirement relevance smoke, `scripts/relaylm_e1r5_primary_mem_recall_bridge_relevance_bounds_smoke.py`, together with its current registry, consolidation, and current-document references, producing exact resulting main `8da6bb98c8019736704a21a12262e6c8d59f5fa9`; historical completion and wave evidence remains retained. Current authority covers 61 surviving Primary modules, 5 existing `relaylm_e1r5_primary_mem_recall_*` smokes, and 10 reviewed R6 rows, with no `retired_after_cutover` row. The ordinary Primary serving reader, selection, and fallback remain retired; post-retirement structural proof, content-free audit projection, RT-1D rollback evidence, characterization, Correction / Forget / Pin, index/log recovery, page/mutation coordination, and retained-current projections remain protected.

A fresh post-PR #1167 semantic audit selects one next R6 responsibility: the `r6.primary.recall_post_retirement_structure` removal gate must atomically converge its four duplicate E1-named structural proofs into one function-oriented post-retirement structural proof, updating every current caller without a compatibility alias. This gate does not authorize any other Primary runtime asset deletion or move. Retained-current and operator/recovery assets are not deletion candidates merely because their namespace still contains `primary`; any other asset may be deleted or moved only after its classification and removal or migration gate legitimately converge through a separate atomic review.
