...
- **D6-R35-N** is complete in implementation PR #1215 with its mandatory P8 authority synchronization. The exact current Character Workspace maintenance run, candidate, proposal, identifier, status, field, validation, write-candidate, approval, and public-projection semantics are now owned by docs/contracts/character-workspace/maintenance-candidates.md; stable responsibility remains in docs/architecture/character-workspace/maintenance-candidates.md. **D6-R35-O** is complete in implementation PR #1218: the CW-A4 implementation handoff is retired to Git history with its exact recovery identity in records/documentation/retirement-manifest.json. **D6-R35-P** is complete in implementation PR #1221: the Phase 6 A/B admission, response-handoff, dispatch-preflight, durable-enqueue, and queue-state transitional sources are retired with exact recovery identities in the retirement manifest, while the permanent SLP contracts remain current; the Phase 6-C worker/source-capture/finalization family remains for Q. No runtime, API, schema implementation, test, workflow, Lane R/C, or evidence change is introduced. D6-R35 remains in progress, D6-R35-Q is next, D6 remains incomplete, and D6-R36 has not started.
