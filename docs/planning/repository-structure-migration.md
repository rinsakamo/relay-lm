...
The R6 classification baseline completed in PR #1159, PR #1165 corrected the E1 recall evaluation surface, PR #1167 removed the sole pre-retirement relevance smoke, and PR #1169 converged four duplicate structural proofs into `scripts/relaylm_primary_recall_post_retirement_structure_smoke.py`; historical implementation and wave evidence remains retained. The reviewed PR #1172 reader-seam classification correction is integrated in exact main `eac68b895333d5e107e9ec3ce6bc02a29c64afdf`. Current authority covers 61 surviving Primary modules, 2 current Primary recall proof smokes, and 12 reviewed R6 rows. The ordinary Primary serving reader, selection, and fallback remain retired. Active protected responsibilities are the post-retirement structural proof, content-free audit projection, read-only Primary admin/store access, reader-retirement regression, current read projections, Correction / Forget / Pin, index/log recovery, and page/mutation coordination.

`r6.primary.characterization_suite` remains transitional and protected because its replacement-coverage gate is open. The sole current `retired_after_cutover` row is `r6.primary.recall_legacy_noop_installers`; its two compatibility no-op modules are not rollback authority, and their atomic removal is the uniquely selected next Lane R responsibility. No other Primary runtime asset deletion or move is authorized by this convergence. Retained-current and operator/recovery assets are not deletion candidates merely because their namespace still contains `primary`; any other asset may be deleted or moved only after its classification and removal or migration gate legitimately converge through a separate atomic review.
