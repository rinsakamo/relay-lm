...
The R6 classification baseline completed in PR #1159, PR #1165 corrected the E1 recall evaluation surface, PR #1167 removed the sole pre-retirement relevance smoke, and PR #1169 converged four duplicate structural proofs into `scripts/relaylm_primary_recall_post_retirement_structure_smoke.py`; historical implementation and wave evidence remains retained. The reviewed PR #1172 reader-seam classification correction is integrated in exact main `eac68b895333d5e107e9ec3ce6bc02a29c64afdf`. PR #1176 removed the two legacy Primary recall compatibility no-op installers through their reviewed atomic gate and updated the package-import purity smoke without losing its metadata-only import, canonical identity, registry non-mutation, or explicit audit-projection installer checks. PR #1179 removed the former Lane R `r6.primary.pin_operations` evidence dependency on the I-5B implementation handoff without retiring or changing active protected Primary Pin runtime authority; Lane D subsequently retired that handoff in PR #1181 and synchronized the retirement in mandatory P8 PR #1183. PR #1182 split shared characterization support from the Primary-only transitional characterization row without changing runtime or test semantics. Current authority covers 59 surviving Primary modules, 2 current Primary recall proof smokes, and 12 reviewed R6 rows; `r6.primary.recall_legacy_noop_installers` is removed and current `retired_after_cutover` rows are 0. The ordinary Primary serving reader, selection, and fallback remain retired. Active protected responsibilities are the post-retirement structural proof, content-free audit projection, read-only Primary admin/store access, reader-retirement regression, current read projections, Correction / Forget / Pin, index/log recovery, and page/mutation coordination.

`r6.primary.characterization_suite` now contains exactly the four Primary formation, lifecycle, store-I/O, and review-regression characterization tests and remains transitional and protected because its replacement-coverage gate is open. `tests/_relaymem_characterization_support.py` is separately active and protected as retained-current `r6.primary.characterization_support`, including for its maintained `tests/test_relaymem_slp_queue_characterization.py` caller; neither the support nor any characterization test is authorized for retirement by this classification-only split. Retained-current and operator/recovery assets remain protected. No further R6 deletion or move is authorized automatically; the next bounded responsibility requires a fresh P1 dependency and removal-gate audit rather than selection by naming alone. Any other asset may be deleted or moved only after its classification and removal or migration gate legitimately converge through a separate atomic review.
