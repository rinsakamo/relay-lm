...
| `docs/relayemo_mvp_initial_design.md` | early RelayEMO design | RelayEMO nodes under `architecture/relationship/**` | absorbed; the MVP milestone name does not survive (rules 8, 9) |
...
| `docs/smoke/relaymem_local_llm_eval_guide.md` | repeatable evaluation method | `docs/evaluation/` | moved (Section L) |
| `docs/smoke/relaymem_local_response_comparison.md` | repeatable evaluation method | `docs/evaluation/` | moved (Section L) |
| `docs/smoke/relaymem_runtime_payload_eval.md` | repeatable evaluation method | `docs/evaluation/` | moved (Section L) |
...
| `docs/evaluation/*.md` | repeatable evaluation method | `docs/evaluation/` | retained |
| `docs/strategy/*.md` | non-binding future direction | `docs/strategy/` | retained (rule 7) |
| `docs/assets/**` | binary reference assets and their local index | `docs/assets/` | retained; not a design source family |
...
### Active-collection boundary gap

`docs/evaluation/` is a target collection in Section L and `docs/strategy/` is the rule 7 target, but neither is a permanent active location in `scripts/relaylm_documentation_governance_validate.py`, and neither has an allowed `relaylm_doc_type`. Documents there therefore cannot carry canonical graph metadata today. Closing that gap is a D6 implementation slice against the validator, not a change to this inventory.
