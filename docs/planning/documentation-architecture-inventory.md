...
|---|---|---|---|
| repository system context | `docs/architecture/pipeline_responsibility_design.md`, `docs/architecture/safe_soul_scene_ctx_compile_chain.md`, current architecture index responsibility text | `architecture/system-overview.md` and `architecture/pipeline-responsibilities.md` | superseded in D6-R35-V; durable system, pipeline, privacy, and compile-chain responsibilities are already canonical and the exact source remains Git-recoverable |
...
| context assembly | `docs/architecture/context_packing_design.md`, `docs/architecture/relayctx_wake_loop_design.md` | `architecture/context/context-assembly.md` | superseded in D6-R35-V for the context-packing source; RelayCTX wake-loop retirement and exact runtime contracts remain separately governed |
...
| Source family | Representative current sources | Target authority | Disposition |
|---|---|---|---|
| file-first workspace system | `file_first_character_workspace_design.md`, `safe_soul_scene_ctx_compile_chain.md` | `architecture/character-workspace/system.md` | superseded in D6-R35-V after the permanent Character Workspace architecture and source/compiler contracts absorbed the durable source, compiled-projection, and activation boundaries; the exact sources remain Git-recoverable |
...
| maintenance candidates | Character Workspace maintenance candidate/proposal implementation handoff | `architecture/character-workspace/maintenance-candidates.md` for durable responsibility and `contracts/character-workspace/maintenance-candidates.md` for exact current semantics | superseded in D6-R35-O after the permanent stable architecture and exact contract absorbed the durable responsibility and exact current semantics; the implementation handoff remains Git-recoverable |
| UI presentation | `cw_a3_character_workspace_ui_rebuild.md`, SOUL Lab UI sources | `architecture/ui/soul-lab.md` for the stable Character Workspace/SOUL Lab UI topology and browser/server authority boundary, with exact continuing semantics in `contracts/ui/home-conversation.md`, `contracts/ui/soul-lab-management.md`, and `contracts/ui/lifecycle-visibility.md`; operation-specific mutation surfaces remain under their own exact contracts | superseded; CW-A3 retired in D6-R35-L after the permanent UI architecture and exact UI contracts already owned the top-level surface split, Home conversation and Real Runtime/Local Preview boundary, character-scope and stale-request fencing, content-free default projection, browser storage minimization, and loopback security boundary; the earlier `architecture/ui/character-workspace.md` target never existed in the tree and is corrected here to the existing permanent owner rather than created as a duplicate UI architecture page; exact route hashes and component layout remain implementation details in `apps/soul-lab/**`, and the exact historical source remains Git-recoverable |
...
| formation pipeline | Phase 6 A/B/C sources, RelayMEM M3 formation/writer sources, integration I1 source | `architecture/memory/formation.md` | synthesized; exact queue/worker contracts rebuilt; the Phase 6 A/B admission, response-handoff, durable-queue, dispatch-preflight, enqueue, and queue-state sources are retired in D6-R35-P to permanent SLP contracts; the Phase 6-C worker/source-capture/finalization family is retired in D6-R35-Q to permanent formation and SLP owners; slice records to evidence |
| retrieval and grounding | `e1r4_retrieval_response_grounding.md`, `e1r5_primary_mem_recall_candidate_bridge.md`, ACG retrieval sources | `architecture/memory/retrieval-and-grounding.md`, `contracts/grounded-recall.md` | synthesized; E1-R4/E1-R5 handoffs retired in D6-R35-U while current slice evidence is retained |
...
| observation and character belief | `character_belief_relationship_dynamics_design.md`, ADR character-conditioned belief | `architecture/memory/observation-and-character-belief.md` | superseded in D6-R35-V after the permanent observation/belief and social-expression owners absorbed the durable provenance, belief, probing, repair, and disclosure boundaries; ADR remains decision authority and the exact source is Git-recoverable |
...
| Source family | Representative current sources | Target authority | Disposition |
|---|---|---|---|
| relationship state | `relayrel_relationship_design.md` | `architecture/relationship/relationship-state.md` | superseded in D6-R35-V after the permanent relationship-state, social-expression, workspace, and privacy owners absorbed the durable target-specific boundary; exact schema/update mechanics remain with their owning contracts/runtime and the source is Git-recoverable |
| scene model | scene portions of workspace, ACG-6, P0 ordering, SCN documents | `architecture/scene/scene-model.md` | synthesized; D6-R35-V retires the legacy `scene_lifecycle_design.md` source only; current RelaySCN v0 and P0 ordering handoffs remain protected |
| emotion modulation | RelayEMO design/cleanup/style sources | `architecture/emotion/affect-modulation.md` | synthesized |
| social expression | `character_belief_relationship_dynamics_design.md` | `architecture/relationship/social-expression.md` | superseded in D6-R35-V after the permanent social-expression and observation/belief owners absorbed the durable expression, probing, repair, and dependency-boundary semantics; the exact source is Git-recoverable |
...
| durable queue/worker | Phase 6 queue and worker contracts/handoffs | memory formation plus contracts | synthesized/rebuilt/evidence; D6-R35-P retires the Phase 6 A/B queue family and D6-R35-Q retires the Phase 6-C worker/source-capture/finalization family |
