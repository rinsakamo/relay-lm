...
Before every semantic request the transaction revalidates the live llama.cpp/GGUF/runtime binding. Runtime/material identity must remain equal across all four independently started shard servers. The transaction never contacts LM Studio.

A completed campaign is citable only within the frozen same-model S3 scope if all four shards complete and the exact 498/996 ledger, runtime/material binding, lineage/equality checks, and cleanup conditions pass. Any incomplete shard makes the transaction `S3_INCOMPLETE` and `citable=false`; later shards are not executed.
