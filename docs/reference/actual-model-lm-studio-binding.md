...
`write_lm_studio_actual_model_execution_result(...)` persists this envelope immutably. Identical same-ID bytes are idempotent. Different evidence under the same ID is rejected and requires a distinct `replicate_id` through the existing run identity contract.
