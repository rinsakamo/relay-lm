...
RT-1D-R5 immediate retirement remains unstarted during this correction and introduces no retirement behavior. After this correction merges and its exact resulting `main` is independently verified, RT-1D-R5 immediate retirement/proof becomes uniquely next and may restart only from a fresh branch created from that verified correction result, never from PR #837 head `efd936329f214464f3e872d2fe0e314a2e90210a`, never from the frozen `agent/rt1d-r5-immediate-retirement-proof` branch, and never from this correction's PR head.
