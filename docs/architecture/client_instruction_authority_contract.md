...
The canonical client-instruction fingerprint dry-run owner is `relaylm/interfaces/openai/client_instruction_fingerprint.py`; it consumes only canonical content-free extraction metadata and owns the unchanged `client_instruction_fingerprint_dry_run.v0` metadata-only readiness contract, default-off non-applied posture, blocked vocabulary, and content-free node projection without computing a fingerprint hash or performing cache I/O.
The canonical client-instruction cache dry-run owner is `relaylm/interfaces/openai/client_instruction_cache.py`; it consumes only the canonical content-free fingerprint dry-run artifact and owns the unchanged `client_instruction_cache_dry_run.v0` default-off diagnostics-only content-free dry-run metadata contract, fail-closed unexpected hash/cache-key validation, lookup/save readiness projection, and content-free node result without computing hashes or keys, performing cache I/O, or mutating payload, history, or persistence. Actual lookup/write owners and runtimes plus trace/audit projections remain separate consumers.
...
The canonical client-instruction identity runtime owner is `relaylm/interfaces/openai/client_instruction_identity_runtime.py`; it orchestrates canonical extraction and identity construction into request-local `PipelineContext` storage, normalizes exceptions and absent results fail closed, and projects only content-free `PipelineNodeResult` diagnostics while generic context/result ownership and trace/audit consumption remain separate.
The canonical client-instruction request-evidence owner is `relaylm/interfaces/openai/client_instruction_evidence.py`; it constructs deterministic low-trust request-local content-bearing `CLIENT_INSTRUCTION_EVIDENCE` blocks from canonical identity candidates, owns the shared rendered-size limit and fail-closed exactly-one legacy typed-block replacement, while generic compiler structures, renderer enforcement, and v1 preparation orchestration remain separate responsibilities.
...
The canonical pure client-instruction cache lookup validator is `relaylm/interfaces/openai/client_instruction_cache_lookup.py`; its persisted `client_instruction_cache_lookup.v0` and `relaylm.client_instruction_cache.v0` schema identities and runtime-private content-bearing result boundary are unchanged. The canonical read-only client-instruction cache filesystem reader is `relaylm/interfaces/openai/client_instruction_cache_reader.py`; its persisted `client_instruction_cache_reader.v0` schema identity, runtime-private content-bearing result boundary, path-hardening rules, and bounded-read policy are unchanged.
The canonical runtime-private read-only cache lookup orchestrator is `relaylm/interfaces/openai/client_instruction_cache_lookup_runtime.py`; its persisted `client_instruction_cache_lookup_runtime.v0` schema identity, content-bearing non-applied result boundary, fail-closed preparation, and content-free diagnostics projection are unchanged.
The canonical runtime-private client-instruction typed-parse validator is `relaylm/interfaces/openai/client_instruction_typed_parse.py`; its `client_instruction_parse.v1` and `client_instruction_typed_parse_runtime.v0` schema identities, content-bearing non-persisted artifact boundary, fail-closed allowlist validation, and content-free diagnostics are unchanged.
The canonical gated client-instruction cache-write preflight owner is `relaylm/interfaces/openai/client_instruction_cache_write.py`; its `client_instruction_cache_write_preflight.v0`, `relaylm.client_instruction_cache.v0`, `client_instruction_cache_key.v0`, and `client_instruction_authority.v1` schema and policy identities, default-off dry-run-first posture, bounded atomic filesystem write behavior, runtime-private result boundary, and content-free diagnostics are unchanged.
The canonical runtime-private typed-parse and cache-write orchestrator is `relaylm/interfaces/openai/client_instruction_cache_write_runtime.py`; its trusted in-process typed-parse source, identity dependency, default-off and dry-run-first configuration gates, preflight/write delegation, runtime-private result boundary, content-free node diagnostics, and fail-closed preparation behavior are unchanged.
The canonical OpenAI-compatible client-history exclusion preflight owner is `relaylm/interfaces/openai/client_history_exclusion_preflight.py`; its `client_history_exclusion_preflight.v0` schema identity, runtime-private content-bearing current-user candidate, content-free diagnostics, managed/pass-through classification, cache hit/miss/blocked resolution, and active-tool fail-closed semantics are unchanged.
The canonical no-instruction client-history exclusion pure apply owner is `relaylm/interfaces/openai/client_history_exclusion_apply.py`; its `client_history_exclusion_apply.v0` schema identity, runtime-private content-bearing forwarded-payload candidate, content-free diagnostics and node projection, managed/pass-through classification, default-off dry-run-only posture, exact current-user preservation, active-tool fail-closed behavior, and no-history-fallback rule are unchanged.
The canonical instruction-bearing client-history exclusion typed result owner is `relaylm/interfaces/openai/client_history_exclusion_apply_v1_types.py`; its `client_history_exclusion_apply.v1` schema identity, exact status/resolution/source/blocked-reason vocabularies, runtime-private content-bearing forwarded-payload result, forced-false raw-instruction, RelayLM-control, cache-entry-content, and cache-projection safety flags, and result-builder semantics are unchanged.
The canonical instruction-bearing client-history exclusion validation owner is `relaylm/interfaces/openai/client_history_exclusion_apply_v1_validation.py`; its validated-input shape, preflight and instruction-identity schema/type/runtime-private checks, candidate count/source and current-user consistency checks, active-tool fail-closed behavior, instruction-evidence and compiler-block validation, blocked-reason semantics, runtime-private content-bearing input boundary, and content-free public diagnostic boundary are unchanged.
The canonical instruction-bearing client-history exclusion preparation owner is `relaylm/interfaces/openai/client_history_exclusion_apply_v1_prepare.py`; its validated-input consumption, explicit provenance, instruction identity/source and preflight/current-user consistency, compiler-block and instruction-evidence construction, active-tool fail-closed behavior, runtime-private content-bearing prepared state, forwarding/cache safety boundaries, blocked-reason semantics, and content-free public diagnostic boundary are unchanged.
The canonical instruction-bearing client-history exclusion render owner is `relaylm/interfaces/openai/client_history_exclusion_apply_v1_render.py`; its rendered-result shape, prepared-artifact dependency, instruction-source control stripping, RelayLM control-envelope exclusion, request-compiler rendering and evidence escaping/bounds, exact current-user preservation, runtime-private content-bearing candidate, forwarding/cache safety boundaries, fail-closed render behavior, and content-free public diagnostic boundary are unchanged.
The canonical instruction-bearing client-history exclusion runtime selection owner is `relaylm/interfaces/openai/client_history_exclusion_apply_v1_runtime.py`; its request-local eligibility decision from canonical preflight and instruction identity with the existing bounded role classification is unchanged, and it owns no payload mutation, result union, pipeline-node publication, backend-block projection, backend-forward authority, persisted content, old-path alias, or forwarder.
The canonical client-history exclusion outer apply runtime owner is `relaylm/interfaces/openai/client_history_exclusion_apply_runtime.py`; it owns the v0/v1 result union, request-local idempotent apply orchestration, detached forwarded-payload mutation, pipeline-node publication, and content-free backend-block/failure projection while preserving managed/pass-through and default-off dry-run gates, fail-closed behavior, and runtime-private content-bearing payloads. Adapter backend forwarding remains a separate consumer responsibility, and no old-path alias or forwarder exists.
The canonical client-history exclusion backend-forward gate policy owner is `relaylm/interfaces/openai/client_history_exclusion_backend_forward_gate.py`; it converts the canonical outer-runtime result and exact forwarded-payload comparison into one content-free allow/block decision with a bounded failure reason. The adapter retains active-context lookup, `BackendRequestError` transport ownership, and the stream/non-stream pre-I/O integration seam without duplicating policy.
The canonical OpenAI backend request payload projection owner is `relaylm/interfaces/openai/backend_request_payload.py`; after the backend-forward gate evaluates the exact request-local forwarded payload, it constructs the detached stream/non-stream backend-bound payload by preserving pass-through controls, stripping the reserved RelayLM control envelope for managed routes, and projecting the selected backend model. Adapter transport retains backend I/O ownership, and no old compatibility path or duplicate projection owner exists.
