...
Current mode fields and route-level mode overrides are defined in [Configuration Reference](../reference/configuration.md); mode-specific target dataflow is owned by [Request / Response Pipeline](runtime/request-response-pipeline.md); current implementation posture remains owned by [Project Status](../PROJECT_STATUS.md).
