...
- a ready client-instruction identity result whose cache key matches the current runtime lookup expectation,
...
C5b keeps persisted `parser_version` set to `null` because the current runtime cache lookup path validates entries without a parser-version expectation. To avoid writing entries under keys that the current runtime cannot find, the writer recomputes the runtime-compatible cache key with `parser_version=null` and blocks identities whose supplied key does not match that value. A later runtime-wiring slice may pass a parser version through lookup; until then, writer output must remain compatible with the runtime reader path.
...
2. validated cache-root directory handle opening with no-follow and `fstat` checks,
3. temporary file creation relative to the validated directory handle,
4. file flush + `fsync`,
5. atomic replace relative to the same validated directory handle,
6. best-effort directory `fsync`.
...
- parser-versioned identity blocking when the key is not runtime-compatible,
