...
- heading sections with multiple non-empty body lines and heading-addressed chunks that also contain a canonical inline assignment for the same key remain outside the heading-negation rule; multiple scalar/reserved-degree assignments and partially unrecognized boolean assignment sets remain outside special multiple-assignment interpretation; ordinary positive heading/assignment matching and mismatch behavior is unchanged;
...
- the bounded exact-leading-`not` rules do not define aliases, synonyms, contractions, `never`, double-negation semantics, omitted-key inference, multi-line heading-body negation, heading-plus-inline overlap semantics, free-form multiple-assignment negation, reserved-degree negation, or broader natural-language negation;
...
- outside the single-inline, multiple-inline exact-boolean, or heading-single-body exact boolean rules, a boolean State-addressing non-historical chunk is suppressed only when it contains the exact opposite `true` / `false` token and does not also contain the current boolean token;
...
- State-vs-memory authority beyond the current deterministic structural addressing forms and typed-current positive-scalar/exact-scalar-negation/exact-positive-or-negated-boolean/explicit-reserved-degree grammar, including omitted-key aliases/synonyms, multi-line heading-body negation, heading+inline overlap, scalar/reserved-degree or partially unrecognized multiple-assignment semantics, structural reserved-degree negation, non-exact boolean negation, contractions and broader negation NLP, natural-language degree/intensity interpretation, degree ordering/tolerance or cross-axis comparison, and other non-lexically-comparable values;
