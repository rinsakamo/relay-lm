...
- this heading-addressed single-body structural negation rule also applies to typed `current` and typed `unknown` MEMORY, while typed `historical` remains exempt before structural filtering;
- heading sections with multiple non-empty body lines and heading-addressed chunks that also contain a canonical inline assignment for the same key remain outside the heading-negation rule; chunks with multiple canonical assignments for the same key also remain outside the single-inline rule; ordinary positive heading/assignment matching and mismatch behavior is unchanged;
...
- the bounded exact-leading-`not` rules do not define aliases, synonyms, contractions, `never`, double-negation semantics, omitted-key inference, multi-line heading negation, heading-plus-inline overlap semantics, multiple-assignment negation, heading-addressed boolean negation, reserved-degree negation, or broader natural-language negation;
...
- for State values handled by the structural heading/field rule, the non-historical chunk is retained if at least one current State value appears as an exact lexical token sequence in the chunk, except for the bounded single-inline and heading-single-body exact-negation corrections above;
...
- outside the single-inline exact boolean assignment rule, a boolean State-addressing non-historical chunk is suppressed only when it contains the exact opposite `true` / `false` token and does not also contain the current boolean token;
- outside that exact assignment rule, a structurally State-addressing boolean chunk containing the current token remains compatible; a chunk containing neither boolean token, or both tokens, is left untouched rather than being semantically or temporally reclassified;
...
- State-vs-memory authority beyond the current deterministic structural addressing forms and typed-current positive-scalar/exact-scalar-negation/exact-positive-or-negated-boolean/explicit-reserved-degree grammar, including omitted-key aliases/synonyms, multi-line heading-body negation, heading+inline overlap, multiple-assignment negation semantics, heading-addressed boolean negation, structural reserved-degree negation, non-exact boolean negation, contractions and broader negation NLP, natural-language degree/intensity interpretation, degree ordering/tolerance or cross-axis comparison, and other non-lexically-comparable values;
