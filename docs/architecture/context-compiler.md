...
- all matching C24 assignments continue through C1 rather than retaining early, so the existing heading-owned section-wide degree check can still suppress; if any member is non-exact or begins with exact leading `not`, C24 makes no partial set decision and falls through to C1;
...
- C28 does not redefine C23. If any member is positive, non-exact, bare `not`, double-negated such as `not not likes`, or otherwise outside the exact all-negated boundary, C28 makes no partial set decision and the whole set falls through to existing authority. Mixed positive+negated reserved-pair sets, heading-addressed multiple reserved-pair negation sets, exactly-one inline assignments (C22/C25 remain owners), heading-only single-body negation (C27 owner), typed-current free-form reserved-degree negation, `never`, contractions, aliases/synonyms, composites, omitted-key inference, degree ordering/tolerance/cross-axis comparison, natural-language intensity, and broader NLP remain deferred;
