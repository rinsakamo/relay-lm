...
- heading sections with multiple non-empty body lines remain outside the heading-single-body rule; heading+inline overlap outside C16's exact single boolean assignment and C17's all-exact boolean assignment set, heading+multiple simple-scalar assignments, reserved-degree multiple assignments, empty/non-lexical simple-scalar assignment sets, and partially unrecognized boolean assignment sets remain outside special overlap/multiple-assignment interpretation; ordinary positive heading/assignment matching and mismatch behavior is unchanged;
...
- State-vs-memory authority beyond the current deterministic structural addressing forms and typed-current positive-scalar/exact-scalar-negation/exact-positive-or-negated-boolean/explicit-reserved-degree grammar, including omitted-key aliases/synonyms, multi-line heading-body negation, scalar/reserved-degree or partially unrecognized/non-exact boolean heading+inline overlap, heading+multiple scalar assignments, reserved-degree or empty/non-lexical multiple-assignment semantics, structural reserved-degree negation, non-exact boolean negation, contractions and broader negation NLP, natural-language degree/intensity interpretation, degree ordering/tolerance or cross-axis comparison, and other non-lexically-comparable values;
