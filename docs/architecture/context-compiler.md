...
- heading sections with multiple non-empty body lines remain outside the heading-single-body rule; heading+multiple simple-scalar assignments, reserved-degree heading+inline or multiple-assignment semantics, empty/non-lexical simple-scalar assignment sets, and partially unrecognized boolean assignment sets remain outside special overlap/multiple-assignment interpretation; ordinary positive heading/assignment matching and mismatch behavior is unchanged;
