...
- for those simple scalar State values only, a captured claim beginning with the exact leading lexical token `not` and at least one following token receives bounded negation handling: `not <active State value>` suppresses the chunk, while `not <different value>` is retained rather than being treated as a positive mismatching scalar;
- this exact leading-`not` rule does not define aliases, synonyms, contractions, `never`, double-negation semantics, omitted-key inference, structural heading/field negation, boolean negation, reserved-degree negation, or broader natural-language negation;
