...
- a matching C21 assignment does not weaken C1: every other explicit numeric `degree_hint:` / `degree_hint=` in the heading-owned section remains associated with that key and any stale section degree still suppresses; non-exact assignment values, inline-only reserved semantic locality, and multiple same-key reserved assignments remain outside C21 local interpretation;
- for State values handled by the structural heading/field rule, the non-historical chunk is retained if at least one current State value appears as an exact lexical token sequence in the chunk, except for the bounded single-inline, multiple-inline simple-scalar, heading+single-inline simple-scalar, heading+multiple-inline simple-scalar, heading+single-inline exact-boolean, heading+multiple-inline exact-boolean, multiple-inline exact-boolean, heading+single-inline exact-reserved-degree, and heading-single-body exact-negation corrections above;
...
- State-vs-memory authority beyond the current deterministic structural addressing forms and typed-current positive-scalar/exact-scalar-negation/exact-positive-or-negated-boolean/explicit-reserved-degree grammar, including omitted-key aliases/synonyms, multi-line heading-body negation, inline-only reserved-degree semantic locality, heading+multiple reserved-degree assignments, reserved-degree or empty/non-lexical multiple-assignment semantics beyond current fallback, structural reserved-degree negation, non-exact boolean negation, contractions and broader negation NLP, natural-language degree/intensity interpretation, degree ordering/tolerance or cross-axis comparison, and other non-lexically-comparable values;
