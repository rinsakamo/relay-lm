...
- heading sections with multiple non-empty body lines remain outside the heading-single-body rule; heading+inline overlap outside C16's exact single boolean assignment and C17's all-exact boolean assignment set, multiple scalar/reserved-degree assignments, and partially unrecognized boolean assignment sets remain outside special overlap/multiple-assignment interpretation; ordinary positive heading/assignment matching and mismatch behavior is unchanged;
...
- the bounded exact-leading-`not` rules do not define aliases, synonyms, contractions, `never`, double-negation semantics, omitted-key inference, multi-line heading-body negation, structural heading-plus-inline overlap beyond the exact boolean rules above, free-form multiple-assignment negation, reserved-degree negation, or broader natural-language negation;
...
- for State values handled by the structural heading/field rule, the non-historical chunk is retained if at least one current State value appears as an exact lexical token sequence in the chunk, except for the bounded single-inline, heading+single-inline exact-boolean, heading+multiple-inline exact-boolean, multiple-inline exact-boolean, and heading-single-body exact-negation corrections above;
...
- State-vs-memory authority beyond the current deterministic structural addressing forms and typed-current positive-scalar/exact-scalar-negation/exact-positive-or-negated-boolean/explicit-reserved-degree grammar, including omitted-key aliases/synonyms, multi-line heading-body negation, scalar/reserved-degree or partially unrecognized/non-exact boolean heading+inline overlap, scalar/reserved-degree or partially unrecognized multiple-assignment semantics, structural reserved-degree negation, non-exact boolean negation, contractions and broader negation NLP, natural-language degree/intensity interpretation, degree ordering/tolerance or cross-axis comparison, and other non-lexically-comparable values;
