...
relaylm_update_trigger:
  - LC-1C Pin/Unpin input, transition, persistence, or recovery changes
  - RT-1 begins consuming pinned lifecycle eligibility
  - Subjective MEM lifecycle state or selector authority changes
...
---
# LC-1C Subjective MEM Pin / Unpin Runtime

## Scope

LC-1C is the next bounded Subjective MEM lifecycle slice after LC-1A Correct and
LC-1B Forget. It defines one caller-invoked, default-off Pin / Unpin runtime on
the canonical Markdown and content-free operations boundaries established by
ST-1 and the earlier lifecycle slices.
...
The accepted Subjective MEM logical contract owns Pin state as lifecycle state,
not as an orthogonal mutable flag:
...
The existing Primary MEM I-5A/I-5B implementation is characterization and
migration evidence only. Its durable pin receipt/state projection and ranking
hint are not imported as the Subjective MEM semantic authority.

LC-1C therefore does not:
...
projection consumption, ranking, cache, usage events, and hard cutover.

## One semantic owner

`relaylm/subjective_mem_pin_runtime.py` is the only LC-1C operation owner. It
owns one shared Pin/Unpin engine and exposes two bounded public entry points:
...
Both entry points delegate to the same engine. The explicit operation kind
derives the only supported transition pair:
...
recovery bodies are prohibited. A second runtime, private core, ContextVar
dispatch, monkeypatch, fallback import, wrapper-only indirection, or
...
The owner must not import Primary MEM Pin/Unpin runtime code or a high-level
Correct/Forget runtime private helper.
...
LC-1C reuses the existing Subjective MEM authority boundaries:
...
Before canonical page replacement, LC-1C stores one lifecycle operation claim,
one prepared lifecycle intent, and the singleton selector with
`mutation_state: prepared`.

After exact post-image verification, one Evidence-space transaction inserts:
...
No Pin/Unpin-specific durable state or second current selector is allowed.
Lifecycle transition plus final selector state are the canonical Pin authority.
...
LC-1C fails closed for stale or non-current revision, wrong direction,
...
LC-1C uses the existing Subjective MEM lifecycle gate and remains default-off,
...
The bounded runtime slice should normally own:
...
tests/test_subjective_mem_smoke_registration.py
```

Small changes to generic Subjective MEM lifecycle or Markdown modules are
allowed only when they transfer a genuinely shared responsibility to one stable
owner. Copying a large Correct/Forget implementation, importing its private
helpers, adding a permanent compatibility wrapper, or adding a second
selector/projection authority fails the Stable-Structure Gate.

The existing `runtime/subjective_mem_lifecycle` consolidated group is the CI
owner. LC-1C extends that group rather than creating a workflow or editing the
Lane R changed-matrix authority.
...
- concurrent Correct/Forget/Pin/Unpin first-writer behavior;
...
- focused pytest and process smoke in the existing lifecycle group;
- Python 3.10/3.12 unit suites and exact-head repository CI.

## Non-goals

LC-1C does not implement ordinary Subjective MEM Retrieval/ranking/cache/RT-1,
API/UI, Correct, Forget, Restore, Consolidate, Merge, Supersession, Purge,
Primary MEM migration/dual-write/retirement, model-generated Pin/Unpin,
background recovery, multi-host publication, non-POSIX apply, or user-data
migration.
