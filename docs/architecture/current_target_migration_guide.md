...
This section includes the Wave 7 / E1-R5 compatibility interpretation plus the post-Wave-7 P0-PIPE, ACG-1 through ACG-6, CW-A1 through CW-A5, O2/O3, and PM-D5 through PM-D7 current boundaries.
...
PM-D5 through PM-D7 are completed post-MVP debt slices. PM-D5 removed legacy flat RelayMEM runtime discovery. PM-D6 replaced the input-side RelayREF-shaped RelayINT compatibility artifact with a native RelayINT artifact. PM-D7 added an explicit dry-run-first `relaylm-runtime-install` command for local setup/preflight and allowlisted safe directory creation under `--write`.
