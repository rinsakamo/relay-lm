...
E1-R5 is a correction to the Wave 7 E1 proof boundary. It does not reopen Wave 7 and does not add production authority beyond the bounded request-side bridge described in [E1-R5 Primary MEM Recall Candidate Discovery Bridge](e1r5_primary_mem_recall_candidate_bridge.md).
...
Current docs must not read the E1 recall proof as "M2 alone always selects current eligible scoped Primary MEM". The corrected proof boundary is M2-preferred recall plus the E1-R5 bounded scoped candidate bridge for the no-M2-scoped-candidate gap.
...
- E1-R4 remains request-side only and consumes the selected-memory shape produced by either the M2-preferred path or the E1-R5 fallback bridge.

## Follow-up debt

The bounded runtime bridge is intentionally tracked as Post-MVP decision debt rather than silently remaining as an unowned adapter drift risk.

```text
PM-D8: Fold E1-R5 bounded Primary MEM candidate bridge into canonical Primary recall adapter
```

PM-D8 is closely related to PM-D5 RelayMEM flat-store compatibility removal because both may touch Primary recall layout discovery and adapter/root handling. Until PM-D8 is closed or absorbed, E1-R5 bridge behavior must remain covered by the E1-R5 smoke set.
...
PYTHONPATH=. python scripts/relaylm_e14_unsupported_detail_suppression_smoke.py
