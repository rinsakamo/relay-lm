...
O1A contract/model/smoke is complete. It fixes replay-before-queue ordering, at most one delegation per lane, independent queue discovery after replay, lane-local failure isolation, stop/run-next/idle semantics, target-only default-off gates, and a content-free projection. It adds no scanner, delegation, polling, sleep, config field, CLI, daemon, or service.
...
Thread C  O1B/O1C lane implementation after exact dependency readiness
