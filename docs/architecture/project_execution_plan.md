...
The current sequence is: PR #803 exact result `eee986422b45c50e0d9ad0528e863457be4db9a1` -> renewed R2 P1 Return without mutation -> live-root budget amendment PR #804 exact result `00ba475c689631520538b7531022603447f11bd0` -> R2 P1 Return recorded in closed tree-neutral Draft PR #805 head `733b38fd3e74dcc542dd1c8f2ec1353a2cab6a95` -> current queued-runner root budget amendment Draft PR #PENDING -> independently verify the amendment exact resulting main -> renewed R2 implementation on a fresh branch, not started -> mandatory R2 P8 only after the implementation merges. Only the verified amendment result, never its PR head and never the PR #805 head, may bootstrap renewed R2. The amendment requires no P8 and changes no production/runtime behavior.
