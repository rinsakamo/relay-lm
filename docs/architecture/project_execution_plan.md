...
Within the pre-existing post-E1-R5 decision-debt registry: The remaining candidates are PM-D1/PM-D4/PM-D9 follow-through and PM-D2 closure or absorption after PM-D6. The separately registered dependency-first implementation program is complete through RT-1D-S2: EV-1, OVL-1, ASM-1, SM-1, the default-off bounded ST-1 create commit slice, LC-1A through LC-1E, the RT-1A contract and projection foundation, the RT-1B projection builder and deterministic rebuild, the default-off shadow-only RT-1C selection, characterization, and usage ledger, the behavior-preserving RT-1D-S1 reader seams completed in PR #789, and the behavior-preserving RT-1D-S2 worker seams completed in PR #791 with exact resulting main `31b700a2db0af7819f761d51bd946ff6798eb4c9`. LC-1 lifecycle migration is complete; RT-1D-R1 is complete and mandatory R1 P8 PR #802 is current. PR #790 completed the S1 mandatory P8 current-authority sync with exact resulting main `3e20274f18306f7db2410fd5239051411b9c052b`. PR #792 completed the S2 mandatory P8 current-authority sync, and the S3A-S3C structural sequence plus mandatory P8 transactions subsequently completed through PR #799 result `d9caff1750e93f9d4ce2f0852e070bc96cb1bf2f`. Architecture PR #800 and R1 PR #801 then completed with their recorded results; mandatory R1 P8 PR #802 now gates R2. Primary MEM remains the sole ordinary served memory and Retrieval authority. PM-D4, PM-D9, and PM-D1 apply as the narrow default-on, multilingual-generation, and SOUL-conditioned-formation gates recorded above; PM-D2 remains separately governed. Durable-memory E2 value smoke is complete as local human-reviewed v0.1 readiness evidence. RelayATN remains ATN-0 planning-only debt after voice-out / SOUL Lab Runtime MVP and does not authorize implementation.
...
PR #800 completed the R1-R5 architecture budget with result `68cc16b9d5ed7b999c22d27457390e53de851335`. R1 PR #801 completed with result `90a3c4f1cedf54e007cf5c0a6a9abc69a30d2acd`; mandatory R1 P8 PR #802 completed with exact resulting main `9ce7de054869ef29cc508d176023a93947489c25`. The initial R2 attempt returned at P1 with no repository mutation after its exact-main inspection triggered structural review. This architecture-only amendment is the current Lane C transaction as Draft PR number pending and requires no P8. R2 remains not started and may restart only after the amendment merges and its exact resulting main is independently verified; the amendment PR head cannot bootstrap R2.
...
### R2 reviewed structural resolution

The approximately-700-line rule is applied as the `AGENTS.md` P1 review trigger, not a universal hard cap. Exact main contains `relaylm/_relaymem_primary_pipeline_impl.py` at 1,033 physical lines, blob `faf3fc71227da8e156f8d12f940591f189d1bb0c`, and `relaylm/relaymem_primary_pin.py` at 742 physical lines, blob `ee4db564faccb41e2354a639a527e3c8a3652f40`. They already own the Primary pipeline implementation and Primary Pin responsibilities. R2 adds only immutable writer-decision carriage and bounded pre-side-effect checks; it does not move those responsibilities. Splitting only to reduce line count would be meaningless fragmentation, wrapper-only indirection, and unrelated refactoring, so no new production owner is authorized.

The original exact twenty-path R2 production budget remains unchanged. This exact-blob acceptance is narrow to R2 and is not reusable by later slices without fresh P1 evidence. The pipeline implementation may finish at no more than 1,083 lines (+50 net), Primary Pin at no more than 777 (+35 net), and the 403-line cutover semantic owner at no more than 550. Every other authorized sub-700 module remains below 700 and gains at most 80 lines. New functions are at most 80 lines and new orchestration functions at most 60. A touched pre-existing function above 80 must record its baseline, gain neither responsibility nor span, and delegate the check to a bounded same-owner helper; existing functions at or below 80 remain at or below 80. No line-golfing, removed validation, unrelated cleanup, extraction, rename, formatting sweep, schema redesign, or ownership movement is authorized.

After this amendment's merge and independently verified result, renewed R2 P0/P1 must freeze focused tests; enumerate all live Primary formation/mutation roots; measure all twenty modules and functions; project the 1,083/777 maxima; prove no real split is justified; prove in-budget checks dominate every side effect; and prove no twenty-first production path is required. Config, EvidenceRecordStore, direct M3e/M3g implementations, ordinary readers, docs, workflows, and scripts must remain unchanged. Any exceeded allowance or genuine responsibility split requires another P1 amendment. This amendment changes authority only and changes no production/runtime behavior.
