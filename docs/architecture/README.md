...
Phase 6-A1 validates deferred RelaySLP admission metadata. Phase 6-A2 creates one runtime-private dry-run enqueue candidate after a finalized `turn_end`. Phase 6-B0 owns the durable-record, dispatch-idempotency, state-machine, duplicate/collision, lease/restart/corruption, and content-free projection contract. Phase 6-B1 generates deterministic dispatch/job identities and a runtime-private initial queued durable-job candidate without queue I/O. Phase 6-B2 performs gated atomic create-if-absent durable enqueue. Phase 6-B3 performs fenced claim, renewal, retry release, stale recovery, and terminal commit. I1-B wires ordinary managed non-stream and stream response finalization to post-response A1 -> A2 -> B1 -> B2 enqueue and protected capture production. C1-0 owns exact current-claim source construction, C1-1 composes M3a-M3h, C1-2 executes one already-claimed job, C1-3 classifies outcomes, C1-4 verifies integrated crash/fault convergence, and C1-5 durably persists and restart-rehydrates the claim-independent protected capture. Phase 6-C1 is restart-complete for protected worker-source recovery. Next-turn Primary MEM recall, queue scanning, scheduling, and daemon lifecycle remain unimplemented.
