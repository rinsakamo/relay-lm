...
The current provider adapter requires an HTTP or HTTPS base endpoint with a host and no query or fragment delimiter. Preflight rejects obvious malformed provider hostname syntax containing whitespace or a backslash and rejects literal whitespace or ASCII control characters anywhere in the configured base-URL string before startup succeeds. Percent-encoded URL data is not decoded or reinterpreted by this lexical check. RCFG4 does not issue network requests during `doctor`; hostname resolution and provider reachability are intentionally not inferred from an adapter-specific discovery endpoint.
