...
The apply gate's position relative to RelayMEM injection and token-budget truncation is owned by [Pipeline Responsibility Design §9 RelayCTX Repack](../architecture/pipeline_responsibility_design.md#9-relayctx-repack), which states the phase order (`relaymem runtime CTX/snippet injection -> RelayCTX short-term runtime injection -> token_budget_truncation`) and the rule that injection phases must not run after truncation. This contract records where the extraction/assembly/preflight artifact chain is built around that owned mutation order; it does not create a second ordering authority. [Project Status](../PROJECT_STATUS.md) records that the apply stage once ran after truncation and was fixed; `scripts/relaylm_ctx_repack_final_gate_smoke.py` pins that mutation-order regression.
