...
RelayINT's quick-clarification chain is a three-stage, default-off, content-free, plan-only diagnostics path: fast-path dry-run -> quick-clarification preflight -> quick-clarification apply plan. All three stages run as part of `relaylm/relayint.py`'s request-local processing; each stage returns `None` (or, for the apply plan, a plan that always carries `apply_allowed: False`) while its own or an upstream stage's flag is disabled.
...
- Stage 1 returns `None` unless `relayint_fast_path_dry_run_enabled` is `True`.
- Stage 2 returns `None` unless `relayint_quick_clarification_preflight_enabled` is `True` and Stage 1 produced an artifact.
- Stage 3 returns `None` unless `relayint_quick_clarification_apply_enabled` is `True`. When produced, its `apply_allowed` is unconditionally `False` today (see Stage 3).

Enabling a later stage without its predecessor does not synthesize a missing predecessor artifact; Stage 2/3 record the documented missing-input blocked reason instead.
...
- Sole input artifact: Stage 2's `relayint_quick_clarification_preflight.v0`.
- `apply_allowed` is computed from `block_reasons` being empty, but **`block_reasons` always includes `phase4_plan_only`** (`relaylm/relayint.py:243`, unconditional), so `apply_allowed` is currently always `False`. `response_short_circuit_allowed` and `short_circuit_applied` are hardcoded `False`.
- Full current `block_reasons` taxonomy (`build_relayint_quick_clarification_apply_plan()`):

```text
preflight_missing              - Stage 2 artifact absent
...
scene_gate_blocked             - Stage 2 scene_gate.quick_clarification_allowed is not True
                                  (plus the underlying scene_gate.block_reasons, deduplicated)
dry_run_only                   - apply_dry_run_only is True
streaming_not_supported        - the request is a streaming request
response_format_requested      - request compatibility gate (see below)
tools_requested
tool_choice_requested
functions_requested
function_call_requested
multiple_choices_requested / unsupported_n_value
max_completion_tokens_too_small / unsupported_token_limit
max_tokens_too_small / unsupported_token_limit
logprobs_requested
top_logprobs_requested
stop_sequence_requested
audio_modality_requested / non_text_modality_requested / unsupported_modalities_value
audio_options_requested
response_template_missing      - no template resolves for the clarification_type
response_max_chars_exceeded    - resolved template exceeds response_max_chars
phase4_plan_only               - always appended; forces apply_allowed = False
```

- Request compatibility gate: `build_relayint_request_compatibility_gate()` in `relaylm/relayint.py` evaluates the raw backend-bound payload and returns `compatible` plus the block reasons above. It rejects `response_format`, `tools`/`tool_choice`, `functions`/`function_call`, `n > 1`, too-small `max_completion_tokens`/`max_tokens`, `logprobs`/`top_logprobs`, `stop`, non-text `modalities` (including `audio`), and `audio` options.
- Response template metadata, never rendered text: `generated_response_kind` (`generic_prior_memory_reentry`, `generic_reference_clarification`, `generic_open_clarification`, or `none`), `response_template_id` (`{kind}.ja.v0` or `none`), `response_chars` (a fixed count per template: 25 or 19, never computed from live text).
...
As of this cutover, no phase beyond MVP-47 has shipped actual user-visible quick-clarification apply. `block_reasons` always includes `phase4_plan_only`; no runtime code path reads `apply_allowed` to change request handling. [RelayINT MVP Design](../architecture/relayint_mvp_design.md) records this as a target "Phase 6" route requiring compatibility/active-transaction gates, RelaySCN scene/formality/recovery gates, RelayRUN short-circuit/checkpoint routing, and the normal output safety boundary — none of which this chain implements. Treat any future claim that actual apply exists as requiring independent code verification, not this contract or the retired MVP-47 text.
