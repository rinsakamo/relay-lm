...
P0-PIPE RelayREL / RelaySCN / RelayEMO ordering: complete for RelaySCN ownership, RelayREL content-free placeholder/projection, and content-free order smoke boundary
...
P0-PIPE ordering slice merged as the Character Workspace reset prerequisite
```
...
P0-PIPE is complete as the Character Workspace prerequisite slice. RelaySCN no longer accepts RelayEMO artifact `scene_state` as normalized scene-state fallback, RelaySCN remains the owner of normalized `scene_state` and `scene_policy`, RelayEMO remains affect/expression ownership only, and RelayREL now has a content-free placeholder/projection for request-path ordering evidence.
