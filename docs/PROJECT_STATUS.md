...
v0.1 readiness is recorded in [v0.1 Release Readiness](release/v0.1-release-readiness.md). All current MVP execution lanes listed below are complete. The durable-memory E2 value smoke after O2/O3 scheduler draining evidence has been completed locally and human-reviewed; content-bearing comparison artifacts remain local-only under `local/value_smoke/` and are not committed. The only remaining tracked items are post-v0.1 decision debt and broader not-yet-implemented capabilities listed below.
...
ASM-1 Shared Assessment runtime foundation: complete as a default-off deferred library/runtime slice with exact EV-1 authorization, character-independent Assessment Pass bundles, immutable consecutive revisions, one logical current-state selector, and a decision-transaction-bound formation authorization receipt builder; no independently persisted receipt, Subjective MEM writer, or response-path integration
...
Offline extraction tooling (runtime non-contact) added: caller-invoked, bounded `scripts/relaylm_twin_extraction_*` preprocessing/batch-runner/merge CLIs that turn X archive and ChatGPT export material into a reviewable style/fact extraction artifact. See [Twin Extraction runbook](operations/twin-extraction.md). This tooling does not import `relaylm`, does not write to MEM/SOUL, and does not change O2/O3, CW-A4/CW-A5, or RelaySLP status.
...
Twin review import source -> CW-A4 candidate/proposal planning flow is documented and smoke-covered: see [Twin Review Import -> CW-A4 Workspace Candidate Flow](operations/twin-review-to-workspace-candidates.md) and `scripts/relaylm_twin_review_to_cw_a4_flow_smoke.py`. This connects the existing bridge output to existing CW-A4 dry-run and write-candidates planning; it does not add MEM/SOUL auto promotion, Primary MEM bootstrap from Twin Extraction, SOUL auto rewrite, or runtime dogfood ingestion.
...
Single-owner mobile dogfood target boundary is documented in [Mobile Dogfood Entry](operations/mobile-dogfood-entry.md), but external publication remains blocked until a concrete dedicated chat-only origin and negative-path acceptance evidence exist. The current Vite Character Workspace development server, RelayLM `/v1`, `/lab/api`, LM Studio, SOUL Lab management surfaces, Memory Inspector, and admin/debug routes are not supported public tunnel origins. This target operations document does not change RelayLM runtime behavior, add multi-user/family-tester/actor support, or add Cloudflare API integration.

Mobile dogfood observation method and local-only record templates added: see [Mobile Dogfood Observation Method](evaluation/mobile-dogfood-observation.md). This is local-only evaluation guidance only: it does not change runtime behavior, does not complete mobile dogfood evidence, does not update release readiness, and does not improve latency.

LAT-2 mobile perceived-latency measurement added: a content-free `relayrun.stream_timing.v0` trace records streaming `time_to_first_chunk_ms`/`stream_drain_ms`/`stream_chunk_count` as a second, later trace record once a stream finishes, since LAT-1's `timing_summary.time_to_first_token_ms` stays `null` for streaming responses. See [LAT-2 Mobile Perceived Latency](architecture/lat2_mobile_perceived_latency.md). This is measurement only: no SSE payload, backend-forwarding, retrieval-algorithm, timeout, or degradation-ladder change, and no response-time guarantee or optimization is implemented or claimed.
