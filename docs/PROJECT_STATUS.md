...
- auditable formed-Primary-MEM Correct is implemented,
- Phase I-4A defines Forget / Hide only as a target contract; production Forget apply, M2 exclusion, and Forget UI are unimplemented,
- pin/unpin/merge remain unimplemented.
...
- no production Forget/Pin/Unpin/Merge or held apply/discard operation,
- no SOUL Lab Forget UI despite the defined I-4A target contract,
...
The target contract fixes one terminology and persistence model:

```text
user-facing operation: Forget
canonical lifecycle state: hidden
runtime-private audit artifact: Forget tombstone
persistence decision: immutable hidden successor Primary page with revision N+1
```

The hidden successor page is lifecycle authority; the tombstone is audit/recovery evidence, not an independently updated sidecar flag. Correct and Forget must share one per-memory revision fence and one canonical current-state resolver. Prepared, recovery-required, and corrupt states must fail closed for ordinary retrieval.

Contract definition is not product completion. Production Forget preflight/apply/history, lifecycle artifacts, M2 exclusion, historical lifecycle projection, and the SOUL Lab Forget UI remain unimplemented.
...
- Phase I-4A route and schema names are target definitions and are not accepted by the current runtime.
...
- durable pin/merge or held apply/discard operations,
...
The memory write path remains explicitly gated. C1-5 and C2 provide restart-safe protected-source recovery and one exact queued-job execution; Phase I-1 provides ordinary scoped recall; Phase I-2 provides bounded read-only observation; Phase I-3 provides token-gated audited Correct with later M2 convergence. Phase I-4A adds a target contract only. Queue scheduling and the pre-enqueue background-finalizer crash window remain separate unresolved operational boundaries.
...
I1-G tracks the process-exit window after visible response delivery but before protected-source and B2 queue publication. Phase I-2 observation receipts and the Phase I-4A contract do not repair or reclassify that durability gap.
...
The dedicated target contract defines Forget as the user operation, `hidden` as the canonical retrieval-ineligible lifecycle state, and the Forget tombstone as the immutable runtime-private audit artifact. It selects an immutable successor Primary page with a new revision as lifecycle authority and requires one canonical resolver plus a shared Correct/Forget revision fence.

Production Forget apply, M2 exclusion, historical lifecycle projection, SOUL Lab Forget UI, restore/unhide, and physical deletion remain unimplemented. UI-B0, O0, I1-G, queue scanning, and daemon/service status are unchanged.
