...
LOCAL_PATH = Path("docs/evidence/migrations/cutover-1c56-phase-i3.md")
...
MARKER = "### C1C56-001 — Phase I-3 Auditable Primary MEM Correct handoff (pending merge attribution)\n"
FINAL_BLOCK = '''### C1C56-001 — Phase I-3 Auditable Primary MEM Correct handoff
...
if ledger.count(MARKER) != 1:
    raise SystemExit(f"expected exactly one pending C1C56 marker, found {ledger.count(MARKER)}")
prefix = ledger.split(MARKER, 1)[0]
LOCAL_PATH.write_text(LOCAL_CONTENT, encoding="utf-8")
LEDGER_PATH.write_text(prefix + FINAL_BLOCK, encoding="utf-8")

for temporary in (
    ".github/cutover-1c56-bookkeeping-bootstrap.txt",
    ".github/cutover-1c56-bookkeeping-apply.py",
    ".github/workflows/cutover-1c56-bookkeeping-apply.yml",
    ".github/workflows/cutover-1c56-bookkeeping-pr-apply.yml",
):
    path = Path(temporary)
    if path.exists():
        path.unlink()
