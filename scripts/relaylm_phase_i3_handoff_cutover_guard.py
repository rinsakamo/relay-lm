#!/usr/bin/env python3
"""Fail-closed guard for Documentation Hard Cutover 1C-56."""
...
RETIRED = "docs/architecture/phase_i3_auditable_primary_mem_correct.md"
CANONICAL = "docs/evidence/implementation/phase-i3-auditable-primary-mem-correct-handoff.md"
RETIRED_BASENAME = Path(RETIRED).name
SELF_PATH = "scripts/relaylm_phase_i3_handoff_cutover_guard.py"
...
REFERENCE_ALLOWLISTED_FILES = {
    "docs/evidence/migrations/cutover-1c56-phase-i3.md",
...
def check_canonical_metadata(errors: list[str], root: Path = ROOT) -> None:
    path = root / CANONICAL
    if not path.is_file():
        errors.append(f"{CANONICAL}: canonical Phase I-3 correction evidence is missing")
        return
    text = path.read_text(encoding="utf-8")
...
def self_test() -> None:
    ...
    run("local receipt may narrate retired source", lambda root: _write(root, "docs/evidence/migrations/cutover-1c56-phase-i3.md", f"source: {RETIRED}\n"), None)
    ...
    if failed:
        print("Phase I-3 cutover guard self-test FAILED", file=sys.stderr)
        for item in failed:
            print(f"- {item}", file=sys.stderr)
        raise SystemExit(1)
    print(f"Phase I-3 cutover guard self-test passed: {len(cases)} assertions")
...
def main() -> None:
    ...
    errors = validate()
    if errors:
        print("Phase I-3 cutover guard FAILED", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        raise SystemExit(1)
    print("Phase I-3 cutover guard passed")
