#!/usr/bin/env python3
"""Fail-closed guard for Documentation Hard Cutover 1C-50.

The completed Phase 5.5-C4 implementation handoff moved from the live
...
RETIRED = "docs/architecture/phase55c4_runtime_tts_transport_envelope_wiring.md"
CANONICAL = "docs/evidence/implementation/phase55c4-runtime-tts-transport-envelope-wiring.md"
RETIRED_BASENAME = Path(RETIRED).name
SELF_PATH = "scripts/relaylm_phase55c4_handoff_cutover_guard.py"
...
REFERENCE_ALLOWLISTED_FILES = {
    "docs/evidence/migrations/cutover-1c50-phase55c4.md",
...
def check_retired_path(errors: list[str], root: Path = ROOT) -> None:
    if (root / RETIRED).exists():
        errors.append(f"{RETIRED}: retired Phase 5.5-C4 handoff path reintroduced")
...
def check_canonical_metadata(errors: list[str], root: Path = ROOT) -> None:
    path = root / CANONICAL
    if not path.is_file():
        errors.append(f"{CANONICAL}: canonical Phase 5.5-C4 evidence is missing")
        return
...
def self_test() -> None:
    ...
    if failed:
        print("Phase 5.5-C4 cutover guard self-test FAILED", file=sys.stderr)
        for item in failed:
            print(f"- {item}", file=sys.stderr)
        raise SystemExit(1)
    print(f"Phase 5.5-C4 cutover guard self-test passed: {len(cases)} assertions")
...
def main() -> None:
    ...
    errors = validate()
    if errors:
        print("Phase 5.5-C4 cutover guard FAILED", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        raise SystemExit(1)
    print("Phase 5.5-C4 cutover guard passed")
