...
def main() -> None:
    with prepared_store() as (root, first_id):
        ...
        apply_primary_memory_pin(primary_writer_decision=PRIMARY_WRITER_DECISION, store_root=str(root), character_id=CHARACTER, namespace=NAMESPACE, memory_id=second_id, expected_revision=1, reason=PIN_REASON, operation_id="phase-i5b-ranking-pin", apply_token=pin["apply_token"], now=NOW)
        ...
        apply_primary_memory_unpin(primary_writer_decision=PRIMARY_WRITER_DECISION, store_root=str(root), character_id=CHARACTER, namespace=NAMESPACE, memory_id=second_id, expected_revision=1, reason=UNPIN_REASON, operation_id="phase-i5b-ranking-unpin", apply_token=unpin["apply_token"], now=NOW)
