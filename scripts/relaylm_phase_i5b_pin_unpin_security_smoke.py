...
def main() -> None:
    with prepared_store() as (root, memory_id):
        ...
        expect({"token_invalid"}, lambda: apply_primary_memory_pin(primary_writer_decision=PRIMARY_WRITER_DECISION, store_root=str(root), character_id=CHARACTER, namespace=NAMESPACE, memory_id=memory_id, expected_revision=1, reason=REASON, operation_id="phase-i5b-security-pin", apply_token=token + "x", now=NOW))
        expect({"token_invalid", "target_not_found", "stale_revision"}, lambda: apply_primary_memory_pin(primary_writer_decision=PRIMARY_WRITER_DECISION, store_root=str(root), character_id=CHARACTER, namespace="wrong-scope", memory_id=memory_id, expected_revision=1, reason=REASON, operation_id="phase-i5b-security-pin", apply_token=token, now=NOW))
        result = apply_primary_memory_pin(primary_writer_decision=PRIMARY_WRITER_DECISION, store_root=str(root), character_id=CHARACTER, namespace=NAMESPACE, memory_id=memory_id, expected_revision=1, reason=REASON, operation_id="phase-i5b-security-pin", apply_token=token, now=NOW)
