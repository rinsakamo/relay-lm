...
async def assert_runtime_handoff_dry_run_from_b2_safe_output() -> None:
    chunks = [
        sse_content(VISIBLE_PREFIX),
        sse_content(VISIBLE_SECOND),
        sse_content(RELAYCTX_UPDATE_OPEN + INTERNAL_BODY),
        sse_done(),
    ]
    ctx = DummyPipelineContext()
    b2_iter = wrap_stream_with_relayctx_suppression(
        iter_bytes(chunks),
        enabled=True,
        dry_run_only=False,
    ...
    require(VISIBLE_SECOND in decoded, decoded)
    ...
    print("ok runtime TTS handoff dry-run consumes B2 safe visible output")
...
async def assert_runtime_handoff_ready_without_tts_execution() -> None:
    ...
    output = await collect(c2_iter)
    require(VISIBLE_PREFIX in output.decode("utf-8"), output)
    hint_log = log_by_node(ctx, "relayctx_tts_segmentation_hints")
    handoff_log = log_by_node(ctx, "relayctx_tts_adapter_handoff")
...
async def assert_no_handoff_without_b2_safe_output() -> None:
    ...
    output = await collect(c2_iter)
    decoded = output.decode("utf-8")
    require(RELAYCTX_UPDATE_OPEN in decoded, decoded)
    require(INTERNAL_BODY in decoded, decoded)
    node_names = [result.node_name for result in ctx.node_results]
    require(node_names == ["relayctx_stream_suppression_gate"], node_names)
...
async def assert_invalid_safe_output_observation_blocks_handoff() -> None:
    ...
    output = await collect(
        wrap_stream_with_tts_adapter_handoff(
            iter_bytes([sse_multi_content(VISIBLE_PREFIX, VISIBLE_SECOND), sse_done()]),
            enabled=True,
            dry_run_only=False,
            b2_safe_visible_output_available=True,
            pipeline_context=ctx,
...
async def main_async() -> None:
    await assert_runtime_handoff_dry_run_from_b2_safe_output()
