...
def main() -> None:
    with prepared_store() as (root, memory_id):
        ...
        first = apply_primary_memory_pin(primary_writer_decision=PRIMARY_WRITER_DECISION, store_root=str(root), character_id=CHARACTER, namespace=NAMESPACE, memory_id=memory_id, expected_revision=1, reason=PIN_REASON, operation_id="phase-i5b-idempotent-a", apply_token=a["apply_token"], now=NOW)
        require(first.status == "applied", first)
        replay = apply_primary_memory_pin(primary_writer_decision=PRIMARY_WRITER_DECISION, store_root=str(root), character_id=CHARACTER, namespace=NAMESPACE, memory_id=memory_id, expected_revision=1, reason=PIN_REASON, operation_id="phase-i5b-idempotent-a", apply_token=a["apply_token"], now=NOW)
        require(replay.idempotent_replay is True, replay)
        second = apply_primary_memory_pin(primary_writer_decision=PRIMARY_WRITER_DECISION, store_root=str(root), character_id=CHARACTER, namespace=NAMESPACE, memory_id=memory_id, expected_revision=1, reason="second bounded reason", operation_id="phase-i5b-idempotent-b", apply_token=b["apply_token"], now=NOW)
        ...
        done = apply_primary_memory_unpin(primary_writer_decision=PRIMARY_WRITER_DECISION, store_root=str(root), character_id=CHARACTER, namespace=NAMESPACE, memory_id=memory_id, expected_revision=1, reason=UNPIN_REASON, operation_id="phase-i5b-idempotent-unpin", apply_token=u["apply_token"], now=NOW)
