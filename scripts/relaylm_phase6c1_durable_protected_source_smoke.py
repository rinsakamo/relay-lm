"""Sanitized bounded runner for Phase 6-C1-5 durable-source smokes."""
...
from relaylm_phase6c1_durable_source_uncertain_enqueue_smoke import (
...
_STAGES = (
    ...
    ("uncertain_enqueue_retention", uncertain_enqueue_retains_source),
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase 6-C1-5 durable protected source smoke failed: {current_stage}")
        return 1
    combined = captured_out.getvalue() + captured_err.getvalue()
    for token in PRIVATE_TOKENS:
        require(token not in combined, ("private token exposed", token))
