#!/usr/bin/env python3
"""Fail-closed guard for Documentation Hard Cutover 1C-57."""
...
RETIRED = "docs/architecture/relaymem_m3a_primary_formation_handoff.md"
CANONICAL = "docs/evidence/implementation/relaymem-m3a-primary-formation-handoff.md"
RETIRED_BASENAME = Path(RETIRED).name
SELF_PATH = "scripts/relaylm_relaymem_m3a_handoff_cutover_guard.py"
...
def check_canonical_metadata(errors: list[str], root: Path = ROOT) -> None:
    path = root / CANONICAL
    if not path.is_file():
        errors.append(f"{CANONICAL}: canonical RelayMEM M3a formation evidence is missing")
        return
    text = path.read_text(encoding="utf-8")
...
def self_test() -> None:
    ...
    run("local receipt may narrate retired source", lambda root: _write(root, "docs/evidence/migrations/cutover-1c57-relaymem-m3a.md", f"source: {RETIRED}\n"), None)
    ...
    if failed:
        print("RelayMEM M3a cutover guard self-test FAILED", file=sys.stderr)
        for item in failed:
            print(f"- {item}", file=sys.stderr)
        raise SystemExit(1)
    print(f"RelayMEM M3a cutover guard self-test passed: {len(cases)} assertions")
...
def main() -> None:
    ...
    errors = validate()
    if errors:
        print("RelayMEM M3a cutover guard FAILED", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        raise SystemExit(1)
    print("RelayMEM M3a cutover guard passed")
