...
def check_canonical_metadata(errors: list[str], root: Path = ROOT) -> None:
    path = root / CANONICAL
    if not path.is_file():
        errors.append(f"{CANONICAL}: canonical I-4C2 recovery/finalization evidence is missing")
        return
    text = path.read_text(encoding="utf-8")
...
def self_test() -> None:
    ...
    run("local receipt may narrate retired source", lambda root: _write(root, "docs/evidence/migrations/cutover-1c54-i4c2.md", f"source: {RETIRED}\n"), None)
    ...
    if failed:
        print("I-4C2 cutover guard self-test FAILED", file=sys.stderr)
        for item in failed:
            print(f"- {item}", file=sys.stderr)
        raise SystemExit(1)
    print(f"I-4C2 cutover guard self-test passed: {len(cases)} assertions")
...
def main() -> None:
    ...
    errors = validate()
    if errors:
        print("I-4C2 cutover guard FAILED", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        raise SystemExit(1)
    print("I-4C2 cutover guard passed")
